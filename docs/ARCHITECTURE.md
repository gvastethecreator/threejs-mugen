...
- `RuntimeActorConstraintControllerDispatchWorld` is the active-state CNS Width dispatch boundary: it records controller telemetry, extracts typed `collision:width` operations when available, records operation telemetry, and hands bounded body-width mutation to `RuntimeActorConstraintWorld`. `PlayableMatchRuntime` still owns trigger filtering, ordering, per-frame constraint reset, stage clamp, and body-push ordering.
