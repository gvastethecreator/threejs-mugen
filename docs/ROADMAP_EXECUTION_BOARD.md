...
- Current guard-effect proof: required `synthetic-imported-hitdef-guard-sound.json` checksum `fdf1f7f6` gates bounded `HitDef guardsound = S6,0` into attacker-side `PlaySnd` telemetry, and required `synthetic-imported-hitdef-guard-spark.json` checksum `72c8fa3a` gates bounded `guard.sparkno = S7000` plus `sparkxy = 12,-64` into attacker-side `HitSpark` telemetry after guarded imported direct-hit routes. This is event evidence only, not exact FightFX/common sprite or sound lookup, channel, timing, mixing, visual binding, layering, scale, palette, or full system-effect parity.
