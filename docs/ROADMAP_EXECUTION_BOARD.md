...
Current R2 helper proof: `HelperSystem` / `RuntimeEffectActorWorld` own explicit visual-helper removal by helper id or runtime serial, all-owner helper clear, p1/p2 store isolation, and the first bounded helper-local micro-VM for visual Helper actors. `RuntimeEffectSpawnWorld` passes owner runtime-program data and animations into the helper store. Focused `EffectActorSystem` and `EffectSpawnSystem` tests prove the bounded helper-store mutation and helper-local execution boundary, including helper-local control/metadata/resources/variables, helper-local int `VarRandom`, and helper-local `PlaySnd` / `StopSnd` sound-event telemetry, while redirects, parent/root/team ownership, helper fvar/sysvar `VarRandom`, exact helper-local sound timing/channel/redirect ownership, helper-owned visual effects/combat/projectiles/HitDefs/Explods, exact helper resource semantics, exact helper random stream/tick-order, and full Helper parity remain blocked.
