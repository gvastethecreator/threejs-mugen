...
Latest Studio/UI checkpoint: the Studio CSS hygiene pass puts active desktop command-center chrome, docks, mission strip, viewport framing, and console rules in `src/styles/studio-command-center.css`, adds `pnpm qa:css`, and removes exact duplicate CSS rules from the scanned styles. `pnpm qa:css` now reports 4,018 scanned rules, 423 duplicate selector keys, and 0 exact duplicate rules; broader shared chrome/ledger/status primitive extraction remains open. This is Studio/product-surface hygiene only; it does not replace the runtime next slice, prove new Studio workflows, or move port scores.
