...
- Build next recovery/guard proof not already covered by threshold/actor-frame tick-order/named controller-operation order/early-reject/positive/air-velocity/ground-selection/official ordered-threshold/official auto guard-start-end/official guard-hit frame-physics/required guard sound plus hit/guard spark telemetry/player-AIR, bounded common/FightFX system render/source-metadata routes, package-frame handoff tests, first-pass real fight.def/FightFX/common AIR/SFF loading/provider registration tests, renderer global-provider lookup tests, AIR-duration frame advance tests, and bounded AIR/SFF axis binding: broader recovery parity, deeper VM tick-order coverage, exact FightFX/common layering/scale/palette, or broader guard-effect parity.
