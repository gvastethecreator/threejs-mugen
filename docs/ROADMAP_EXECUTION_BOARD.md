...
spawn and an accepted falling contact reaches existing `HitFall`/`GetHitVar`
consumers. Required `synthetic-imported-projectile-dynamic-fall-flags` passes
at `a39589d7` / `fb879f83`; full tests, typecheck, build and trace QA `887/887`
...
contact retains finite siblings through existing recovery-window and `GetHitVar`
paths while preserving `owner/root = p1` and `parent = p1-helper-0`. Required
`synthetic-imported-helper-projectile-dynamic-fall-recovery` passes at
`d1594d58` / `a3042cd6`; full tests `328/4100`, typecheck, build and trace QA
`886/886` (`852` required) pass. Nested/`ownProjectile` topology,
...
`fall.recover`/`fall.recovertime`/`down.recover`/`down.recovertime` for one
fresh root-owned Projectile. One accepted falling contact retains finite
siblings independently through existing recovery-window and `GetHitVar` paths.
Required `synthetic-imported-projectile-dynamic-fall-recovery` passes at
...
Issue 364 closes active EnvShake diradd/decay. Product 63ef2769, 25 focused
tests, typecheck and diff hygiene pass. The required trace
synthetic-imported-envshake-diradd-decay records dirAdd=20 and decay=1.25.
...
Issue 353 closed Helper caller-context broadcast of live `ModifyProjectile`
`air.velocity`; its evidence remains in commits `513d8e58` / `08c1594e`.
...
context evaluation, pinned zero-fill one/two/three-component writes,
...
retains the inherited helper-bind target-link blocker; fresh/default derivation,
...
`8b9552db` (`b7850d2e` -> `bce06ab9`) passes. The Helper creates a root-owned
Projectile, resolves `airguard.velocity=-2,var(1)` with `var(1)=-8`, and an
accepted airborne guard observes the replaced physical Y response; defender
life ends at `998`. Focused tests, typecheck and diff hygiene pass. Aggregate
QA stops on the inherited helper-bind target-link blocker. Z/default
...
`airguard.velocity` X readback. Evidence commit `c3438d6e`; required trace
`641792d6` (`b7850d2e` -> `61647146`) passes. The accepted guard observes
`maxVel.x=6` and defender life `998`. See [issue
...
trace `3d47deb8` (`fcbebd7b` -> `ae032d99`) passes. The accepted guard exposes
`GetHitVar(xvel)=8`, physical guard velocity, and defender life `20`. See
[issue 344](../.scratch/roadmap/issues/344-helper-modifyprojectile-guard-velocity.md).
...
`938303dd` -> `0073df18` passes. The Helper creates a root-owned Projectile,
resolves `redlife=var(0),0` in Helper context, and the accepted unguarded hit
ends at `life=5/redLife=20` while authored `GetHitVar(redlife)=40` remains
separate. Focused Helper/trace tests, typecheck and diff hygiene pass. The full
trace suite is 810/811 and aggregate QA stops on the inherited
`synthetic-imported-helper-bind-to-target-redirect` target-link blocker; an
unrelated EffectActor guardpoints assertion remains known. Guard contact,
nested/shared-resource topology, exact arithmetic/timing, rollback and full
parity remain outside. See [issue
...
and the real direct InGuardDist latch. Fresh components use bounded defaults;
live ModifyHitDef preserves omitted siblings. Product `194c0b6`; evidence
...
case. MUGEN 1.1 scalar `guard.dist` and Ikemen bounds are separate claims;
...
caller-context values preserve omitted live components and reach accepted
ground-guard `GetHitVar`/velocity metadata. Required trace
`synthetic-imported-modifyhitdef-dynamic-guard-velocity-yz.json` proves
`VarSet -> HitDef -> ModifyHitDef -> guard`, with checksums `a2eb52db` /
`f0fb19a8`; aggregate QA passes `824/824` artifacts (`790` required, `34`
optional). M.U.G.E.N 1.1 only documents ground-guard X; air guard, Projectiles
and exact physics parity remain excluded. See [issue
...
caller-context dynamic suffixes preserve their `F`/`S`/`M` prefix; omission
or an unresolved suffix preserves the active identity. The required guard trace
proves `S7000 -> F19`, angle `-5`, offset `(-2,-3)`, and no hit route.
...
eight entries resolve once in caller context; finite values truncate and an
incoming `HitDef` id matching a non-negative entry is rejected. Focused
...
`ModifyReversalDef` `chainid` expressions. Fresh and live values resolve once
in caller context; finite values truncate, explicit negatives disable the
requirement, and reversal admission rejects a mismatched latest `HitDef` id.
Focused compiler/runtime coverage passes `179/179`; typecheck and diff hygiene
pass. No durable end-to-end trace is claimed because the exploratory fixture
did not isolate contact ordering. See [issue 299](../.scratch/roadmap/issues/299-reversaldef-chainid-expressions.md).
...
resolve once in the original caller context and clamp the resulting target id
to a non-negative integer. Accepted reversal contact carries the resolved id
through target memory and `GetHitVar(hitid)`. Required trace
`27b44d26/c0adb366`; aggregate QA passes `815/815` artifacts (`781` required,
`34` optional). `chainid`/`nochainid`, Helper-owned ModifyReversalDef,
Projectile/ModifyProjectile, priority/hitonce, exact id/tick parity,
...
Issue 299 proposes caller-context `chainid` matching and incompatible-chain
rejection for direct/root `ReversalDef` and root/RedirectID
`ModifyReversalDef`. `nochainid`, Helper-owned mutation,
Projectile/ModifyProjectile, priority/hitonce, exact tick behavior, teams,
rollback and full ReversalDef parity remain excluded. See [issue
...
replacement retain typed static or dynamic values, resolve them once in the
original caller context, and accepted reversal contact consumes the resolved
hit-count metadata. Required trace `92c3151e/4cecfee3`; aggregate QA passes
`812/812` artifacts (`778` required, `34` optional). This is a pinned Ikemen
HitDef-parameter reuse slice; the local M.U.G.E.N ReversalDef reference does
not document a standalone optional `numhits` field. Helper-owned
ModifyReversalDef, Projectile reflection, negative/overflow int32 parity,
exact combo/tick ordering, teams, rollback, and full ReversalDef parity
remain blocked. See [issue 295](../.scratch/roadmap/issues/295-reversaldef-numhits-expressions.md).
...
resolved once and applied to the selected Projectile, preserving the existing
state-transition ownership path and the authored-state default for
`p2getp1state`. Required trace `16fdce2c/8ed5f8c8`; aggregate QA passes
`807/807` (`773` required, `34` optional). `p1facing`/`p1getp2facing`,
...
active stun tick, then release it before the next active state-controller pass
while retaining the remaining hit metadata. Required traces are
`05c07804/3445d810` (root) and `46636488/719f5d6e`; the two new artifacts are
green. The full aggregate runner reached `803/803` generated artifacts before
its wall-clock timeout, so the global count remains inconclusive until a
fresh run completes. Exact actionRun timing, resource cleanup, hitonce,
facing, custom-state ownership, teams, rollback and full parity remain blocked.
See [issue 286](../.scratch/roadmap/issues/286-projectile-keepstate-release.md).
...
are implemented and covered by focused tests. Required traces are
`a00bf194` (root fresh), `a2c03112` (Helper fresh) and `6bb8fe78`
(`ModifyProjectile` root). Aggregate QA passes `797/797` artifacts (`763`
required, `34` optional), full Vitest passes `328/3849`, and the production
build is green. Keep local omitted Projectile damage=30 and exclude negative
...
The Helper-local typed expression resolves once in caller context, mutates only
the helper-parented Projectile, resets AIR playback, and leaves a same-id
player-owned Projectile unchanged. Required trace checksum is `705a96e0` /
final `f47441cf`; aggregate QA passes `791/791` artifacts (`757` required,
`34` optional), with focused Helper/runtime coverage `140/140`. Typecheck,
full tests, build, and diff hygiene are recorded by the closeout commits.
FFX, cross-namespace broadcast, nested teams, terminal timing, rollback, and
full Helper/Projectile parity remain blocked. See [issue
280](../.scratch/roadmap/issues/280-helper-modifyprojectile-anim-dynamic.md).

## Authoritative runtime cursor — T705 closed-bounded

T705 / issue 279 is closed-bounded for root-owned live Ikemen
`ModifyProjectile projanim`. The typed operation preserves static or dynamic
one-value expressions, resolves them once in caller context, replaces the
selected AIR action, resets playback, and exposes the result through
`ProjVar(id, index, anim)`. The required trace checksum is `ed1f6e8a` / final
`43956cf2`; focused compiler/spawn coverage is `162/162`; aggregate QA passes
`790/790` artifacts (`756` required, `34` optional), with typecheck and diff
hygiene green. FFX prefixes, exact negative/overflow and invalid-action
behavior, Helper-owned mutation, broadcast order, terminal playback, teams,
...
optional), root trace checksum `0e1c310f` / final `42890c95`, Helper checksum
`4a23726b` / final `93048c4f`, focused compiler/spawn coverage `238/238`,
typecheck, and diff hygiene pass.

T704 / issue 278 is closed-bounded: fresh root- and Helper-authored
Projectiles preserve typed static or dynamic `projanim`, resolve it once in the
original caller context before AIR action lookup, and prove selected action,
ownership, target payload, and spawn/active/remove lifecycle. Live
`ModifyProjectile` animation mutation, FFX action prefixes, exact
invalid-action timing, multi-projectile broadcast order, nested teams,
...
optional), with required fresh root Projectile `down.hittime` trace checksum
`800059f` and final checksum `ac8eff48`. The full `3793/3793` Vitest suite
across `328` files, typecheck, the `363`-module build, and the diff gate pass.

T693 / issue 267 is closed-bounded: fresh root Projectiles resolve dynamic
`down.hittime` once in caller context, retain the pinned fresh `20` default
when omitted, and expose the authored value through accepted lying-hit
`GetHitVar(hittime)`. Projectile target/lifecycle and imported Common1-style
progression are required. Helper caller resolution is covered by focused
tests; live `ModifyProjectile`, non-zero down launch, exact countdown/landing
timing, and full Projectile timing parity remain excluded. See [issue 267](../.scratch/roadmap/issues/267-projectile-down-hittime-dynamic.md).
...
`e3cfd800` and final checksum `6401d0a9`. The full `3788/3788` Vitest suite
across `328` files, typecheck, the `363`-module build, and the diff gate pass.

T691 / issue 265 is closed-bounded: fresh direct HitDefs resolve dynamic
`down.hittime` once in root/Helper caller context, use the pinned fresh default
`20` without inheriting a prior move, and root-owned `ModifyHitDef` replaces or
preserves the live scalar in caller context. An accepted lying hit with
`down.velocity.y = 0` exposes the authored timer through `GetHitVar(hittime)`
and imported Common1-style progression. Projectile, nonzero down launch,
Helper-owned live mutation, exact countdown/landing timing, and full timing
parity remain excluded. See [issue 265](../.scratch/roadmap/issues/265-hitdef-down-hittime-dynamic.md).
...
Final T683 verification: `758/758` trace artifacts (`724` required, `34`
optional), with root Projectile `down.velocity` checksums
...
optional), with required root-owned `ModifyProjectile airguard.velocity`
trace checksum `c4d55bd9` and final checksum `f0982ef8`. The full
`3764/3764` Vitest suite across `328` files, typecheck, the `363`-module
build, and the diff gate pass.
...
evidence. T683 / issue 257 is closed-bounded: fresh root/Helper Projectiles
...
T685 / issue 259 is closed-bounded for live root-owned
`ModifyProjectile airguard.velocity` mutation: caller-context dynamic/mixed
values broadcast to selected Projectiles with Ikemen's zero-filled one-,
two-, and three-component semantics, and the accepted airborne-guard trace
exposes the resulting vector through physics/GetHitVar and lifecycle/target
evidence.
T686 / issue 260 is closed-bounded for live root-owned `ModifyProjectile
air.velocity` mutation: caller-context dynamic/mixed values broadcast to
...
T688 / issue 262 is closed-bounded for live root-owned `ModifyProjectile
ground.velocity` mutation: caller-context dynamic/mixed X/Y/Z values replace
the selected Projectile's grounded velocity component-wise, and the accepted
grounded-hit trace exposes GetHitVar/physics plus target/lifecycle evidence.
Helper-owned mutation, dynamic `n`, fresh defaults, and full Projectile parity
remain excluded.
...
with component-wise preservation and accepted grounded-hit GetHitVar/physics,
target, and lifecycle evidence. The required trace checksum is `0cda3247`.
Helper-owned mutation remains blocked by the pinned upstream helper guard. See
[issue 262](../.scratch/roadmap/issues/262-modifyprojectile-ground-velocity-dynamic.md).
...
Ikemen's zero-filled one/two/three-component broadcast semantics. The
required airborne-hit trace checksum is `c335ca9d` and final checksum is
`2ecb639a`; Helper-owned mutation remains blocked by the pinned upstream
helper guard. See [issue 260](../.scratch/roadmap/issues/260-modifyprojectile-air-velocity-dynamic.md).

T685 / issue 259 is closed-bounded: root-owned live Projectiles accept
caller-context dynamic `ModifyProjectile airguard.velocity` replacement with
Ikemen's zero-filled one/two/three-component broadcast semantics. The
required airborne-guard trace checksum is `c4d55bd9` and final checksum is
`f0982ef8`; Helper-owned mutation remains blocked by the pinned upstream
helper guard. See [issue 259](../.scratch/roadmap/issues/259-modifyprojectile-airguard-velocity-dynamic.md).
...
T683 / issue 257 is closed-bounded: fresh root/Helper Projectiles resolve
caller-context dynamic/mixed `down.velocity` X/Y/Z, inherit omitted components
from effective `air.velocity`, and prove accepted lying-hit physics/GetHitVar
plus lifecycle/ownership evidence. Root checksums are `869ee367/95984159`;
Helper checksums are `3d0f88d0/9aaf44ce`. See [issue 257](../.scratch/roadmap/issues/257-projectile-down-velocity-dynamic.md).
...
T664 / issue 238 is closed-bounded: exact X/Y `airguard.velocity` pairs replace
root-owned live ModifyHitDef X/Y in caller context, preserve Z and omission,
and feed accepted airborne guard plus GetHitVar. The full suite passes
...
T670 / issue 244 is closed-bounded: accepted root/Helper direct airborne hits
select the existing static `air.velocity` X/Y/Z vector instead of the ground
vector, while grounded/down/guard selection remains unchanged. The full suite
passes 3719/3719; required trace checksum is `1fe99cdb`; aggregate traces pass
743/743 with 709 required; typecheck and the 363-module build pass.

T671 / issue 245 is closed-bounded: direct-HitDef `air.velocity` X/Y
expressions resolve in root/Helper caller context, single values materialize Y
zero, and accepted airborne hits expose the result through physics/GetHitVar.
The full suite passes 3722/3722; required trace checksum is `18588507`;
...
T673 / issue 247 is closed-bounded: fresh direct-HitDef `down.velocity` X/Y
expressions resolve in root/Helper caller context, inherit each missing
component from effective air velocity, and feed accepted lying-hit
physics/GetHitVar. The full suite passes 3729/3729; required trace checksum is
...
T683 / issue 257 closes the fresh Projectile `down.velocity` source gate with
root/Helper caller-context inheritance and lying-hit evidence. T684 / issue
258 is now the active live `ModifyProjectile down.velocity` source gate.
