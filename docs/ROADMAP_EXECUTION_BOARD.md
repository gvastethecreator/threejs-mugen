...
Latest implementation checkpoint: required `synthetic-imported-default-fall-gethit.json` checksum `6af73a91` now gates bounded Common1-style fall get-hit entry order with ordered P2 actor-frame evidence (`5000 -> 5030 -> 5050`, all `moveType = H`) and controller/typed-operation evidence through `5000` `ChangeState`, `5030` `VelAdd` / `HitVelSet` / `kinematic:hitvelset` / `ChangeState`, and `5050` `VelAdd` / `ChangeState`. Optional private-fixture `kfm-official-default-fall-gethit.json` checksum `813ff55d` now requires matching bounded official KFM order through `5000 -> 5030 -> 5050 -> 5100 -> 5101 -> 5110`, including `HitVelSet`, `kinematic:hitvelset`, ground-impact `VelSet` / `kinematic:velset`, bounce `HitFallVel` / `hitfall:hitfallvel`, and lie-down `HitFallDamage` / `hitfall:hitfalldamage` evidence when `.scratch/fixtures/kfm-official.zip` exists. Previous required `synthetic-imported-default-fall-recovery.json` checksum `d83797d9` still gates bounded Common1-style lie-down/get-up recovery order with ordered P2 actor-frame evidence (`5110` with `moveType = H` before `5120` with `moveType = I`) and controller/typed-operation evidence through `5110` `ChangeState` before `5120` `VelSet` / `kinematic:velset` / `HitFallSet` / `hitfall:hitfallset` / `ChangeState`. Optional private-fixture `kfm-official-default-fall-recovery.json` checksum `978b8343` still requires matching actor-frame order plus bounded official KFM `5110`/`5120` controller/typed-operation order when the private fixture exists. Previous required `synthetic-imported-default-gethit-progression.json` checksum `ef2a67f8` still gates bounded Common1-style stand get-hit progression with ordered P2 controller events (`5000` `ChangeState` before `5001` `ChangeState`), actor-frame `5000 -> 5001` evidence, and final idle/control. Previous required presentation gates `synthetic-imported-hitdef-common-spark.json` checksum `5ea054d7` and `synthetic-imported-hitdef-fightfx-spark.json` checksum `11537b56` still gate bounded common/default and FightFX `HitSpark` asset-frame metadata before renderer handoff. Optional private-fixture `kfm-official-x-hit-sound.json` and `kfm-official-x-hit-spark.json` still pass with checksum `9668e88a`, proving the real KFM `x -> 200` route emits bounded `HitDef` hit sound telemetry `S5,0` and bounded `HitSpark` telemetry from authored `sparkno 0` after contact when `.scratch/fixtures/kfm-official.zip` exists. Previous required runtime checkpoint `synthetic-imported-target-noko.json` checksum `28ac8636` gates bounded target-controller no-KO behavior: defender-side `AssertSpecial NoKO` runs before P1 `HitDef` and later lethal `TargetLifeAdd`, target-link evidence proves P1 still owns target id `77`, and final P2 life clamps to `1`. Previous required `synthetic-imported-target.json` remains checksum `f5a16dc9` and gates bounded TargetLifeAdd/TargetPowerAdd/TargetVel*/TargetFacing/TargetBind/BindToTarget/TargetDrop through typed operations, target-link/binding evidence, P2 facing/velocity telemetry, final P1 `targetCount = 0`, and final P2 `life = 943` / `power = 40`. `RuntimeSnapshotWorld` remains the latest R2 ownership checkpoint for player actor snapshot projection previously inline in `PlayableMatchRuntime`: actor identity/source, sprite-owner metadata, cloned runtime state, target refs/bindings, active move or AIR-frame collision boxes, fallback hurtbox, and cloned sound/hit-effect/env-shake event histories. `RuntimeCompatibilityTelemetryWorld` remains the imported compatibility telemetry/session checkpoint, `RuntimeSnapshotWorld` also remains the stage/camera snapshot projection checkpoint, `RuntimeAssertSpecialWorld` remains the imported pre-facing `AssertSpecial` checkpoint, `RuntimeHitPauseWorld` remains the hitpause checkpoint, `RuntimePausedMatchWorld` remains the regular pause checkpoint, `RuntimeStunWorld` remains the stun checkpoint, `RuntimeStateAvailabilityWorld` remains the state/action availability checkpoint, `RuntimeHitStateTransitionWorld` remains the custom-state transition checkpoint, `RuntimeGetHitStateWorld` remains the default get-hit checkpoint, `RuntimeGuardWorld` remains the guard checkpoint, `RuntimeOrientationWorld` remains the orientation checkpoint, `RuntimeHitEligibilityWorld` remains the hit-eligibility lifetime/reset checkpoint, `RuntimeTargetWorld.applyTargetBindings` / `applyBindToTarget` remain the active target-binding checkpoint, `RuntimeRecoverySystem` remains the recovery checkpoint, `HitSparkAssetSystem` remains the presentation checkpoint, and `RuntimeRandomSystem` remains the deterministic random checkpoint with required `synthetic-imported-variable.json` checksum `3b33f7a8`. This is R1 trace strengthening for bounded Common1 fall-entry order, lie-down/get-up order, stand get-hit order, common/FightFX source-frame metadata, optional local KFM HitDef presentation telemetry, bounded two-actor Target* / NoKO, and the existing R2 snapshot ownership checkpoint; it is not public bundled KFM support, exact `HitShakeOver`/`HitOver` timing, exact fall/bounce/liedown/recovery tick-order parity, SND decode/playback, exact FightFX/common lookup, render timing, layering, scale, palette, motif ownership, full target redirects, helper/root/parent target ownership, team or multi-target semantics, exact NoKO lifetime, exact bind/drop tick-order, renderer parity, motif/screenpack camera rules, full Common1 tick-order parity, exact presentation parity, exact MUGEN random stream, or VM parity.

Latest Studio/UI checkpoint: the Studio surface extraction imports `src/styles/studio-trust-ledgers.css`, `src/styles/studio-system-ledgers.css`, `src/styles/studio-command-palette.css`, `src/styles/studio-stage.css`, and `src/styles/studio-inspector.css` after the command-center/workbench/assets modules. Build/Evidence right-rail rows, status cells, key-value tables, trace strips, and Impact/Next copy live in the trust-ledger module; Modules/Debug contracts, runtime focus, compact rows, target/effect/pause/audio lenses, and execution-evidence rows live in the system-ledger module; Command Palette, Stage, and Inspector desktop density live in their surface modules. The latest CSS cleanup removed obsolete legacy Evidence/Release Desk blocks, fully overridden module-covered rules, the old global Module ledger repair block, command-center desktop overrides, and dead `tab-dot*` CSS from `src/style.css`; `src/styles/studio-command-center.css` now owns the Studio chrome/mission/viewport/console surface, `src/styles/studio-system-ledgers.css` owns two-line Module rows plus 40px system actions, and mission rows plus compact Studio tabs expose textual status. `pnpm qa:css` now reads CSS files in `src/main.ts` import order and reports cross-file selector overlap plus legacy `style.css` rules fully shadowed by later imports. Current working-tree audit reports 2,622 scanned rules, 115 duplicate selector keys / 264 instances, 0 exact duplicate rules, 216 repeated declaration groups, 40 cross-file duplicate selectors, 16 selectors shared with `src/style.css`, 3 command-center selectors still shared with legacy `src/style.css`, and 0 legacy `style.css` rules fully shadowed by later imports. Broader token consolidation, dense typography cleanup, shared chrome/status primitives, and remaining legacy `src/style.css` selector-cascade reduction remain open. This is Studio/product-surface hygiene only; it does not replace the runtime next slice, prove new Studio workflows, or move port scores.
...
3. R1 presentation: deepen FightFX/common spark presentation beyond first-pass package `fight.def`/AIR/SFF loading, decoded system-SFF provider registration, bounded AIR-duration frame advance, bounded AIR/SFF axis binding, and required source-frame trace metadata: exact render lookup, layering, scale, palette, timing, and motif/screenpack ownership remain open.
...
- Current proof: `RuntimePausedMatchWorld` owns bounded regular `Pause` / `SuperPause` paused-match ordering consumed by `PlayableMatchRuntime`, with focused `PauseSystem` coverage for source `movetime`, frozen actor presentation, pause replacement interruption, and pause countdown ticking while trace behavior is expected to remain stable.
- Current proof: `RuntimeHitPauseWorld` owns bounded global hitpause ordering consumed by `PlayableMatchRuntime`, with focused `RuntimeHitPauseSystem` coverage for command buffering, `ignorehitpause` dispatch, paused presentation, actor countdown, and no-op behavior outside hitpause while trace behavior is expected to remain stable.
...
- Current proof: `RuntimeSnapshotWorld` owns bounded stage/camera and player actor snapshot projection consumed by `PlayableMatchRuntime`, with focused system coverage for `ScreenBound` camera exclusion/fallback, EnvShake/EnvColor handoff, actor identity/source/sprite-owner metadata, cloned runtime/event histories, target refs/bindings, active/frame collision boxes, and missing-frame fallback hurtbox while `pnpm qa:trace` stays stable at 156/156 artifacts.
