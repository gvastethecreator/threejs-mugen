...
Latest R2 ownership addendum: `RuntimeResourceSystem` now owns shared authored life/power max resolution, runtime power-delta clamping, and control-flag writes used by `PlayableMatchRuntime`, `RuntimeDirectCombatWorld`, and `RuntimeReversalWorld`, replacing duplicate local resource-max helpers and inline power/control mutation paths. Focused resource, direct-combat, reversal, and playable-runtime tests prove existing bounded semantics stay intact. This is architecture debt reduction only; it does not add new controller semantics, exact CNS resource timing, helper/team/redirect resource ownership, or score movement.
