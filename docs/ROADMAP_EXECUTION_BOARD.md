...
- Current guard proof: required `synthetic-imported-default-guard-state.json` checksum `016938a1` gates the bounded stand guard-hit controller/operation route from `150` `ChangeAnim` / `ChangeState` into `151` `HitVelSet` / `kinematic:hitvelset` / `CtrlSet` / `resource:ctrlset` / `ChangeState`.
