...
Latest implementation checkpoint: `RuntimeOrientationWorld` owns bounded automatic facing and `Turn` facing flips previously inline in `PlayableMatchRuntime` / `StateControllerExecutor`. `RuntimeHitEligibilityWorld` remains the hit-eligibility lifetime/reset ownership checkpoint; `RuntimeTargetWorld.applyTargetBindings` / `applyBindToTarget` remain the active target-binding ownership checkpoint; `RuntimeRecoverySystem` remains the recovery ownership checkpoint; `HitSparkAssetSystem` remains the presentation ownership checkpoint; `RuntimeRandomSystem` remains the deterministic random ownership checkpoint with required `synthetic-imported-variable.json` checksum `3b33f7a8`. This is R2 orientation ownership cleanup, not exact auto-facing order, target-facing/team/helper semantics, redirect ownership, exact Common1 recovery thresholds, FightFX/common presentation parity, exact MUGEN random stream, or VM parity.
