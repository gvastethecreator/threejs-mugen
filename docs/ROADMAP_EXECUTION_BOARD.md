...
Latest implementation checkpoint: `RuntimeSnapshotWorld` owns bounded stage/camera snapshot projection previously inline in `PlayableMatchRuntime`: camera center respects `ScreenBound moveCameraX = 0`, falls back when all actors disable camera following, applies stage camera offsets, and carries EnvShake/EnvColor presentation data into `StageSnapshot`. `RuntimeAssertSpecialWorld` remains the imported pre-facing `AssertSpecial` ownership checkpoint, `RuntimeHitPauseWorld` remains the hitpause ownership checkpoint, `RuntimePausedMatchWorld` remains the regular pause ownership checkpoint, `RuntimeStunWorld` remains the stun ownership checkpoint, `RuntimeStateAvailabilityWorld` remains the state/action availability checkpoint, `RuntimeHitStateTransitionWorld` remains the custom-state transition ownership checkpoint, `RuntimeGetHitStateWorld` remains the default get-hit ownership checkpoint, `RuntimeGuardWorld` remains the guard ownership checkpoint, `RuntimeOrientationWorld` remains the orientation ownership checkpoint, `RuntimeHitEligibilityWorld` remains the hit-eligibility lifetime/reset ownership checkpoint, `RuntimeTargetWorld.applyTargetBindings` / `applyBindToTarget` remain the active target-binding ownership checkpoint, `RuntimeRecoverySystem` remains the recovery ownership checkpoint, `HitSparkAssetSystem` remains the presentation ownership checkpoint, and `RuntimeRandomSystem` remains the deterministic random ownership checkpoint with required `synthetic-imported-variable.json` checksum `3b33f7a8`. This is R2 snapshot projection ownership cleanup, not exact stage/motif camera logic, actor snapshot projection, compatibility session ownership, renderer parity, screenpack ownership, Common1 tick-order parity, FightFX/common presentation parity, exact MUGEN random stream, or VM parity.

Latest Studio/UI checkpoint: the Studio chrome CSS containment pass merged the duplicate desktop correction block into the main command-desk block after the redundant override hid the Build/Compile action in smoke. `pnpm qa:smoke` now confirms Workbench desktop/tablet have no horizontal overflow, command palette keyboard execution still compiles, and Studio Build still exports a passing trace package. This is Studio/product-surface evidence only; it does not replace the runtime next slice, finish broader CSS primitive extraction, or move port scores.
