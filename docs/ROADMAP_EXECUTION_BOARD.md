...
Latest R2 ownership addendum: `RuntimeEffectActorWorld.countActors(...)` now owns the unified bounded effect-count query consumed by `PlayableMatchRuntime` for `NumExplod`, `NumHelper`, and `NumProj`/`NumProjID` trigger callbacks. Focused `EffectActorSystem` coverage proves Explod/Helper/Projectile counts, id filters, and removed-projectile exclusion through one world query. This is architecture debt reduction only; it does not add helper VM execution, exact projectile lifetime parity, parent/root/team scopes, or broader effect semantics.

Latest Studio/UI checkpoint: the Studio CSS cascade now imports smaller ownership modules in `src/main.ts` import order after the base stylesheet: app shell, legacy/editor/hardening/runtime command-room/desktop authority slices, runtime command decks, command-center, workbench/assets, Build/Evidence trust ledgers, Modules/Debug system ledgers, Command Palette, Stage, and Inspector. `src/style.css` remains a small base/reset stylesheet. Clean `HEAD` `pnpm qa:css` reports 2,610 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 173 repeated declaration groups, 143 cross-file duplicate selectors, 0 selectors shared with `src/style.css`, and 0 legacy `style.css` rules fully shadowed by later imports. Broader token consolidation, dense typography cleanup, shared chrome/status primitives, and remaining Studio cross-file selector reduction remain open. This is Studio/product-surface hygiene only; it does not replace the runtime next slice, prove new Studio workflows, or move port scores.
