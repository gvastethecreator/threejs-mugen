...
3. R1 presentation: deepen FightFX/common spark presentation beyond first-pass package `fight.def`/AIR/SFF loading, decoded system-SFF provider registration, bounded AIR-duration frame advance, and bounded AIR-frame offsets: exact binding, layering, scale, palette, and motif/screenpack ownership remain open.
...
- Build next recovery/guard proof not already covered by threshold/actor-frame tick-order/named controller-operation order/early-reject/positive/air-velocity/ground-selection/official ordered-threshold/official auto guard-start-end/official guard-hit frame-physics/required guard sound plus hit/guard spark telemetry/player-AIR, bounded common/FightFX system render/source-metadata routes, package-frame handoff tests, first-pass real fight.def/FightFX/common AIR/SFF loading/provider registration tests, renderer global-provider lookup tests, and AIR-duration frame advance tests: broader recovery parity, deeper VM tick-order coverage, exact FightFX/common binding/layering/scale/palette, or broader guard-effect parity.
