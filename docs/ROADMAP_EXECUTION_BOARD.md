...
- Current proof: `RuntimeSnapshotWorld` owns bounded stage/camera snapshot projection consumed by `PlayableMatchRuntime`, with focused system coverage for `ScreenBound` camera exclusion/fallback and EnvShake/EnvColor handoff while `pnpm qa:trace` stays stable at 156/156 artifacts.
