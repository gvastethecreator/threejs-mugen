...
- Current trigger proof: focused compiler/evaluator/runtime-controller tests cover bounded `HitPauseTime` expression support against the current actor hitpause counter, and focused match-runtime tests prove imported active-state controllers with `ignorehitpause = 1` can run during global hitpause while the same controller without `ignorehitpause` stays frozen. Persistent controller semantics, helper-owned controller execution, and exact hitpause tick-order parity remain blocked.
