...
Latest R2 helper lifecycle addendum: `HelperSystem` now owns a bounded helper-local micro-VM for current visual Helper actors spawned with owner runtime-program data. `RuntimeEffectSpawnWorld` hands the owner runtime program and animation map to Helpers; focused `EffectActorSystem` coverage proves helper-local `Time` trigger evaluation, `VelSet`, `ChangeAnim`, `ChangeState`, `DestroySelf` removal, helper-local `CtrlSet` / `StateTypeSet`, helper-local `LifeAdd` / `LifeSet` / `PowerAdd` / `PowerSet`, and helper-local `VarSet` / `VarAdd` / `VarRandom` / `VarRangeSet` trigger branches, while `EffectSpawnSystem` coverage proves the runtime-program/animation handoff. This is architecture debt reduction plus a tiny helper-local execution subset; it does not add redirects, parent/root/team ownership, `keyctrl`, helper fvar/sysvar `VarRandom`, helper audio/effects, helper-owned HitDefs/Projectiles/Explods, helper combat, exact helper resource semantics, exact random stream parity, exact tick-order/pause parity, full custom-state helper lifecycle, or score movement.
