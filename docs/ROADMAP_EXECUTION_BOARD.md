...
- Current trigger proof: focused compiler/evaluator/runtime-controller tests cover bounded `HitPauseTime` expression support against the current actor hitpause counter. Ordinary controller execution during global hitpause, `ignorehitpause` semantics, and exact hitpause tick-order parity remain blocked.
