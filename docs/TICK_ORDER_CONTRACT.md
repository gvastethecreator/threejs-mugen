...
The match profile is explicit. `ikemen-go` snapshots the two current root players' MoveType before current-tick command/input mutation, then orders all root prepare and run passes by attacking (`A`) before idle (`I`) before remaining (`H`), with lower runtime id breaking ties. `mugen-1.1` and `unknown` retain input pair order because equivalent MUGEN behavior is not established. This does not cover `RunFirst` / `RunLast`, `RunOrder`, helpers, appended actors, or team modes.
...
Current guard behavior is useful but not parity. Both roots receive a pre-controller auto guard-start check in selected RunOrder before either advances, then each root receives a post-controller check after its own pass. Imported defenders holding back can enter defender-owned Common1-style `120 -> 130` when latched `InGuardDist` is true before contact. A bounded guard-end bridge can then leave through `140` and return to idle/control after `InGuardDist` is no longer true. Optional KFM fixture auto guard gates now require matching real Common1 controller/operation order for `120 -> 130 -> 140 -> 0`, including `StateTypeSet` / `metadata:statetypeset` and return-to-idle `VelSet` / `kinematic:velset` evidence when the private fixture is present. The required synthetic stand, crouch, atomic-`DB`, and air guard-hit gates now also prove ordered named controller/typed-operation evidence for `150 -> 151`, `152 -> 153`, and `154 -> 155`: `ChangeAnim` -> `ChangeState` -> `HitVelSet` -> `kinematic:hitvelset` -> optional air `VelAdd` -> `CtrlSet` -> `resource:ctrlset` -> `ChangeState`. The required `synthetic-imported-air-guard-landing.json` gate extends the synthetic air route through `154 -> 155 -> 52 -> 20`, requiring `155` `HitVelSet`, `VelAdd`, `CtrlSet`, landing `VelSet` / `kinematic:velset`, landing `PosSet` / `kinematic:posset`, final `ChangeState`, state-`52` y = 0 actor-frame evidence, and final imported walk/control `20`. Optional KFM fixture guard-hit gates require matching real Common1 controller/operation order and actor-frame state/physics/body telemetry for `150 -> 151`, `152 -> 153`, and air `154 -> 155 -> 52 -> 20` including landing `VelSet`/`PosSet` operations and bounded velocity/landing frames when the private fixture is present.

Required synthetic auto guard gates also now require controller-event plus actor-frame order: `synthetic-imported-auto-guard-start.json` checksum `0c734290` requires P2 state `120` `ChangeState` `Guard Start Done` plus `120` before `130`, and `synthetic-imported-auto-guard-end.json` checksum `d1dc0aa3` requires P2 `120` `Guard Start Done` before state `130` `ChangeState` `Stop Guarding`, `120 -> 130 -> 140`, plus final idle/control evidence. This is a minimum controller/state-order contract for the bounded route; exact full Common1 controller-loop timing, proximity guard boxes, guard-end timing, and full guard VM parity remain blocked.
...
- actor-frame state/physics/body/velocity telemetry where supported
