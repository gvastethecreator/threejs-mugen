...
- Helper visual motion now has a bounded `velset` cut: `synthetic-imported-helper-velocity.json` proves a typed/raw Helper can spawn with `velset = 3,-1` and expose moving actor-frame position/velocity evidence, checksum `34a1a791`. Helper VM, redirects, helper combat, helper-owned projectiles/HitDefs, exact helper physics, and full parity remain blocked.
- Helper visual scale now has a bounded cut: `synthetic-imported-helper-scale.json` proves a typed/raw Helper can spawn with `scale = 2,0.5`, expose helper effect payload scale, and expose actor-frame render-scale evidence, checksum `c749d4ba`. Helper VM, collision-scale parity, ownpal/palette ownership, redirects, helper combat, and full parity remain blocked.
- Helper visual pause budgets now have a bounded `supermovetime` cut: `synthetic-imported-helper-supermovetime.json` proves a typed/raw Helper can spawn with `supermovetime = 4` and advance during `SuperPause` after source movetime expires, checksum `391eb2dc`. Helper VM during pause, exact pause layering/tick order, redirects, helper combat, and full parity remain blocked.
- Helper visual pause-budget coverage now includes the full current bounded trio: `synthetic-imported-helper-pausemovetime.json` proves regular `Pause` budget advance with checksum `59cd59ad`, and `synthetic-imported-helper-ignorehitpause.json` proves hitpause advance with checksum `5dd3072e`. Helper VM during pause, exact hitpause/pause layering, redirects, helper combat, and full parity remain blocked.
...
2. **Move one side-effect family behind a runtime system**
   ...
   - Current cut: `ContactMemorySystem` now wraps bounded direct/projectile contact-memory mutation and reads for `MoveContact`/`MoveHit`/`MoveGuarded`, `HitCount`/`UniqHitCount`, `HitAdd`, `MoveReversed`, `ReceivedDamage`/`ReceivedHits`, and projectile contact/time markers. `PlayableMatchRuntime` remains the actor/state-number integration point, so this is ownership cleanup with behavior-stable trace evidence, not exact contact/combo lifetime parity.
   - Current cut: `RuntimeResourceSystem` now wraps bounded resource and variable writes for `CtrlSet`, `LifeAdd`, `LifeSet`, `PowerAdd`, `PowerSet`, `VarSet`, `VarAdd`, `VarRangeSet`, and sysvar assignments. `StateControllerExecutor` remains the parameter/expression fallback layer, so this is ownership cleanup for the current controller subset, not exact CNS VM or variable-scope parity.
   - Current cut: `RuntimeStunSystem` now wraps bounded hitstun/guardstun input-lock, timer decay, guarding flag maintenance, and existing horizontal friction. `PlayableMatchRuntime` still owns presentation action changes and imported-state preservation, so this is ownership cleanup, not exact guard recovery or hitpause tick-order parity.
