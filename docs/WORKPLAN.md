...
2. **Move one side-effect family behind a runtime system**
   ...
   - Current cut: `RuntimeTargetWorld` now wraps target-memory mutation/read paths behind a world-style contract for remember, advance, snapshot, count, find, and simplified Target* controller application. `MatchWorld` creates and owns that world, injects it into `PlayableMatchRuntime`, and keeps existing target-link trace evidence behavior-stable while exact target semantics remain future work.
