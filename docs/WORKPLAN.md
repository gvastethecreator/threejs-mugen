...
2. **Move one side-effect family behind a runtime system**
   ...
   - Current cut: `synthetic-imported-targetbind-pause.json` now proves a bounded TargetBind + SuperPause movetime route: a real `HitDef` creates target memory, typed `TargetBind` records a binding, typed `SuperPause` creates a match-pause snapshot, a source `PosAdd` executes during movetime, and `MatchWorld.targetLinks` keeps the bound target visible with binding metadata.
...
3. **Typed ControllerOp expansion**
   ...
   - Current cut: `Pause`/`SuperPause` now compile into typed `pause:*` operations and the required synthetic `SuperPause` trace gate also requires `matchPause` snapshot evidence; `Projectile`, `Helper`, `Explod`, `HitFall*`, `FallEnvShake`, static numeric movement/position controllers, static numeric resource/control/variable controllers, typed `VarSet`/`VarAdd sysvar(n)` assignments, static hit-eligibility setup controllers, static `ReversalDef`, and static `AttackMulSet`/`DefenceMulSet` now compile into typed operation evidence, with required synthetic trace coverage now including `kinematic:posadd`, `resource:ctrlset`, `eligibility:nothitby`, `reversaldef`, `damage-scale:attackmulset`, and `damage-scale:defencemulset`. Static `HitDef` operation data now also preserves bounded `kill`, `guard.kill`, and `fall.kill` params for nonlethal direct/chip/fall-damage clamps; static `LifeAdd kill` lowers into resource ops for bounded negative-life clamps.
...
5. **Guard-rule improvement**
   ...
   - Done: trace proves guard event, guard damage/stun/push, state/anim route where supported, simple `guard.slidetime` / `guard.ctrltime` evidence when present, attacker-side `Unguardable` hit-through-guard evidence, bounded air guard-hit route `154 -> 155` with KFM `VarSet`/`sysvar(0)` landing/control evidence, and explicit unsupported gaps.
