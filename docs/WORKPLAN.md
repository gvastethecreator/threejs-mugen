...
- Previous-state metadata now includes bounded `PrevMoveType` and `PrevStateType`: imported routes can record parsed current statedef move/state type during `ChangeState` and branch from the next state through `PrevMoveType = A` or `PrevStateType = A`. `synthetic-imported-prevmovetype.json` gates `200(A) -> 269(I) -> 270`, and `synthetic-imported-prevstatetype.json` gates airborne state type metadata through `200(A) -> 271(S) -> 272`. Exact tick-order parity, redirects/custom-state ownership, helpers/teams, and full previous-state parity remain blocked.
