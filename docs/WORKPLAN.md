...
- Previous-state metadata now includes bounded `PrevMoveType`: imported routes can record the parsed current statedef movetype during `ChangeState` and branch from the next state through `PrevMoveType = A`. `synthetic-imported-prevmovetype.json` gates `200(A) -> 269(I) -> 270`. Exact tick-order parity, redirects/custom-state ownership, helpers/teams, and full previous-state parity remain blocked.
