...
| ControllerOp / IR | Active, partial | `HitDef`, `Target*`, `Pause`/`SuperPause`, `Projectile`, `Helper`, `Explod`, `HitFall*`, `FallEnvShake`, static numeric `VelSet`/`VelAdd`/`VelMul`/`HitVelSet`/`PosSet`/`PosAdd`/bounded `Gravity`, static `PosFreeze`/`ScreenBound`, static numeric `CtrlSet`/`LifeAdd`/`LifeSet`/`PowerAdd`/`PowerSet`/`VarSet`/`VarAdd`/`VarRangeSet`, typed `VarSet`/`VarAdd sysvar(n)` assignments, static `HitBy`/`NotHitBy`/`HitOverride` setup, static `ReversalDef` setup, and static `AttackMulSet`/`DefenceMulSet` damage-scale setup have typed operation evidence; dynamic movement/bounds/resource/variable/eligibility/reversal/damage-scale params still fall back to runtime expression evaluation, and many controllers still execute from raw params. | Next controller family or system boundary must shrink raw controller paths without overstating parity. |
