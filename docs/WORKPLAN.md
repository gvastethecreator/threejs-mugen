...
2. **Move one side-effect family behind a runtime system**
   ...
   - Current cut: `BindToTarget` now compiles into typed `bindtotarget` operation data and can keep the controller owner bound to a recent target with static `pos = x,y,Foot|Mid|Head`; `Mid`/`Head` use parsed target `[Size]` anchors when available, active owner-to-target binds are exposed in runtime snapshots and `MatchWorld.targetLinks`, and `synthetic-imported-bindtotarget-head.json` is now required trace evidence for the Head path.
