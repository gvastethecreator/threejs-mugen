...
- Scanner/runtime refresh: 2026-07-02 adds character `fightfx.prefix` recognition, source-mapped to the local Ikemen-GO `char.go` parser path, plus bounded runtime metadata handoff from imported DEF raw sections into F-prefixed FightFX hit-spark trace events.
- Runtime refresh: 2026-07-02 adds bounded character FightFX package selection inspired by Ikemen-GO `char.go` / `fightscreen.go`: character `[Files] fx = ...` entries can load FightFX DEF `[Info] prefix`, AIR, and SFF, and matching `fightfx.prefix` selects that package for runtime `F` spark frames. This still does not implement full `sys.ffx` cache/refcount lifetime, SND fallback, exact scale/palette/timing/layering, or screenpack/motif ownership.
