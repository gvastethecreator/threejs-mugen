...
- Scanner/runtime refresh: 2026-07-02 adds character `fightfx.prefix` recognition, source-mapped to the local Ikemen-GO `char.go` parser path, plus bounded runtime metadata handoff from imported DEF raw sections into F-prefixed FightFX hit-spark and hit-sound trace events.
- Runtime refresh: 2026-07-02 adds bounded character FightFX package selection inspired by Ikemen-GO `char.go` / `fightscreen.go`: character `[Files] fx = ...` entries can load FightFX DEF `[Info] prefix`, AIR, SFF, and SND, and matching `fightfx.prefix` selects that package for runtime `F` spark frames plus prefixed sound lookup. This still does not implement full `sys.ffx` cache/refcount lifetime, exact channel fallback, exact scale/palette/timing/layering/mixing, or screenpack/motif ownership.
...
- Compatibility reports now include a report-only `ikemen-go-scan` profile. `IkemenFeatureScanner` recognizes ZSS files/references, ZSS `[Statedef ...]` / `[State ...]` code blocks, and ZSS controller syntax, Lua files/hooks including the `hook.*` API, IKEMEN config JSON, `IkemenVersion`, selected IKEMEN-only controllers and `AssertSpecial` flags, source-mapped extended trigger identifiers that are not in the bounded supported subset, select/system screenpack signals such as `unlock` and `commandlist`, character `fightfx.prefix` metadata, model-stage assets, and named 3D/Z stage params such as `topz`, `botz`, `ztopscale`, `depthtoscreen`, `zoffsetlink`, `startz`, `model`, and `fov`; the scanner also recognizes IKEMEN-GO data ZSS presentation/system controllers such as `LifeBarAction`, `GameMakeAnim`, `Text`, `ModifyText`, `RemoveText`, and `RedLifeSet`, plus the text-count trigger `NumText`, as report-only findings. `fightfx.prefix` additionally has bounded runtime metadata handoff into F-prefixed FightFX hit-spark and hit-sound trace events, and character `[Files] fx` packages can be loaded/selected by matching FightFX prefix for runtime spark frames plus prefixed sound lookup. Full `sys.ffx` lifetime/refcount/cache semantics, exact channel fallback, exact visual/audio parity, and broader IKEMEN execution remain unsupported. Scanner findings are exported as `Recognized + Unsupported` unless a separate runtime gate proves a bounded subset. `PrevMoveType`, `PrevAnim`, and `PrevStateType` are currently excluded from unsupported scanner findings because the runtime now has bounded evidence for them.
...
4. Expand `HitDef` from partial damage/guard/pause/stun/push/default stand get-hit progression/guard-hit routing/fall ground-impact/bounce/lie-down entry plus bounded synthetic recovery/recovery-input/threshold/actor-frame-order/air-velocity/ground-selection evidence and optional official air/ground recovery-input evidence into attr/spark/sound/priority-aware runtime data, exact threshold/velocity math, broader recovery parity, and exact guard states.
