...
| --- | --- | --- |
| Overall closeout | `RuntimeSnapshotWorld` player actor projection. | `docs/BUILD_EXECUTION_BACKLOG.md` |
| Runtime/port | `RuntimeSnapshotWorld` player actor projection. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
| Studio/UI | Studio CSS module ownership cleanup: foundation/primitives load before command-center/workbench/assets, command-center owns shell/stage/HUD/toolbar/compact-tab/mission-strip layout, and workbench owns ledger panels. | `docs/ENGINE_STUDIO_ROADMAP.md`, `docs/INTERFACE_SYSTEM.md` |
...
Latest implementation checkpoint: `RuntimeSnapshotWorld` owns player actor snapshot projection for current match-loop routes: actor identity/source, sprite-owner metadata, cloned runtime state, target refs/bindings, active move or AIR-frame collision boxes, fallback hurtbox, and cloned sound/hit-effect/env-shake event histories. This is ownership cleanup and does not prove new target semantics, effect snapshot ownership, compatibility session ownership, renderer parity, motif/screenpack snapshot rules, broad imported runtime parity, or move the score band.
