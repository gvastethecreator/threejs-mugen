...
Latest implementation checkpoint: `RuntimeStunWorld` owns bounded hitstun/guardstun timer advance, hitstun presentation-action requests, imported hit-state moveType preservation, current-move guardrails, and non-imported idle moveType restoration for current match-loop routes. This is ownership cleanup and does not prove exact MUGEN/IKEMEN hitstun, guard recovery thresholds, Common1 tick-order parity, helper/custom-state stun ownership, full guard/hitstun semantics, or move the score band.
