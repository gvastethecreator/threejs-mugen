...
| Runtime/port | `RuntimeHitPauseWorld` ownership extraction. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
...
Latest implementation checkpoint: `RuntimeHitPauseWorld` owns bounded global hitpause ordering for current match-loop routes: hitpause command buffering, `ignorehitpause` controller dispatch, paused presentation, and actor hitpause countdown. This is ownership cleanup and does not prove persistent-controller semantics, helper-owned hitpause, broad side-effect ordering, exact tick parity, full MUGEN/IKEMEN hitpause parity, or move the score band.
