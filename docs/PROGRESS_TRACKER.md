...
Latest implementation checkpoint before this setup refresh: `RuntimeContactMemoryWorld` extracted bounded contact-memory creation, reset, mutation, and readback into an R2 runtime-world boundary consumed by the main match loop plus direct-combat and reversal systems. It preserved `pnpm qa:trace` at 156/156 artifacts, 138 required and 18 optional. This improved ownership and testability, not the score band.
...
| --- | --- | --- |
| Runtime | `pnpm qa:trace` required artifacts, native/generated roster, imported KFM optional fixtures. Latest aggregate: 156/156 artifacts passed, 138 required and 18 optional. Latest ownership proof: `RuntimeContactMemoryWorld` owns bounded contact-memory mutation/readback for direct/projectile contact triggers and is injected into direct-combat/reversal paths without trace drift. Recent required runtime oracles include `synthetic-imported-noop.json` checksum `72d54f24`, `synthetic-imported-target-owned-custom-state.json` checksum `410fb8c0`, and guard-hit actor-frame telemetry checksums `016938a1`, `6c4321af`, `1dd33fb5`, `ce9cc9ba`. | Exact controller/VM tick order inside frames, helpers as real VM actors, persistent controllers, exact hit-eligibility attr grammar/slot priority, broad hitpause side-effect ordering, custom states/throws, teams, full guard/fall/recovery semantics and exact FightFX/sound timing. |
