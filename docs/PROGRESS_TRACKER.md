...
| --- | --- | --- |
| Runtime | `pnpm qa:trace` required artifacts, native/generated roster, imported KFM optional fixtures. Latest runtime aggregate: 153/153 artifacts passed, 135 required and 18 optional. Latest new required oracle: `synthetic-imported-hitby-allow.json` checksum `c75d5c7d`, proving bounded static `HitBy value = S,NA` allow-list telemetry through typed `eligibility:hitby` operation evidence plus accepted direct-hit contact. | Exact controller/VM tick order inside frames, helpers as real VM actors, persistent controllers, exact hit-eligibility attr grammar/slot priority, broad hitpause side-effect ordering, custom states/throws, teams, full guard/fall/recovery semantics and exact FightFX/sound timing. |
