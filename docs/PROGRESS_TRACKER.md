...
| Runtime/port | `RuntimePausedMatchWorld` ownership extraction. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
...
Latest implementation checkpoint: `RuntimePausedMatchWorld` owns bounded regular `Pause` / `SuperPause` paused-match ordering for current match-loop routes: paused command buffering, source movetime advancement, active/presentation effect advancement, target bindings, stage clamp, frozen actor presentation, pause replacement interruption, and pause countdown ticking. This is ownership cleanup and does not prove exact pause layering, helper VM pause behavior, rollback timing, full MUGEN/IKEMEN pause parity, or move the score band.
