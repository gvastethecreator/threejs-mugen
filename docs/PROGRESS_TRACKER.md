...
| Runtime/port | `RuntimeAssertSpecialWorld` ownership extraction. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
...
Latest implementation checkpoint: `RuntimeAssertSpecialWorld` owns bounded imported pre-facing `AssertSpecial` ordering for current match-loop routes: owner-backed active-state lookup, `AssertSpecial` filtering, trigger gating, and runtime flag application before automatic facing. This is ownership cleanup and does not prove exact persistence layering, global/team/helper ownership, pause interaction, full MUGEN/IKEMEN `AssertSpecial` parity, or move the score band.
