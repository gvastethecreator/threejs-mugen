...
contact retains finite siblings in existing recovery metadata and `GetHitVar`
aliases while preserving `owner/root = p1` and `parent = p1-helper-0`.
Required evidence passes at `d1594d58` / `a3042cd6`; full tests `328/4100`,
typecheck, build, and trace QA `886/886` (`852` required) pass.
Nested/`ownProjectile`, ModifyProjectile, ModifyHitDef, fall flags, exact
recovery timing, teams, rollback, and full parity remain outside.

## Historical implementation checkpoint — T806 root Projectile dynamic fall recovery (2026-08-15)

Issue 381 closes finite caller-context
`fall.recover`/`fall.recovertime`/`down.recover`/`down.recovertime` for one
fresh root Projectile. One accepted falling contact transfers finite siblings
to existing recovery metadata and `GetHitVar` aliases. Required evidence
passes at `35607957` / `8e0e1f63`; full tests `328/4098`, typecheck, build, and
trace QA `885/885` (`851` required) pass. Helper/nesting/`ownProjectile`,
ModifyProjectile, ModifyHitDef, fall flags, exact recovery timing, teams,
rollback, and full parity remain outside.

## Historical implementation checkpoint — T805 Helper Projectile dynamic fall impact (2026-08-15)

Issue 380 closes finite caller-context
`fall.damage`/`fall.xvelocity`/`fall.yvelocity`/`fall.zvelocity` for a fresh
Projectile created by a first-generation Helper. An accepted falling contact
retains finite siblings independently in existing physical velocity and
`GetHitVar(fall.*)` metadata while preserving `owner/root = p1` and
`parent = p1-helper-0`. Required evidence passes at `2a3c5e98` / `0229f235`;
full tests `328/4095`, typecheck, build, and trace QA `884/884` (`850`
required) pass. Nested/`ownProjectile` topology, ModifyProjectile,
recovery/flags, `n`, exact timing, teams, rollback, and full parity remain
outside.

## Historical implementation checkpoint — T804 root Projectile dynamic fall impact (2026-08-15)

## Historical implementation checkpoint — T803 Helper Projectile dynamic `fall.envshake` (2026-08-15)
...
## Historical implementation checkpoint — T793 `EnvColor time = -1` (2026-08-15)
...
trace QA (`872/872`, `838` required) pass. T794 later closes positive
...
matches the pinned Ikemen runner: omission is a no-op; single, pair, and
triple writes replace missing trailing components with zero. Evidence commit
`a5cec411` and required trace `a6bfe6bd` -> `b6d30a1d` pass; typecheck and diff
hygiene pass. The full ProjectileSystem file passes `100/100`. EffectActorSystem
retains one unrelated `guardPoints` expectation failure, while aggregate QA
...
rollback and full parity remain separate. See issue 349.
...
`efa376d6`) and evidence commit `c27b658e` prove Helper caller-context
replacement, root-owned Projectile lifecycle, attacker power `44`, defender
life `5` and `GetHitVar(power)` evidence after the accepted hit. Focused
Helper/trace tests, typecheck and diff hygiene pass. Aggregate QA remains
blocked by the inherited helper-bind target-link case. Givepower, guard
contact, nested/shared-resource topology, exact arithmetic/timing, rollback
and full parity remain outside. See issue 339.

## Historical implementation checkpoint — T764 closed-bounded (2026-08-15)

Issue 338 closes the first-generation Helper-authored `ModifyProjectile`
red-life guard seam. Required trace `4e97fdba` -> `eff1e719` and evidence
commit `90156937` prove the Helper caller-context replacement, root-owned
Projectile lifecycle, authored `GetHitVar(redlife)=40` and final
`life=20/redLife=20` after the creation multiplier. Focused red-life traces,
typecheck and diff hygiene pass. Aggregate QA remains blocked by the inherited
helper-bind target-link case; the pre-existing EffectActor guardpoints
assertion remains separate. Hit contact, nested/shared-resource topology,
exact arithmetic/timing, rollback and full parity remain outside. See issue
338.

## Historical implementation checkpoint — T763 closed-bounded (2026-08-15)
...
`a98fb9c0` prove the captured multiplier survives the live redlife
replacement, authored `GetHitVar(redlife)=40` stays separate, and the
accepted hit ends at `life=5/redLife=20`. Projectile coverage is 111/111;
focused trace, typecheck and diff hygiene pass. Aggregate QA remains blocked
by the inherited helper-bind target-link case. Helper-authored
ModifyProjectile, resource ownership, exact arithmetic/timing, rollback and
full parity remain outside. See issue 336.
...
snapshot. Required trace `9051894a` -> `7434fbdc` and evidence commit
`953788b3` prove the captured multiplier survives the later live update,
authored `GetHitVar(redlife)=20` stays separate, and the accepted guard ends
at `life=20/redLife=20`. Aggregate QA remains blocked by the inherited
helper-bind target-link case. Helper-parented guard breadth, ModifyProjectile,
resource ownership, exact arithmetic/timing, rollback and full parity remain
outside. See issue 333.

## Historical checkpoint — T758 closed-bounded (2026-08-14)
...
`GetHitVar`/velocity metadata. Required artifact checksum is `a2eb52db` /
...
M.U.G.E.N 1.1 only documents ground-guard X; Y/Z, air guard, Projectiles and
exact physics parity remain blocked.

## Historical implementation checkpoint — T736 closed-bounded (2026-08-11)
...
root/RedirectID and Helper callers. Static and dynamic caller-context finite
values preserve the active channel on omission/unresolved input and reach typed
hit-audio channel `7`. Required artifact checksum is `b5f4c11e` /
`421be8fe`; focused compiler/runtime/Helper/audio coverage is `335/335`,
typecheck passes and `pnpm qa:trace` passes `822/822` artifacts (`788`
...
timing, teams, rollback and full Projectile parity are blocked. See [issue
281](../.scratch/roadmap/issues/281-modifyprojectile-terminal-anim-dynamic.md).
