...
| --- | --- | --- |
| Runtime | `pnpm qa:trace` required artifacts, native/generated roster, imported KFM optional fixtures. Latest runtime aggregate: 147/147 artifacts passed, 129 required and 18 optional. | Exact controller/VM tick order inside frames, helpers as real VM actors, custom states/throws, teams, full guard/fall/recovery semantics and exact FightFX/sound timing. |
...
2. **KFM/Common1 precision**
   ...
   - Current guard-effect proof added: required `synthetic-imported-hitdef-guard-sound.json` checksum `fdf1f7f6` proves bounded `HitDef guardsound = S6,0` event telemetry as an attacker-side `PlaySnd` after a guarded imported direct-hit route. This is not exact SND/channel/timing/mixing/FightFX/spark parity.
