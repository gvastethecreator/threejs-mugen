...
| Three.js rendering | `pnpm qa:smoke` screenshots and canvas checks, including active desktop/mobile diagnostics for the bounded 120-frame fallback `HitSparkRenderer` overlay and system-source spark metadata from hit-effect snapshot events. | Pixel-perfect MUGEN render parity, FightFX/common spark assets, palette application, screenpack/lifebar parity. |
