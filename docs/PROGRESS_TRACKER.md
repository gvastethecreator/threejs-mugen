...
| --- | --- | --- |
| Overall closeout | Studio CSS cascade prune plus optional private-fixture KFM `x` HitDef sound/spark gates. | `docs/BUILD_EXECUTION_BACKLOG.md` |
| Runtime/port | Latest optional KFM local presentation gates: `kfm-official-x-hit-sound.json` and `kfm-official-x-hit-spark.json`, both checksum `9668e88a`; latest required gate remains `synthetic-imported-target-noko.json` checksum `28ac8636`. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
| Studio/UI | Studio CSS cascade prune: obsolete Evidence/Release Desk legacy blocks removed; `src/styles/studio-command-palette.css`, `src/styles/studio-stage.css`, and `src/styles/studio-inspector.css` added for surface ownership; fully overridden module-covered rules pruned from `src/style.css`; `pnpm qa:css` now reports 3,256 rules / 266 duplicate selector keys / 0 exact duplicates. | `docs/ENGINE_STUDIO_ROADMAP.md`, `docs/INTERFACE_SYSTEM.md` |
...
Latest implementation checkpoint: optional private-fixture `kfm-official-x-hit-sound.json` and `kfm-official-x-hit-spark.json` now pass with checksum `9668e88a`, proving real KFM `x -> 200` emits bounded `hitsound = S5,0` and `sparkno 0` telemetry after contact when the local fixture exists. The latest required checkpoint remains `synthetic-imported-target-noko.json` checksum `28ac8636`, which gates a bounded imported target-controller no-KO route. This is R1 trace strengthening only; it does not prove SND decode/playback, exact FightFX/common presentation parity, exact NoKO lifetime, helper/root/parent redirects, teams, multi-target behavior, round flow, broad imported runtime parity, or move the score band.
...
| --- | --- | --- |
| Runtime | `pnpm qa:trace` required artifacts, native/generated roster, imported KFM optional fixtures. Latest aggregate: 159/159 artifacts passed, 139 required and 20 optional. Latest optional KFM presentation oracles: `kfm-official-x-hit-sound.json` and `kfm-official-x-hit-spark.json`, both checksum `9668e88a`. Latest required oracle: `synthetic-imported-target-noko.json` checksum `28ac8636` gates bounded `TargetLifeAdd` lethal damage clamped by defender-side `AssertSpecial NoKO`; previous `synthetic-imported-target.json` checksum `f5a16dc9` still gates Target* resource/facing/velocity/bind/drop side effects, and `synthetic-imported-variable.json` checksum `3b33f7a8` still accepts typed `variable:varrandom`. Latest ownership proof: `RuntimeSnapshotWorld` owns bounded player actor snapshot projection plus stage/camera snapshot projection; `RuntimeCompatibilityTelemetryWorld` owns imported compatibility telemetry/session projection; `RuntimeAssertSpecialWorld` owns bounded imported pre-facing `AssertSpecial` lookup/filter/trigger/application before auto-facing; `RuntimeHitPauseWorld` owns bounded global hitpause mini-loop ordering; `RuntimePausedMatchWorld` owns bounded regular pause mini-loop ordering; `RuntimeStunWorld` owns bounded hitstun/guardstun timer advance, hitstun presentation-action requests, imported hit-state moveType preservation, current-move guardrails, and non-imported idle moveType restoration; `RuntimeStateAvailabilityWorld` owns bounded state/action availability lookup; `RuntimeHitStateTransitionWorld` owns bounded direct-hit/ReversalDef state transition routing; `RuntimeGetHitStateWorld` owns bounded default get-hit state selection; `RuntimeGuardWorld` owns bounded guard state selection and auto guard-start gating/mutation; `RuntimeOrientationWorld` owns bounded auto-facing and `Turn`; `RuntimeHitEligibilityWorld` owns bounded hit-eligibility lifetime/reset maintenance; `RuntimeTargetWorld.applyTargetBindings` / `applyBindToTarget` own active target binding position application; `RuntimeRecoverySystem` owns bounded recovery timer/default/landing hooks; `HitSparkAssetSystem` owns player/common/FightFX spark asset-frame lookup; `RuntimeRandomSystem` owns deterministic random seed/advance/clamp/fallback behavior; and `RuntimeContactMemoryWorld` owns bounded contact-memory mutation/readback for direct/projectile contact triggers. | Exact controller/VM tick order inside frames, helpers as real VM actors, persistent controllers, exact StateDef lookup/entry parity, exact stage/motif camera logic, effect snapshot ownership, compatibility session ownership, target semantics beyond existing target refs/bindings projection, renderer parity, exact AssertSpecial persistence/layering/global ownership, exact NoKO lifetime/round flow, exact pause layering/helper VM pause behavior, exact auto-facing/Turn/target-facing parity, exact guard proximity/end/effects/landing parity, exact default get-hit animation/route parity, exact hit-state ownership/custom-state VM semantics, exact hit-eligibility attr grammar/slot priority, broad hitpause side-effect ordering, custom states/throws, teams, exact random stream parity, full target/bind semantics, full guard/fall/recovery semantics, exact hitstun/guard recovery/Common1 tick order, and exact FightFX/sound timing/layering/scale/palette. |
...
| Studio | Workbench, Assets, Evidence, Debug, Modules, Build surfaces. Latest visual proof: `pnpm qa:smoke` passes with compact Studio navigator/mission/control rows, denser Build/Evidence route copy, tightened stage toolbar placement, wrapped replacement rows, dense Studio Assets ledger, Studio Build / Modules release-desk screenshots, dense Studio Evidence audit screenshot, a shared command-desk viewport/status strip with icon-backed real metrics plus live Pause/HitPause state, and a CSS ownership/cascade pass that loads shared foundation/primitives first, keeps shell/stage/HUD/toolbar/compact-tab/mission-strip authority in `src/styles/studio-command-center.css`, keeps workbench/assets modules surface-scoped, moves Build/Evidence into `src/styles/studio-trust-ledgers.css`, moves Modules/Debug into `src/styles/studio-system-ledgers.css`, adds Command Palette/Stage/Inspector ownership in `src/styles/studio-command-palette.css`, `src/styles/studio-stage.css`, and `src/styles/studio-inspector.css`, prunes obsolete legacy blocks/fully overridden module-covered rules from `src/style.css`, and reports 3,256 rules / 266 duplicate selector keys / zero exact duplicate CSS rules through `pnpm qa:css`. These remain bound to QA bridge/runtime data for project asset records, replacement flow, source/runtime maps, compiled project, trace artifact, persisted trace history, shared/module contracts, source packages, architecture evidence, Stage imported layer reports, and Debug CNS/command evidence. | True editing workflows, persistent source handles, regenerate/relink automation, multi-artifact trace diff depth, deeper shared CSS primitives for chrome/ledger/status rows, token/typography cleanup, and remaining legacy `src/style.css` selector-cascade reduction. |
...
0. **Common1/FightFX precision**
   ...
   - This should not reselect `VarRandom`, `RuntimeRandomSystem`, `RuntimeSnapshotWorld` stage/camera ownership, `RuntimeSnapshotWorld` player actor projection, `RuntimeCompatibilityTelemetryWorld`, `HitSparkAssetSystem`, `RuntimeRecoverySystem`, `BindToTarget` target-system ownership, active target-binding position ownership, `RuntimeHitEligibilityWorld`, `RuntimeAssertSpecialWorld`, `RuntimeOrientationWorld`, `RuntimeGuardWorld`, `RuntimeGetHitStateWorld`, `RuntimeHitStateTransitionWorld`, `RuntimeStateAvailabilityWorld`, `MakeDust`, target-owned custom-state, or already-closed guard/auto-guard evidence.
