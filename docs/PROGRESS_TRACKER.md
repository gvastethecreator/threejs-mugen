...
Latest implementation checkpoint: `RuntimeStateAvailabilityWorld` owns bounded state/action availability lookup for current state-entry validation routes. This is ownership cleanup and does not prove exact StateDef lookup edge cases, helper/root/parent/team lookup, state-entry mutation, full CNS VM state ownership, or move the score band.
