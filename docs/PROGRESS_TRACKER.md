...
Latest implementation checkpoint: `RuntimeTargetWorld.applyTargetBindings` / `applyBindToTarget` own bounded active `TargetBind` target-position application and active `BindToTarget` owner-position application before `PlayableMatchRuntime` continues interaction ordering. This is ownership cleanup and does not prove exact bind tick order, multi-target/team ownership, helper/parent/root redirects, full target parity, or move the score band.
...
0. **Common1/FightFX precision**
   ...
   - This should not reselect `VarRandom`, `RuntimeRandomSystem`, `HitSparkAssetSystem`, `RuntimeRecoverySystem`, `BindToTarget` target-system ownership, active target-binding position ownership, `MakeDust`, target-owned custom-state, or already-closed guard/auto-guard evidence.
