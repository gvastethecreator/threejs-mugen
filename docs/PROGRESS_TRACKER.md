...
2. **KFM/Common1 precision**
   ...
   - Current guard proof added: required `synthetic-imported-default-guard-state.json` checksum `016938a1` now gates ordered named controller/typed-operation evidence for the bounded stand guard-hit route `150 -> 151`, including `kinematic:hitvelset` and `resource:ctrlset`.
