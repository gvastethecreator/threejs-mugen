...
3. **Compatibility trace coverage**
   ...
   - Current proof added: dedicated `synthetic-imported-kinematic.json` trace artifact for typed `VelSet` / `VelAdd` / `VelMul` / `PosSet` / `PosAdd` position/velocity evidence, `synthetic-imported-gravity.json` for typed `Gravity` / `kinematic:gravity` vertical velocity evidence, `synthetic-imported-envshake.json` for partial `EnvShake` runtime event evidence, `synthetic-imported-sound.json` for partial `PlaySnd` / `StopSnd`, plus `synthetic-imported-resource.json` for typed `LifeAdd` / `LifeSet` / `PowerAdd` / `PowerSet` and `synthetic-imported-variable.json` for `VarSet` / `VarAdd` / `VarRangeSet`.
