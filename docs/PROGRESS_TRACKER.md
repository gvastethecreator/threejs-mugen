...
| --- | --- | --- |
| Overall closeout | Studio CSS hygiene pass and duplication audit guard. | `docs/BUILD_EXECUTION_BACKLOG.md` |
| Runtime/port | `RuntimeCompatibilityTelemetryWorld` ownership extraction. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
| Studio/UI | Command-center CSS module plus exact duplicate rule cleanup. | `docs/ENGINE_STUDIO_ROADMAP.md`, `docs/INTERFACE_SYSTEM.md` |
...
Latest implementation checkpoint: `RuntimeCompatibilityTelemetryWorld` owns imported compatibility telemetry/session projection for current match-loop routes: imported and owner-backed actor detection, executed-state bookkeeping, routed State -1 entries, controller/operation counts, bounded controller-event history, active command projection, command history, and `compatibilitySession` snapshot construction. This is ownership cleanup and does not prove new controller semantics, exact CNS VM timing, actor snapshot ownership, broad imported runtime parity, or move the score band.
...
| Studio | Workbench, Assets, Evidence, Debug, Modules, Build surfaces. Latest visual proof: `pnpm qa:smoke` passes with compact Studio navigator/mission/control rows, denser Build/Evidence route copy, tightened stage toolbar placement, wrapped replacement rows, dense Studio Assets ledger, Studio Build / Modules release-desk screenshots, dense Studio Evidence audit screenshot, a shared command-desk viewport/status strip with icon-backed real metrics plus live Pause/HitPause state, and a CSS hygiene pass that moves command-center authority into `src/styles/studio-command-center.css`, keeps Compile/Nav/Inspect/Focus visible, and reports zero exact duplicate CSS rules through `pnpm qa:css`. These remain bound to QA bridge/runtime data for project asset records, replacement flow, source/runtime maps, compiled project, trace artifact, persisted trace history, shared/module contracts, source packages, architecture evidence, Stage imported layer reports, and Debug CNS/command evidence. | True editing workflows, persistent source handles, regenerate/relink automation, multi-artifact trace diff depth, deeper shared CSS primitives for chrome/ledger/status rows, and remaining duplicated Studio selector cascade. |
...
0. **Common1/FightFX precision**
   ...
   - This should not reselect `VarRandom`, `RuntimeRandomSystem`, `RuntimeSnapshotWorld`, `RuntimeCompatibilityTelemetryWorld`, `HitSparkAssetSystem`, `RuntimeRecoverySystem`, `BindToTarget` target-system ownership, active target-binding position ownership, `RuntimeHitEligibilityWorld`, `RuntimeAssertSpecialWorld`, `RuntimeOrientationWorld`, `RuntimeGuardWorld`, `RuntimeGetHitStateWorld`, `RuntimeHitStateTransitionWorld`, `RuntimeStateAvailabilityWorld`, `MakeDust`, target-owned custom-state, or already-closed guard/auto-guard evidence.
...
1. **MatchWorld ownership**
   ...
   - Current proof added: `RuntimeSnapshotWorld` now owns bounded stage/camera snapshot projection for `ScreenBound` camera-follow exclusion, all-disabled fallback, stage camera offsets, EnvShake camera shake, and EnvColor stage flash. Focused tests prove the boundary and `pnpm qa:trace` stays stable at 156/156 artifacts. This is ownership cleanup, not exact stage/motif camera logic, actor snapshot projection, compatibility session projection, or renderer parity.
   - Current proof added: `RuntimeCompatibilityTelemetryWorld` now owns imported compatibility telemetry/session projection for executed states, routed entries, controller/operation counts, bounded controller events, active commands, and command history. Focused tests prove the boundary and `pnpm qa:trace` stays stable at 156/156 artifacts. This is ownership cleanup, not new controller semantics, exact CNS VM timing, actor snapshot ownership, or imported-runtime parity.
