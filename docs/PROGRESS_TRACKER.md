...
Latest implementation checkpoint: `HitSparkAssetSystem` owns bounded player/common/FightFX HitDef spark asset-frame lookup before hit-effect events enter the match snapshot/render path. `RuntimeRandomSystem` remains the deterministic random ownership checkpoint; required `synthetic-imported-variable.json` remains checksum `3b33f7a8`, with `pnpm qa:trace` passing 156/156 artifacts, 138 required and 18 optional. This is ownership cleanup and does not prove exact FightFX/common presentation parity, exact MUGEN random stream, helper/parent/root variable scope, or move the score band.
...
0. **Common1/FightFX precision**
   ...
   - This should not reselect `VarRandom`, `RuntimeRandomSystem`, `HitSparkAssetSystem`, `MakeDust`, target-owned custom-state, or already-closed guard/auto-guard evidence.
