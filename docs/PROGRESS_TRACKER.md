...
| Studio | Workbench, Assets, Evidence, Debug, Modules, Build surfaces. Latest visual proof: `pnpm qa:smoke` passes with dense Studio Assets ledger, Studio Build / Modules release-desk screenshots, and dense Studio Evidence audit screenshot, all bound to real QA bridge data for project asset records, replacement flow, source/runtime maps, compiled project, trace artifact, persisted trace history, shared/module contracts, source packages, and architecture evidence. | True editing workflows, persistent source handles, regenerate/relink automation, multi-artifact trace diff depth. |
