...
| --- | --- | --- |
| Runtime | `pnpm qa:trace` required artifacts, native/generated roster, imported KFM optional fixtures. Latest runtime aggregate: 152/152 artifacts passed, 134 required and 18 optional. Latest new required oracle: `synthetic-imported-noop.json` checksum `57c74c93`, proving bounded imported `Null` and browser no-op `ForceFeedback` execution visibility before a simple `HitDef` route. | Exact controller/VM tick order inside frames, helpers as real VM actors, persistent controllers, no-op timing/side-effect edge cases, device force feedback, broad hitpause side-effect ordering, custom states/throws, teams, full guard/fall/recovery semantics and exact FightFX/sound timing. |
