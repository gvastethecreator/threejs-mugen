...
| Runtime/port | `RuntimeSnapshotWorld` ownership extraction. | `docs/ROADMAP_PACKAGE_MILESTONES.md` |
...
Latest implementation checkpoint: `RuntimeSnapshotWorld` owns bounded stage/camera snapshot projection for current match-loop routes: camera center respects `ScreenBound moveCameraX = 0`, falls back when every actor disables camera following, applies stage camera offsets, and carries EnvShake/EnvColor presentation data into `StageSnapshot`. This is ownership cleanup and does not prove exact stage/motif camera logic, actor snapshot projection, compatibility session ownership, renderer parity, screenpack ownership, or move the score band.
