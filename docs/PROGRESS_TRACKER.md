...
Latest implementation checkpoint: required `synthetic-imported-noop.json` checksum `a5fe169e` accepts deprecated `MakeDust` as a telemetry-visible presentation no-op next to existing no-op/debug controllers. `pnpm qa:trace` remains 156/156 artifacts, 138 required and 18 optional. This reduces no-crash controller gaps for Common1-style imports; it does not add dust rendering or move the score band.
