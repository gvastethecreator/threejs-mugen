...
- Studio Workbench starts with a command center, not a passive summary. It must show project readiness lanes, the current operator priority, direct jumps to Assets/Evidence/Build/Debug, and primary actions for Playtest, MUGEN intake, trace export, and runtime compile. It now also hosts immediate matchup controls (P1 / CPU / Stage selectors) so operators can reconfigure actors and stage in-context and keep the runtime snapshot consistent.
...
- Studio Build and Evidence share the same Trust Chain contract rows for runtime manifest, QA trace evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries. Those rows must derive from Build Readiness data and expose status, evidence, freshness/delta, impact, blockers, target kind/id, optional target package/path metadata, and one primary next action instead of independent decorative summaries. Project package rows can drill into concrete `package-file` entries from the export manifest, and source package rows can drill into concrete `source-file` required paths from the project manifest. Trust Chain jumps leave a visible focused row so users can see the exact record they landed through; `pnpm qa:smoke` checks shared row ids, target metadata, deltas, next-action binding, package/source file focus, and focused-row state through the Studio QA bridge.
...
- Studio desktop CSS remains split into focused ownership modules after the entrypoint: `src/styles/desktop/studio-desktop-foundation.css` owns neutral command-desk tokens and shared atoms; `src/styles/surfaces/studio-primitives.css` owns reusable ledger panel shells, shared ledge/header chrome, and the shared ledger key-value/badge primitive now used by Build/Evidence plus Modules/Debug; `src/styles/surfaces/studio-ledger-drawers.css` owns the `studio-ledger-drawer` collapsible drawer primitive; `src/styles/surfaces/studio-status-rails.css` owns shared ok/warn/error side-rail state across Studio command, lane, trust, asset, and stage rows; `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css` own the active command shell, mission/action rail, viewport/HUD/tooling frame, and bottom console; `src/styles/workflows/studio-workbench.css` owns the Workbench surface; `src/styles/workflows/studio-assets.css` owns Assets command/triage tokens while reusing shared Studio action primitives, `src/styles/workflows/studio-assets-ledger.css` owns reusable Assets ledger rows, and `src/styles/workflows/studio-assets-graph.css` owns the Assets dependency graph; `src/styles/workflows/studio-trust-ledgers.css` owns Build/Evidence trust-ledger row layout, Trust Chain focus state, trace strips, and Impact/Next copy; `src/styles/workflows/studio-system-ledgers.css` owns Modules/Debug system-ledger rows, contracts, runtime focus, and execution evidence.
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap by walking the `src/main.ts` CSS imports and nested local CSS `@import` graph. Current hygiene target: zero exact duplicate rules, zero same-file duplicate selectors, zero cross-file brace leakage, and zero fully shadowed cross-file rules across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit on 2026-07-09 reports 581,084 active CSS bytes, 2,576 scanned rules, 1 duplicate selector key / 2 instances, 0 exact duplicate rules, 126 repeated declaration groups, 155 cross-file overlaps, 0 selectors shared with `src/style.css`, 0 legacy `style.css` rules fully shadowed by later imports, and 17 cross-file rules fully shadowed by later imports. `pnpm qa:css:budget` currently fails against the older 536,051-byte / 2,364-rule / 119-repeated-group / 108-overlap ceilings, so CSS budget recovery is an explicit follow-up cleanup task rather than a green gate for this Studio workflow slice. The latest Trust Chain work keeps new CSS scoped to `src/styles/workflows/studio-trust-ledgers.css`; future cleanup should collapse shared chrome, ledger-row, status-cell, and command-action primitives instead of adding final override blocks.
