...
- Studio desktop chrome CSS must stay contained in one responsive command-desk block. Avoid tail-end correction blocks that restate the same chrome/readout/action selectors, because those overrides already hid the Build/Compile action once. Future cleanup should extract shared Studio row/status primitives instead of adding another broad override.
...
- Consolidate duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. The latest containment pass removed one duplicate desktop correction block and restored visible Build/Compile controls, but the static interface detector still reports repeated P2/P3 quality, resilience, and design-system patterns.
