...
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap. Current hygiene target: zero exact duplicate rules across the scanned CSS files, then steady reduction of selectors shared between legacy `src/style.css` and `src/styles/*`. Latest audit reports 2,618 scanned rules, 83 duplicate selector keys / 184 instances, 0 exact duplicate rules, 198 repeated declaration groups, 78 cross-file overlaps, 0 selectors shared with `src/style.css`, and 0 legacy `style.css` rules fully shadowed by later imports. `src/style.css` is now a small base stylesheet; app shell and remaining legacy Studio surfaces live in `src/styles/app-shell.css`, `src/styles/studio-legacy-surfaces.css`, `src/styles/studio-editor-cascade.css`, `src/styles/studio-ui-hardening.css`, and `src/styles/studio-desktop-authority.css`. Remaining repeated Studio selectors are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
