...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. The latest trust-ledger extraction moved Build/Evidence right-rail ownership into `src/styles/studio-trust-ledgers.css`, kept exact duplicate rules at zero, and reduced legacy duplicate selector keys from 409 to 390.
