...
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap by walking the `src/main.ts` CSS imports and nested local CSS `@import` graph. Current hygiene target: zero exact duplicate rules, zero same-file duplicate selectors, zero cross-file brace leakage, and zero fully shadowed cross-file rules across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit reports 2,594 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 159 repeated declaration groups, 123 cross-file overlaps, 0 selectors shared with `src/style.css`, 0 legacy `style.css` rules fully shadowed by later imports, and 0 cross-file rules fully shadowed by later imports. `pnpm qa:css:budget` freezes those current debt ceilings for CSS cleanup/review rounds so new Studio work cannot silently increase rules, repeated groups, or overlaps without an explicit budget decision. The Assets right pane now routes repeated collapsible side sections through one `studio-ledger-drawer` helper/class while preserving legacy class hooks, shared Studio truncation and text-wrap declarations are grouped in `src/styles/legacy/studio-legacy-surfaces.css`, repeated status metric colors and ok/warn summary/health/lane panel tints are grouped in `src/styles/base/app-shell.css`, and redundant legacy responsive/shell overrides have been pruned when later authoritative layers own the same selectors. `pnpm qa:css:detail` prints overlap details plus the most repeated declaration groups; the biggest safe primitive targets are remaining grid resets, border-end resets, shell/chrome ownership, and cross-file selector overlap. Remaining repeated declaration groups and cross-file overlaps are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
