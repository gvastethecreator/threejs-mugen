...
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap. Current hygiene target: zero exact duplicate rules and zero same-file duplicate selectors across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit reports 2,608 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 173 repeated declaration groups, 143 cross-file overlaps, 0 selectors shared with `src/style.css`, and 0 legacy `style.css` rules fully shadowed by later imports. `src/style.css` is now a small base stylesheet. App shell and legacy Studio cascade are split across focused `app-shell.css` and `studio-legacy-*` modules; editor, UI hardening, runtime command-room, and desktop authority passes are split into focused `studio-editor-*`, `studio-*hardening.css`, `runtime-*`, and `studio-desktop-*` modules before the existing command-center, workbench/assets, trust/system ledger, command palette, stage, and inspector modules. Remaining repeated declaration groups and cross-file overlaps are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. Use `pnpm qa:css` before cleanup and inspect cross-file overlap manually when touching import order. The latest current-working-tree audit keeps same-file duplicate selectors, exact duplicate rules, and `src/style.css` overlap at zero, but still reports 143 cross-file selector overlaps across Studio modules; those should be collapsed through primitives, not tail-end overrides.
