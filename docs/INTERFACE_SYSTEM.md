...
- Studio desktop chrome, viewport instruments, toolbar, and status strip use one command-desk register. The Studio playfield toolbar should stay as a compact bottom-right instrument dock, not a full-width strip across the viewport. Icon-backed status metrics are allowed only when they clarify live project/runtime data such as gates, assets, trace count, build readiness, atlas state, collision boxes, or Pause/HitPause state.
- Studio desktop CSS loads through one Studio CSS entrypoint: `src/main.ts` imports `src/style.css` for base/reset and `src/styles/studio.css` for the ordered Studio/app cascade. `src/styles/studio.css` owns category entrypoint order: `base.css`, `legacy.css`, `editor.css`, `runtime.css`, `desktop.css`, `shell.css`, `command.css`, `workflows.css`, then `src/styles/surfaces/studio-status-rails.css` as the late shared rail owner. Keep new Studio desktop work in focused modules under those folders instead of adding more tail-end overrides to `src/style.css` or expanding `src/main.ts` imports.
- Studio desktop CSS remains split into focused ownership modules after the entrypoint: `src/styles/desktop/studio-desktop-foundation.css` owns neutral command-desk tokens and shared atoms; `src/styles/surfaces/studio-primitives.css` owns reusable ledger panel shells plus shared ledge/header chrome now used by Build/Evidence right rails; `src/styles/surfaces/studio-ledger-drawers.css` owns the `studio-ledger-drawer` collapsible drawer primitive; `src/styles/surfaces/studio-status-rails.css` owns shared ok/warn/error side-rail state across Studio command, lane, trust, asset, and stage rows; `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css` own the active command shell, mission/action rail, viewport/HUD/tooling frame, and bottom console; `src/styles/workflows/studio-workbench.css` owns the Workbench surface; `src/styles/workflows/studio-assets.css` owns Assets command/triage tokens while reusing shared Studio action primitives, `src/styles/workflows/studio-assets-ledger.css` owns reusable Assets ledger rows, and `src/styles/workflows/studio-assets-graph.css` owns the Assets dependency graph; `src/styles/workflows/studio-trust-ledgers.css` owns Build/Evidence trust-ledger row layout, status cells, key-value tables, trace strips, and Impact/Next copy; `src/styles/workflows/studio-system-ledgers.css` owns Modules/Debug system-ledger rows, contracts, runtime focus, and execution evidence.
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap by walking the `src/main.ts` CSS imports and nested local CSS `@import` graph. Current hygiene target: zero exact duplicate rules, zero same-file duplicate selectors, zero cross-file brace leakage, and zero fully shadowed cross-file rules across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit reports 537,761 active CSS bytes, 2,375 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 122 repeated declaration groups, 108 cross-file overlaps, 0 selectors shared with `src/style.css`, 0 legacy `style.css` rules fully shadowed by later imports, and 0 cross-file rules fully shadowed by later imports. `pnpm qa:css:budget` freezes those current debt ceilings, including bytes, for CSS cleanup/review rounds so new Studio work cannot silently increase payload, rules, repeated groups, or overlaps without an explicit budget decision. The Assets desktop graph is extracted out of `studio-tab-hardening.css`, Assets desktop row/panel selectors are pruned from older desktop/editor finish modules, Assets ledger/graph selectors are compacted with scoped `:is(...)` groups, Assets action icons/primary states reuse shared Studio action primitives, and the Assets right pane routes repeated collapsible side sections through one `studio-ledger-drawer` helper/class while preserving legacy class hooks. Shared Studio truncation, text-wrap, grid, align, mono metric, and subtle text declarations are grouped in `src/styles/legacy/studio-legacy-surfaces.css`; desktop workbench route, list-meta, lane, engine-contract, and drawer-summary truncation now share a late `src/styles/surfaces/studio-primitives.css` primitive instead of repeated editor/desktop/drawer overrides. Repeated status metric colors and ok/warn summary/health/lane panel tints are grouped in `src/styles/base/app-shell.css`, shared row status rails are extracted into `src/styles/surfaces/studio-status-rails.css`, shared Build/Evidence ledge headers reuse `src/styles/surfaces/studio-primitives.css`, repeated Studio desktop shell truncation shares one workstation selector group, repeated left-pane kicker/eyebrow styling shares one desktop foundation group, retired pseudo indicator rules no longer survive as hidden overrides, grouped life/power meter colors and mission/mode status dots reduce repeated declarations, the neutral legacy pass is removed, global responsive shell rules moved from the responsive legacy pass into `src/styles/base/app-shell.css`, unused legacy command/health/readiness selector hooks have been mechanically removed after source search confirmed they no longer appear outside CSS, and unused structural Build/Evidence list, old asset focus/flow, trace scrubber, stat-card, and build-export-console hooks are pruned. `pnpm qa:css:detail` prints overlap details plus the most repeated declaration groups; the biggest safe primitive targets are remaining border-end resets, shell/chrome ownership, ledger-row/status-cell/action primitives, and cross-file selector overlap. Remaining repeated declaration groups and cross-file overlaps are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
