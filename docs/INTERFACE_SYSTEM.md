...
The current visual direction is **Fight Lab / Frame Ledger**. The playable artifact owns the center; a warm frame line marks current, next, and selected records across Runtime, Inspector, and Studio. The shell uses graphite working surfaces, warm text, amber task emphasis, teal focus, and exclusive semantic colors. It explicitly rejects generic terminal cosplay, glass/neon “premium” panels, oversized radii, and equal-weight card dashboards.
...
- CSS loads through three explicit layers: `src/style.css` owns reset/base behavior, `src/styles/studio.css` imports only the structural and workflow modules still used by the product, and `src/styles/redesign.css` owns the shared Fight Lab tokens, composition, hierarchy, states, and responsive authority. Do not restore the removed `legacy/editor/desktop/premium/polish` umbrella cascades or add another final override file.
- Structural ownership remains focused: command files own palette/pipeline/playfield/console behavior; workflow files own Workbench, Assets, Stage, Inspector, Trust Chain, and system-ledger geometry; surface primitives own reusable ledger shells/drawers. `redesign.css` is the only cross-surface visual authority and must fix a repeated problem at the shared level.
- `pnpm qa:css` and `pnpm qa:css:budget` walk the active import graph and enforce zero exact duplicate rules, zero same-file duplicate selectors, zero cross-file brace leakage, and zero fully shadowed cross-file rules. The 2026-07-10 redesign audit passes at 315,380 active CSS bytes, 1,451 rules, 0 duplicate selector keys, 0 exact duplicate rules, 69 repeated declaration groups, 51 cross-file overlaps, and 0 fully shadowed cross-file rules. This replaces the previous 600 KB-plus cascade and removes 10,092 obsolete CSS lines.
...
- Extend persistent browser source handles/IndexedDB metadata with folder write/recovery and permission-state evidence.
