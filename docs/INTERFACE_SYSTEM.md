...
- Studio desktop CSS is split into focused modules after the legacy base stylesheet: `src/styles/studio-desktop-foundation.css` owns neutral command-desk tokens and shared atoms; `src/styles/studio-primitives.css` owns reusable ledger panel shells; `src/styles/studio-command-center.css` owns chrome, docks, mission strip, viewport framing, HUD, toolbar, and console; `src/styles/studio-workbench.css` and `src/styles/studio-assets.css` own their surface-specific ledger rows. Keep new Studio desktop work in those modules instead of adding more tail-end overrides to `src/style.css`.
...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. The latest CSS extraction removed the old desktop command-desk override block from `src/style.css`, added focused Studio modules, kept exact duplicate rules at zero, and reduced legacy duplicate selector keys from 423 to 409.
