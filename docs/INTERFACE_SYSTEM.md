...
- Studio desktop CSS remains split into focused ownership modules after the entrypoint: `src/styles/desktop/studio-desktop-foundation.css` owns neutral command-desk tokens and shared atoms; `src/styles/surfaces/studio-primitives.css` owns reusable ledger panel shells; `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css` own the active command shell, mission/action rail, viewport/HUD/tooling frame, and bottom console; `src/styles/workflows/studio-workbench.css` owns the Workbench surface; `src/styles/workflows/studio-assets.css` owns the Assets surface; `src/styles/workflows/studio-trust-ledgers.css` owns Build/Evidence trust-ledger rows, status cells, key-value tables, trace strips, and Impact/Next copy; `src/styles/workflows/studio-system-ledgers.css` owns Modules/Debug system-ledger rows, contracts, runtime focus, and execution evidence.
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap by walking the `src/main.ts` CSS imports and nested local CSS `@import` graph. Current hygiene target: zero exact duplicate rules, zero same-file duplicate selectors, zero cross-file brace leakage, and zero fully shadowed cross-file rules across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit after `pnpm fix:css` reports 2,661 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 173 repeated declaration groups, 126 cross-file overlaps, 0 selectors shared with `src/style.css`, 0 legacy `style.css` rules fully shadowed by later imports, and 0 cross-file rules fully shadowed by later imports. `pnpm qa:css:detail` prints overlap details plus the most repeated declaration groups; the biggest safe primitive targets are truncation rows, status colors, grid resets, and border-end resets. Remaining repeated declaration groups and cross-file overlaps are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. Use `pnpm qa:css:detail` before cleanup and inspect cross-file overlap plus repeated declaration groups manually when touching import order. The latest current-working-tree audit keeps same-file duplicate selectors, exact duplicate rules, `src/style.css` overlap, and fully shadowed cross-file rules at zero, but still reports cross-file selector overlaps and repeated declaration groups across Studio modules; those should be collapsed through primitives, not tail-end overrides.
