...
- Studio desktop chrome CSS now starts from `src/styles/studio-command-center.css` after the legacy base stylesheet. Keep new command chrome, docks, mission strip, viewport framing, and console work in that module instead of adding more tail-end overrides to `src/style.css`.
- `pnpm qa:css` reports CSS duplication metrics. Current hygiene target: zero exact duplicate rules across the scanned CSS files. Remaining repeated Studio selectors are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
...
- Consolidate duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. The latest CSS hygiene pass removed exact duplicate rules and added `pnpm qa:css`, but 423 duplicate selector keys remain as cascade debt in the legacy Studio stylesheet.
