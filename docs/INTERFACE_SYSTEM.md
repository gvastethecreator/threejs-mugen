...
- Studio Build and Evidence share the same Trust Chain contract rows for runtime manifest, QA trace evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries. Those rows must derive from Build Readiness data and expose status, evidence, impact, blockers, and one primary next action instead of independent decorative summaries; `pnpm qa:smoke` checks the shared row ids and next-action binding through the Studio QA bridge.
...
- Studio desktop CSS is split into focused modules after the legacy base stylesheet: `src/styles/studio-desktop-foundation.css` owns neutral command-desk tokens and shared atoms; `src/styles/studio-primitives.css` owns reusable ledger panel shells; `src/styles/studio-command-shell.css`, `src/styles/studio-command-pipeline.css`, `src/styles/studio-command-playfield.css`, and `src/styles/studio-command-console.css` own the active command shell, mission/action rail, viewport/HUD/tooling frame, and bottom console; `src/styles/studio-workbench.css` owns the Workbench surface; `src/styles/studio-assets.css` owns the Assets surface; `src/styles/studio-trust-ledgers.css` owns Build/Evidence trust-ledger rows, status cells, key-value tables, trace strips, and Impact/Next copy; `src/styles/studio-system-ledgers.css` owns Modules/Debug system-ledger rows, contracts, runtime focus, and execution evidence. Keep new Studio desktop work in those modules instead of adding more tail-end overrides to `src/style.css`.
- `pnpm qa:css` reports CSS duplication metrics plus cross-file selector overlap. Current hygiene target: zero exact duplicate rules, zero same-file duplicate selectors, and zero fully shadowed cross-file rules across the scanned CSS files, then steady reduction of cross-file Studio cascade overlap. Latest audit reports 2,626 scanned rules, 0 duplicate selector keys / 0 instances, 0 exact duplicate rules, 168 repeated declaration groups, 135 cross-file overlaps, 0 selectors shared with `src/style.css`, 0 legacy `style.css` rules fully shadowed by later imports, and 0 cross-file rules fully shadowed by later imports. `src/style.css` is now a small base stylesheet. App shell and legacy Studio cascade are split across focused `app-shell.css` and `studio-legacy-*` modules; editor, UI hardening, runtime command-room, shell overrides, command-center, and desktop authority passes are split into focused `studio-editor-*`, `studio-*hardening.css`, `runtime-*`, `studio-shell-*`, `studio-command-*`, and `studio-desktop-*` modules before the existing workbench/assets, trust/system ledger, command palette, stage, and inspector modules. Remaining repeated declaration groups and cross-file overlaps are known cascade debt and should be reduced by extracting shared chrome, ledger-row, status-cell, and command-action primitives, not by adding more final override blocks.
...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. Use `pnpm qa:css -- --detail-overlaps` before cleanup and inspect cross-file overlap manually when touching import order. The latest current-working-tree audit keeps same-file duplicate selectors, exact duplicate rules, `src/style.css` overlap, and fully shadowed cross-file rules at zero, but still reports cross-file selector overlaps and repeated declaration groups across Studio modules; those should be collapsed through primitives, not tail-end overrides.
