...
- Continue reducing duplicated global Studio CSS into reusable chrome, ledger-row, status-cell, and command-action primitives. Use `pnpm qa:css -- --detail-overlaps` before cleanup and inspect cross-file overlap manually when touching import order. The latest current-working-tree audit keeps same-file duplicate selectors, exact duplicate rules, `src/style.css` overlap, and fully shadowed cross-file rules at zero, but still reports 135 cross-file selector overlaps and 168 repeated declaration groups across Studio modules; those should be collapsed through primitives, not tail-end overrides.
