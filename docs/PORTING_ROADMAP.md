...
| Medium | Exact `RemapPal` / ACT parity | Partial `RemapPal` telemetry plus indexed ACT/SFF destination-palette handoff exists; next step is imported fixture/browser evidence, exact source-bank semantics, truecolor/PNG remap behavior, and PalFX/blend/tick-order parity. |
