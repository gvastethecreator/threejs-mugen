...
| IKEMEN root/helper Pause ownership, actor-local movement, positive `p2defmul`, and paused-pass replacement deferral are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause stack, deferred-activation, and helper-SuperPause traces plus focused helper/pause/runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN helper CNS can own Pause or SuperPause with helper identity, root power, helper sound, movetime, and current-target defense scaling`. | `full IKEMEN pause parity`, opposing-team defense breadth, `p2defmul = 0`, nested helper ancestry, or exact audio/buffer parity. | Exact hitpause interaction, teams, source-oracle replay, and broader target/redirect ownership remain open. |
