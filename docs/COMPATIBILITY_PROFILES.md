...
| IKEMEN actor-local Pause movement, positive `p2defmul` stacking, and paused-pass replacement deferral are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause stack, and deferred-activation traces plus focused pause-world, actor-list, profile, and runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN profile finishes eligible prepared actors before publishing a replacement Pause or SuperPause requested during that paused pass`. | `full IKEMEN pause parity`, helper-owned Pause, exact buffer snapshot parity, team-wide target breadth, or `p2defmul = 0`. | Exact hitpause interaction, teams, source-oracle replay, and broad target/redirect ownership remain open. |
