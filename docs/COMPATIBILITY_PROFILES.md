...
| IKEMEN root/helper Pause ownership, actor-local movement, paused-pass replacement, and SuperPause team defense are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause stack, deferred-activation, helper-SuperPause, and team-defense traces plus focused helper/pause/combat/runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN Pause/SuperPause supports root/helper ownership and omitted/non-positive p2defmul fallback across the represented opposing root plus existing helpers`. | `full IKEMEN pause parity`, simul/tag team topology, global config loading, nested helper ancestry, or exact audio/buffer parity. | Exact hitpause interaction, helper defender combat, source-oracle replay, and broader target/redirect ownership remain open. |
