...
| IKEMEN root/helper Pause ownership, SuperPause team defense, complete-character topology, active EnemyNear/P2 eligibility, and public multi-root registry are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause, and team-defense traces plus focused topology/roster/MatchWorld/helper/pause/combat tests and `RuntimeTeamRoster/v0` diagnostics. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN Pause/SuperPause uses source-backed P1-P8 team identity; public P1-P4-plus-helper registry diagnostics preserve pair scheduling`. | `full IKEMEN team/pause parity`, live team-state projection, distance/cache semantics, playable multi-root systems, global config loading, or exact audio/buffer parity. | Scheduler ownership, tag transitions, partner redirects, hitpause interaction, helper defender combat, and source-oracle replay remain open. |
