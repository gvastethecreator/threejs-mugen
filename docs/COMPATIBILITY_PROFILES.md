...
| IKEMEN root RunOrder is selected explicitly. | `ikemen-go` | Executed Partial. | Required `synthetic-imported-ikemen-runfirst.json` plus focused compiler, scheduler, match-advance, runtime, trace-gate, and public facade tests. | Aggregate trace stability under default `unknown`. | `explicit IKEMEN profile applies bounded two-root previous-tick RunFirst/RunLast, MoveType, and id RunOrder`. | `IKEMEN scheduling supported`, `IKEMEN compatible`, or any helper/team claim. | No `RunOrder` trigger, helpers/appended actors, teams/simul, or full source-oracle trace. |
