...
| IKEMEN root/helper Pause ownership, SuperPause team defense, complete-character topology, and active EnemyNear/P2 eligibility are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause, and team-defense traces plus focused topology/helper/pause/combat/runtime tests and `RuntimeTeamRoster/v0` diagnostics. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN Pause/SuperPause uses source-backed P1-P8 team identity; active candidate projections exclude disabled/standby/neutral characters and P2 over-KO entries`. | `full IKEMEN team/pause parity`, distance/cache semantics, playable multi-root systems, global config loading, or exact audio/buffer parity. | Registry ownership, tag transitions, partner redirects, hitpause interaction, helper defender combat, and source-oracle replay remain open. |
