...
| IKEMEN actor-local Pause movement and positive `p2defmul` stacking are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove and SuperPause stack traces plus focused pause-world, actor-list, profile, and runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN profile advances eligible roots/helpers in prepared order during Pause and stacks bounded same-frame positive p2defmul for current targets`. | `full IKEMEN pause parity`, deferred pause activation, helper-owned Pause, team-wide target breadth, or `p2defmul = 0`. | Exact hitpause interaction, teams, source-oracle replay, and broad target/redirect ownership remain open. |
