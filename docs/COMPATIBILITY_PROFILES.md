...
| IKEMEN root/helper Pause ownership, SuperPause team defense, and complete-character team topology are selected explicitly. | `ikemen-go` | Executed Partial. | Required actor-pausemove, SuperPause, and team-defense traces plus focused topology/helper/pause/combat/runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN Pause/SuperPause uses source-backed P1-P8 team identity and omitted/non-positive p2defmul fallback across represented opposing characters`. | `full IKEMEN team/pause parity`, active/standby selection, playable multi-root systems, global config loading, or exact audio/buffer parity. | Tag transitions, partner redirects, hitpause interaction, helper defender combat, and source-oracle replay remain open. |
