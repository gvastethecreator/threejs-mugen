...
| IKEMEN simultaneous Pause buffers are selected explicitly. | `ikemen-go` | Executed Partial. | Required `synthetic-imported-ikemen-pause-buffer.json` plus focused Pause world/profile/runtime tests. | Existing Pause/SuperPause fixture corpus under default `unknown`. | `explicit IKEMEN profile applies bounded same-frame Pause/SuperPause ownership arbitration and SuperPause precedence`. | `full IKEMEN pause parity`, `multi-actor movetime`, or `p2defmul stacking`. | No actor-local simultaneous movetime, helper/custom-state ownership, simultaneous `p2defmul`, exact hitpause interaction, teams, or source-oracle parity. |
