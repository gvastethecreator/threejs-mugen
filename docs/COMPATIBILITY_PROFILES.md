...
```txt
Partial MUGEN 1.1 fixture-backed runtime, native generated roster, IKEMEN scanner only.
```
...
| MUGEN 1.1 official fixture route executes. | `mugen-1.1` | Executed Partial unless oracle parity exists. | Required synthetic gate plus optional official fixture artifact present and passed. | KFM/KFM720 trace artifacts, imported stage fixture. | `official KFM route executed partial in artifact <name>`. | `KFM works`, `full MUGEN compatible`, `Executed Parity` without oracle comparison. | Exact tick order, exact recovery thresholds/velocities, broader recovery parity beyond bounded `5200/5201` and `5210` routes, guard timing/effects, unsupported controllers. |
