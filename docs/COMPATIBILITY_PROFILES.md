...
| IKEMEN root RunOrder is selected explicitly. | `ikemen-go` | Executed Partial. | Focused scheduler, match-advance, runtime, and public facade tests. | Aggregate trace stability under the default `unknown` profile. | `explicit IKEMEN profile applies bounded two-root previous-tick MoveType RunOrder`. | `IKEMEN scheduling supported`, `IKEMEN compatible`, or any helper/team claim. | No `RunFirst`/`RunLast`, `RunOrder` trigger, helpers, teams/simul, or full source-oracle trace. |
