...
| `ikemen-go` | Explicit IKEMEN-GO runtime policy selection. | Executed Partial for named, source-backed slices only. | Two-root previous-tick `MoveType` RunOrder; no general IKEMEN content execution claim. |
...
| IKEMEN root RunOrder is selected explicitly. | `ikemen-go` | Executed Partial. | Required `synthetic-imported-ikemen-runfirst.json` and `synthetic-imported-ikemen-runorder.json` plus focused compiler/expression/scheduler/runtime/public-facade tests. | Aggregate trace stability under default `unknown`. | `explicit IKEMEN profile applies bounded two-root ordering and exposes one-based root RunOrder`. | `IKEMEN scheduling supported`, `IKEMEN compatible`, or any helper/team claim. | No helper/appended-actor indices, teams/simul, exact Pause/hitpause order, or full source-oracle trace. |
