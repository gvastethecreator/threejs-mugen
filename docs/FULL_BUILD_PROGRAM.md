...
- `evaluateRuntimeTraceGate` now converts runtime traces into explicit pass/fail evidence for actor source/kind, effect kind, routed states, executed states, executed controllers, active commands, event categories, combat reasons, final actor state/control requirements, and simple final hit-fall metadata. This covers native traces, synthetic imported CMD/CNS traces, a synthetic `fall.*` metadata gate, a synthetic attacker-owned custom get-hit controller-flow gate, and optional local KFM fixture traces for `x -> 200`, target partial hitstun, `x -> 200 -> idle/control`, held-back guard, plus `D, DF, F, x -> 1000` when `.scratch/fixtures/kfm-official.zip` is available.
