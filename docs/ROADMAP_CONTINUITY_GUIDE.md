...
Latest closed R1 runtime/trace checkpoint: required `synthetic-imported-crouch-guard-hold-crouch-return.json` checksum `83ecb699` proves bounded synthetic Common1-style crouch guard-hit route `152 -> 153 -> 130 -> 10`, with active `holdback`/`holddown`/`x`, preserved `153` slide-stop controller/typed-operation order, and final imported crouch state/action `10` with control. Previous required `synthetic-imported-default-guard-hold-walk-return.json` checksum `75d4db9c` proves the matching portable stand guard-hit route `150 -> 151 -> 130 -> 20`; optional private-fixture `kfm-official-default-guard-hold-walk-return.json` checksum `885bb1da` mirrors that real KFM/Common1 stand route when `.scratch/fixtures/kfm-official.zip` exists. Previous required `synthetic-imported-default-fall-ground-recovery-priority.json` checksum `e83b2db7` remains the portable ground-recovery priority oracle, and previous optional private-fixture `kfm-official-default-fall-ground-recovery-priority.json` checksum `6d361534` mirrors that recovery route on real KFM data. Current `pnpm qa:trace` passes 294/294 artifacts, 265 required and 29 optional. This is bounded crouch/stand guard-hold return plus ground-over-air selection evidence only; exact KFM `131 -> 11` crouch return routing, exact guard-hold duration, exact guard timing/proximity/effects, recovery threshold tables, controller-loop timing, ground/air recovery arbitration, velocity math, visual/audio parity, score movement, and full Common1 guard/recovery parity remain blocked.
