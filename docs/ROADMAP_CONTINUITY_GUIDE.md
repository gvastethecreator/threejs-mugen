...
Latest closed R1 runtime/trace checkpoint: required `synthetic-imported-default-fall-ground-recovery-priority.json` checksum `e83b2db7` proves bounded Common1-style ground-recovery priority after a fall imported `HitDef` with `fall.recover = 1` and no `p2stateno`. The defender routes `5000 -> 5030 -> 5050 -> 5200 -> 5201 -> 52 -> 0`, keeps active `recovery` command evidence, and forbids generic air-recovery state `5210` plus lie-down chain states. Current trace aggregate is 289/289 artifacts, 263 required and 26 optional. This is ground-over-air selection evidence only; no exact recovery threshold tables, controller-loop timing, ground/air recovery arbitration constants, velocity math, public KFM support, visual/audio parity, score movement, or full fall/recovery parity claim.
