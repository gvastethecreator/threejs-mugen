...
Latest closed R1 palette/presentation checkpoint: `synthetic-imported-palfx-remappal.json` checksum `ba5fc1e6` is required in `pnpm qa:trace` and proves bounded same-actor imported `PalFX` + `RemapPal` execution lowers into typed `sprite-effect:palfx` / `sprite-effect:remappal` operation evidence and reaches combined actor-frame palette telemetry. Current trace aggregate is 282/282 artifacts, 257 required and 25 optional. This is palette telemetry only; no ACT/SFF pixel palette application, exact palette math, `sinadd`, blend order, renderer parity, visual parity, score movement, or full presentation parity claim. Previous R1 presentation checkpoint: `synthetic-imported-envcolor-under.json` checksum `0a7b5c96` remains required for bounded imported `EnvColor under = 1`; `synthetic-imported-envcolor.json` checksum `956b0f4b` remains the `under = 0` route.
