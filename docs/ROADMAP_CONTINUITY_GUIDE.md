...
Latest closed R1 audio diagnostic checkpoint: `synthetic-imported-sound.json` checksum `91574367` remains required in `pnpm qa:trace` and now proves bounded imported `PlaySnd volume = -8` lowers into typed `audio:playsnd` metadata and survives as `legacyVolume: -8` in required sound-event rules, trace evidence, and final actor sound-event telemetry. Playback still ignores legacy `volume` and uses `volumescale` for gain, matching modern MUGEN docs. This is diagnostic telemetry only; no pre-RC8 legacy volume gain semantics, exact priority classes, dynamic sound params, global channel fallback, pause/superpause audio, timing/mixing, visual/audio parity, score movement, or full audio parity claim.
