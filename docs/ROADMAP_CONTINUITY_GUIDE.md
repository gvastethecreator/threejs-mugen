...
Latest closed R1 runtime/trace checkpoint: required `synthetic-imported-hitfall-canrecover-ready.json` checksum `c0097d7f` proves bounded Common1-style `HitFall && CanRecover` routing after a fall imported `HitDef` with `fall.recover = 1` and no `p2stateno`. The defender routes `5000 -> 5030 -> 5050 -> 5250 -> 0`, records positive-to-zero `fall.recovertime` evidence inside `5050`, has no active `recovery` command, and forbids recovery states `5210` / `5200`. Current trace aggregate is 287/287 artifacts, 261 required and 26 optional. This is readiness trigger/order evidence only; no exact recovery threshold tables, controller-loop timing, recovery-input arbitration, velocity math, public KFM support, visual/audio parity, score movement, or full fall/recovery parity claim.
