...
Latest closed R1 runtime/trace checkpoint: required `synthetic-imported-hitfall-recovery-input-priority.json` checksum `bae07bde` proves bounded Common1-style recovery-input priority after a fall imported `HitDef` with `fall.recover = 1` and no `p2stateno`. The defender routes `5000 -> 5030 -> 5050 -> 5210 -> 0`, keeps active `recovery` command evidence, and forbids competing probe state `5250` plus ground-recovery state `5200`. Current trace aggregate is 288/288 artifacts, 262 required and 26 optional. This is input-priority evidence only; no exact recovery threshold tables, controller-loop timing, ground/air recovery arbitration, velocity math, public KFM support, visual/audio parity, score movement, or full fall/recovery parity claim.
