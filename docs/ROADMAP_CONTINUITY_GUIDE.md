...
Latest closed R1 runtime/trace checkpoint: optional private-fixture `kfm-official-default-guard-hold-walk-return.json` checksum `885bb1da` proves real KFM/Common1 stand guard-hit route `150 -> 151` returns through observable guard-hold state `130` and then resumes held-back walking state/action `20` with control while preserving stricter stand slide-stop controller/typed-operation order when `.scratch/fixtures/kfm-official.zip` exists. Required `synthetic-imported-default-fall-ground-recovery-priority.json` checksum `e83b2db7` remains the portable oracle: after a fall imported `HitDef` with `fall.recover = 1` and no `p2stateno`, the defender routes `5000 -> 5030 -> 5050 -> 5200 -> 5201 -> 52 -> 0`, keeps active `recovery` command evidence, and forbids generic air-recovery state `5210` plus lie-down chain states. Previous optional private-fixture `kfm-official-default-fall-ground-recovery-priority.json` checksum `6d361534` mirrors that recovery branch against real KFM data. Current trace aggregate is 292/292 artifacts, 263 required and 29 optional. This is optional guard-hold-to-walk plus ground-over-air selection evidence only; no exact guard-hold duration, exact guard timing/proximity/effects, recovery threshold tables, controller-loop timing, ground/air recovery arbitration constants, velocity math, public KFM support, visual/audio parity, score movement, or full guard/fall/recovery parity claim.
