...
Latest closed R1 runtime/trace checkpoint: optional private-fixture `kfm-official-default-fall-ground-recovery-priority.json` checksum `6d361534` mirrors the bounded Common1-style ground-recovery priority branch against real KFM data when `.scratch/fixtures/kfm-official.zip` exists. Required `synthetic-imported-default-fall-ground-recovery-priority.json` checksum `e83b2db7` remains the portable oracle: after a fall imported `HitDef` with `fall.recover = 1` and no `p2stateno`, the defender routes `5000 -> 5030 -> 5050 -> 5200 -> 5201 -> 52 -> 0`, keeps active `recovery` command evidence, and forbids generic air-recovery state `5210` plus lie-down chain states. Current trace aggregate is 290/290 artifacts, 263 required and 27 optional. This is ground-over-air selection evidence only; no exact recovery threshold tables, controller-loop timing, ground/air recovery arbitration constants, velocity math, public KFM support, visual/audio parity, score movement, or full fall/recovery parity claim.
