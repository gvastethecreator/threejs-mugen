...
Latest ownership checkpoint: `HitSparkAssetSystem` now owns bounded player/common/FightFX HitDef spark asset-frame lookup consumed by `PlayableMatchRuntime` before `RuntimeHitEffectWorld` records events. `RuntimeRandomSystem` remains the deterministic random ownership checkpoint, and `RuntimeContactMemoryWorld` remains the latest contact-memory ownership checkpoint. These are ownership cleanups only, not new parity or score movement.
