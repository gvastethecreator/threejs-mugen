...
Latest trace aggregate: `pnpm qa:trace` passes 157/157 artifacts, with 139 required and 18 optional local-fixture artifacts. The latest required runtime oracle is `synthetic-imported-target-noko.json` checksum `28ac8636`, proving bounded imported `TargetLifeAdd` lethal target-controller damage clamps to final P2 life `1` when defender-side `AssertSpecial NoKO` executed before P1 `HitDef` and later `TargetLifeAdd`; exact NoKO lifetime, round flow, helper/root/parent redirects, teams, multi-target behavior, and full target parity remain blocked. Previous `synthetic-imported-variable.json` checksum `3b33f7a8` still proves bounded imported `VarRandom` executes as typed `variable:varrandom` next to `VarSet`, `VarAdd`, and `VarRangeSet`.
...
The latest optional official KFM guard-hit oracles remain `kfm-official-default-guard-state.json` checksum `885bb1da`, `kfm-official-default-crouch-guard-state.json` checksum `d11153d0`, and `kfm-official-default-air-guard-state.json` checksum `f4378971`.
