...
Latest trace aggregate: `pnpm qa:trace` passes 161/161 artifacts, with 141 required and 20 optional local-fixture artifacts. The latest required Common1 oracle is `synthetic-imported-default-gethit-progression.json` checksum `ef2a67f8`, proving bounded stand get-hit progression order through `5000` `ChangeState` before `5001` `ChangeState`, actor-frame `5000 -> 5001`, and final idle/control evidence. The latest required presentation oracles are `synthetic-imported-hitdef-common-spark.json` checksum `5ea054d7` and `synthetic-imported-hitdef-fightfx-spark.json` checksum `11537b56`, proving bounded common/default and FightFX `HitSpark` source-frame metadata before renderer handoff. The latest optional private-fixture KFM oracles are `kfm-official-x-hit-sound.json` checksum `9668e88a` and `kfm-official-x-hit-spark.json` checksum `9668e88a`, proving the real KFM `x -> 200` route emits bounded `HitDef` hit sound telemetry `S5,0` and bounded `HitSpark` telemetry from authored `sparkno 0` after contact when `.scratch/fixtures/kfm-official.zip` exists. Previous required `synthetic-imported-target-noko.json` checksum `28ac8636` proves bounded imported `TargetLifeAdd` lethal target-controller damage clamps to final P2 life `1` when defender-side `AssertSpecial NoKO` executed before P1 `HitDef` and later `TargetLifeAdd`; exact NoKO lifetime, round flow, helper/root/parent redirects, teams, multi-target behavior, and full target parity remain blocked. Previous `synthetic-imported-variable.json` checksum `3b33f7a8` still proves bounded imported `VarRandom` executes as typed `variable:varrandom` next to `VarSet`, `VarAdd`, and `VarRangeSet`.

Recent required oracles remain stable: `synthetic-imported-noop.json` checksum `a5fe169e`, `synthetic-imported-target-owned-custom-state.json` checksum `410fb8c0`, `synthetic-imported-default-gethit-progression.json` checksum `ef2a67f8`, `synthetic-imported-hitdef-hit-sound.json` checksum `6fc00d8a`, `synthetic-imported-hitdef-common-spark.json` checksum `5ea054d7`, `synthetic-imported-hitdef-fightfx-spark.json` checksum `11537b56`, `synthetic-imported-hitby-reject.json` checksum `65185fd1`, `synthetic-imported-hitby-allow.json` checksum `c75d5c7d`, `synthetic-imported-hitpausetime-ignorehitpause.json` checksum `a3a78bb8`, and `synthetic-imported-modifyexplod.json` checksum `bca75991`.
