...
Latest trace aggregate: `pnpm qa:trace` passes 187/187 artifacts, with 167 required and 20 optional local-fixture artifacts. The latest required Common1 oracle is `synthetic-imported-default-fall-official-air-recovery.json` checksum `b0363be9`, proving a bounded official-style synthetic `5050 -> 5210 -> 52 -> 0` air-recovery route after `command = "recovery"` while airborne, with positive-to-zero `fall.recovertime` actor-frame sequence evidence, `5210` recovery velocity telemetry, landing-state `52` y = 0 evidence, ordered recovery/land controller and typed-operation evidence, and final idle/control. Previous Common1 oracles remain required: `synthetic-imported-default-fall-official-ground-recovery.json` checksum `74b72495`, `synthetic-imported-default-fall-official-recovery-threshold.json` checksum `86804271`, `synthetic-imported-default-fall-official-recovery-too-early.json` checksum `ef945ff5`, and `synthetic-imported-default-fall-recovery.json` checksum `d83797d9`. The latest helper/effect oracle remains `synthetic-imported-helper-projcontact.json` checksum `07653cee`, now requiring owner-side target-link evidence, shared guard sound/FightFX spark package telemetry, authored `sparkxy = 15,-63`, and Projectile payload `parentId = p1-helper-0`; companion required helper/effect proofs `synthetic-imported-helper-projguard.json` checksum `3353eda7` and `synthetic-imported-helper-projhit.json` checksum `3892716e` now carry matching package requirements with authored spark offsets. This does not move scores because exact `down.recovertime` / `fall.recovertime` tables, exact Common1 controller-loop timing, exact velocity math, public bundled KFM support, player-state `BindToParent` / `BindToRoot`, exact helper effect-count/ownership parity, exact helper-owned projectile target memory, exact `ProjContact` / `ProjHit` / `ProjGuarded` timing and lifetime, dynamic bind/effect/projectile params, position rebinding, nested helper ancestry where root differs from parent, `EnemyNear(index)`, helper-owned opponents, helper-owned `HitDef`/effect namespaces, helper combat/contact presentation, helper/projectile target redirects, unsupported or negative target-id expressions, mutation through redirects, teams, multi-target selection, exact target/helper lifetime or tick order, keyctrl, and broader VM behavior remain blocked. The latest optional private-fixture KFM recovery oracle is `kfm-official-default-fall-recovery.json` checksum `b1c6456a`, applying matching bounded `5110 -> 5120` actor-frame/controller-order requirements to real KFM when `.scratch/fixtures/kfm-official.zip` exists. The latest required presentation oracles are `synthetic-imported-hitdef-common-spark.json` checksum `5ea054d7`, `synthetic-imported-hitdef-fightfx-spark.json` checksum `11537b56`, `synthetic-imported-hitdef-common-guard-spark.json` checksum `7650a09c`, `synthetic-imported-hitdef-fightfx-guard-spark.json` checksum `32f3e92d`, `synthetic-imported-hitdef-hit-effect-package.json` checksum `46aa5ce1`, and `synthetic-imported-hitdef-guard-effect-package.json` checksum `1c3167b7`, proving bounded common/default and FightFX hit/guard `HitSpark` source-frame metadata plus authored spark offsets before renderer handoff and combined direct hit/guard sound and spark telemetry.
