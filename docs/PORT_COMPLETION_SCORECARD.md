...
Latest trace aggregate: `pnpm qa:trace` passes 194/194 artifacts, with 174 required and 20 optional local-fixture artifacts. The latest direct target oracle is `synthetic-imported-default-target-redirect.json` checksum `d43caabf`, proving bounded player state `200` can activate direct `HitDef` without `id`, remember P2 in target memory with target id `0`, expose target link `p1 -> p2 / 0`, and branch to state `269` through `Target(0), Life < 1000`. Previous default `NumTarget(0)` oracle remains `synthetic-imported-default-numtarget.json` checksum `5869ebbd`; previous helper direct default-target oracle remains `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default-target oracle remains `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous helper Projectile explicit-id oracle remains `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous helper direct target oracle remains `synthetic-imported-helper-target.json` checksum `68f95b67`; previous helper direct-combat oracle remains `synthetic-imported-helper-hitdef.json` checksum `89f9e876`. Common1, helper/effect Projectile, optional KFM recovery, and presentation oracles remain active. This does not move scores because exact `down.recovertime` / `fall.recovertime` tables, exact Common1 controller-loop timing, exact velocity math, public bundled KFM support, player-state `BindToParent` / `BindToRoot`, exact helper effect-count/ownership parity, target multi-target/mutation parity, helper custom-state targets, throws, teams, exact helper hitpause/tick order, exact helper `HitDef` / Projectile lifetime parity, exact `ProjContact` / `ProjHit` / `ProjGuarded` timing and lifetime, dynamic bind/effect/projectile params, position rebinding, nested helper ancestry where root differs from parent, `EnemyNear(index)`, unsupported or negative target-id expressions, mutation through redirects, multi-target selection, exact target/helper lifetime or tick order, keyctrl, and broader VM behavior remain blocked.
