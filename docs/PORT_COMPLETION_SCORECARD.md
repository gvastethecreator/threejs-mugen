...
Latest trace aggregate: `pnpm qa:trace` passes 196/196 artifacts, with 176 required and 20 optional local-fixture artifacts. The latest helper target oracle is `synthetic-imported-helper-bare-target.json` checksum `15f3c1db`, proving bounded helper state `1200` can activate direct `HitDef` with target id `8878`, remember P2 in helper target memory, expose target link `p1-helper-0 -> p2 / 8878`, and branch to state `1230/962` through bare `Target, Life <= 965`. Previous player bare `Target, Life` oracle remains `synthetic-imported-bare-target-redirect.json` checksum `f9c90aa8`; previous default `Target(0), Life` oracle remains `synthetic-imported-default-target-redirect.json` checksum `d43caabf`; previous default `NumTarget(0)` oracle remains `synthetic-imported-default-numtarget.json` checksum `5869ebbd`; previous helper direct default-target oracle remains `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default-target oracle remains `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous helper Projectile explicit-id oracle remains `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous helper direct target oracle remains `synthetic-imported-helper-target.json` checksum `68f95b67`; previous helper direct-combat oracle remains `synthetic-imported-helper-hitdef.json` checksum `89f9e876`. Common1, helper/effect Projectile, optional KFM recovery, and presentation oracles remain active. This does not move scores because exact `down.recovertime` / `fall.recovertime` tables, exact Common1 controller-loop timing, exact velocity math, public bundled KFM support, player-state `BindToParent` / `BindToRoot`, exact helper effect-count/ownership parity, target multi-target/mutation parity, helper custom-state targets, throws, teams, exact helper hitpause/tick order, exact helper `HitDef` / Projectile lifetime parity, exact `ProjContact` / `ProjHit` / `ProjGuarded` timing and lifetime, dynamic bind/effect/projectile params, position rebinding, nested helper ancestry where root differs from parent, `EnemyNear(index)`, unsupported or negative target-id expressions, mutation through redirects, multi-target selection, exact target/helper lifetime or tick order, keyctrl, and broader VM behavior remain blocked.
