...
Latest trace aggregate: `pnpm qa:trace` passes 159/159 artifacts, with 139 required and 20 optional local-fixture artifacts. The latest optional private-fixture KFM oracles are `kfm-official-x-hit-sound.json` checksum `9668e88a` and `kfm-official-x-hit-spark.json` checksum `9668e88a`, proving the real KFM `x -> 200` route emits bounded `HitDef` hit sound telemetry `S5,0` and bounded `HitSpark` telemetry from authored `sparkno 0` after contact when `.scratch/fixtures/kfm-official.zip` exists. The latest required runtime oracle remains `synthetic-imported-target-noko.json` checksum `28ac8636`, proving bounded imported `TargetLifeAdd` lethal target-controller damage clamps to final P2 life `1` when defender-side `AssertSpecial NoKO` executed before P1 `HitDef` and later `TargetLifeAdd`; exact NoKO lifetime, round flow, helper/root/parent redirects, teams, multi-target behavior, and full target parity remain blocked. Previous `synthetic-imported-variable.json` checksum `3b33f7a8` still proves bounded imported `VarRandom` executes as typed `variable:varrandom` next to `VarSet`, `VarAdd`, and `VarRangeSet`.

Recent required oracles remain stable: `synthetic-imported-noop.json` checksum `a5fe169e`, `synthetic-imported-target-owned-custom-state.json` checksum `410fb8c0`, `synthetic-imported-hitdef-hit-sound.json` checksum `6fc00d8a`, `synthetic-imported-hitby-reject.json` checksum `65185fd1`, `synthetic-imported-hitby-allow.json` checksum `c75d5c7d`, `synthetic-imported-hitpausetime-ignorehitpause.json` checksum `a3a78bb8`, and `synthetic-imported-modifyexplod.json` checksum `bca75991`.

The current HitDef-effect oracle set is `synthetic-imported-hitdef-hit-sound.json` checksum `6fc00d8a`, `synthetic-imported-hitdef-guard-sound.json` checksum `fdf1f7f6`, `synthetic-imported-hitdef-hit-spark.json` checksum `b6554124`, and `synthetic-imported-hitdef-guard-spark.json` checksum `72c8fa3a`. `HitSparkRenderer` treats `S` spark refs as player AIR action refs, classifies unprefixed refs as common/default and `F` refs as FightFX, preserves runtime-provided package frames from `hitSparkLibraries`, synthesizes bounded common/FightFX system lookup frames when no package frame exists, and `pnpm qa:smoke` requires active desktop/mobile sparks plus player-source resolved-sprite diagnostics. `MugenCharacterLoader` can load optional `data/fight.def` / `fightfx.air` / `fightfx.sff`, `createImportedFighterDefinition` can hand those AIR actions to runtime `hitSparkLibraries`, and `App` can register decoded system SFF sprites through the global hit-spark provider route; exact common/FightFX layering, scale, palette, timing, channel ownership, motif/screenpack ownership, and full presentation parity remain blocked.
