...
| Combat/Common1 | 30 | Direct HitDef, guard, hitstun, fall/get-hit/recovery gates, bounded guard-sound and hit/guard-spark telemetry plus smoke-gated fallback spark rendering/source metadata. | Exact guard/fall/recovery, custom states, throws, priority, KO/round flow. |
...
| Stages/presentation/audio | 25 | Basic imported stage route, EnvShake/EnvColor, partial SND event path, bounded HitDef guard sound plus hit/guard spark telemetry, source metadata, and 120-frame fallback Three.js spark overlay. | BGCtrl parity, screenpacks/lifebars, FightFX/common assets, audio mixing/timing. |
...
1. Remaining `AssertSpecial` and guard precision: broader lifetime/persistence layering, priority, helper/team/global ownership, pause interaction, FightFX/common guard-effect asset lookup beyond fallback sparks, proximity rules, and KFM/Common1 confirmation beyond bounded stand/crouch/air denial, one-frame expiry traces, and optional guard-hit actor-frame physics telemetry.
