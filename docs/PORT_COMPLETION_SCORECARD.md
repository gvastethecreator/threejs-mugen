...
`synthetic-imported-projectile-dynamic-fall-flags` passes at `a39589d7` /
`fb879f83`; full tests, typecheck, build and trace QA `887/887` (`853`
required) pass. This remains bounded, so the score does not move while
Helper/nesting, airborne-only selection, ModifyProjectile, exact fall/KO
timing, teams, rollback and full parity stay open. See [issue
...
transfers resolved siblings to existing recovery metadata and `GetHitVar`
aliases while preserving `owner/root = p1` and `parent = p1-helper-0`. Required
...
## 2026-08-15 T793 `EnvColor time = -1` — historical closed-bounded, no score movement
...
finite duration above 240 is closed by T794. Exact blend/layer/pause,
nested/team ownership, rollback and full parity remain blocked, so the score
does not move. See [issue
368](../.scratch/roadmap/issues/368-envcolor-indefinite-lifetime-selection.md).

## 2026-08-15 T794 finite `EnvColor` duration — closed-bounded, no score movement

Issue 369 closes one finite stage-flash lifetime contract. Product `1cab062e`
removes the local positive 240-tick ceiling while retaining zero rejection and
negative indefinite handling. Required
`synthetic-imported-envcolor-long-finite` proves finite `time = 241` persists
for 241 frames then expires (`6a60ee57` / `58e50e4b`); aggregate trace QA is
`873/873` (`839` required). Exact blend/layer/pause, nested/team ownership,
