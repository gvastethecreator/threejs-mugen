...
`src/tests/RuntimeTrace.test.ts` and `scripts/qa_traces.cjs` also contain optional official KFM trace gates. They run only when `.scratch/fixtures/kfm-official.zip` exists locally, load the ZIP through the normal browser loader path, route MUGEN `x` into state `200`, verify the target reaches the current partial hitstun/get-hit snapshot, verify a longer `x` recovery script returns P1 to idle/control, route `D, DF, F, x` into Light Kung Fu Palm state `1000`, and trace basic movement through authored KFM states `20 -> 11 -> 41 -> 0` from scripted `F`, `D`, and `U` input. The gates verify imported actor source, routed/executed state IDs, executed controllers, active commands such as `x` and `QCF_x`, ordered actor-frame buckets where relevant, and final actor state/control constraints. The required portable tests use synthetic imported CMD/CNS fixtures so the repository test suite does not depend on ignored external assets.
