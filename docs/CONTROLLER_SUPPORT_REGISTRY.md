...
| `HitDef` | compiled, executed-partial | `synthetic-imported-x.json`, `synthetic-imported-custom-state.json`, `synthetic-imported-hitdef-priority.json`, `synthetic-imported-hitdef-kill.json`, `synthetic-imported-hitdef-guard-kill.json`, `synthetic-imported-air-guard-state.json`, `synthetic-imported-assertspecial-noko.json`, KFM optional traces | Basic attr, guardflag, damage, pause, hit velocity, fall metadata, p1/p2 state routing, bounded owner-backed `p2stateno` custom-state chain/return, bounded direct `priority` clash where a higher numeric priority suppresses a lower-priority direct attack, bounded stand/crouch/air guard-hit state routing where defender-owned Common1-style states exist, and bounded nonlethal life clamps for direct `kill = 0`, `guard.kill = 0`, stored `fall.kill = 0` fall damage, and defender-side `AssertSpecial NoKO`. Exact MUGEN/IKEMEN priority classes, KO/round/no-KO interactions beyond this clamp, exact guard physics/effects, reversal/projectile/helper/custom-state priority, throws, and multi-hit priority remain partial. |
