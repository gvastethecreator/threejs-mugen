...
| `PlaySnd`, `StopSnd` | executed-partial | `synthetic-imported-sound.json` checksum `911aa62d`, runtime events | Emits bounded `RuntimeSoundEvent` evidence with parsed `Sgroup,index`, channel, state, and tick data. Web Audio playback/SND channel parity, looping, volume, pan, priority, and exact timing remain partial. |
