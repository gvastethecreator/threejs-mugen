...
| `EnvShake` | executed-partial | `synthetic-imported-envshake.json` checksum `50e0ad4c`, runtime events | Numeric `time`/`freq`/`ampl`/`phase` params emit bounded `RuntimeEnvShakeEvent` telemetry and deterministic camera-shake snapshots. Dynamic params, `mul`, exact pause/stage/layer interaction, helper/redirect ownership, and engine-perfect waveform parity remain future work. |
