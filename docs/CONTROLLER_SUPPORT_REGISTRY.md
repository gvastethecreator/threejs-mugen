...
| `MoveHitReset` | executed-partial | runtime tests | Clears bounded state-local direct `MoveContact`/`MoveHit`/`MoveGuarded` contact memory for the controller owner. Projectile contact memory is intentionally not cleared by this bridge. Exact helper/redirect/team ownership and tick-order parity remain future work. |
