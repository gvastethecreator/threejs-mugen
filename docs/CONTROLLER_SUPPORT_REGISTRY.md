...
| `Target*`, `BindToTarget` | compiled, executed-partial | `synthetic-imported-target.json`, `synthetic-imported-targetbind-pause.json` | Target memory exists; TargetBind can keep a bound target offset while the source actor advances during bounded `SuperPause` movetime. BindToTarget can bind the controller owner to a recent target with static `pos = x,y,Foot` and bounded `time`. Complex target binding/custom-state parity, redirects, teams, and Head/Mid anchors remain partial. |
