...
Current Angle presentation note: `RuntimeSpriteEffectWorld` now includes bounded `AngleDraw value/scale` render-angle/render-scale telemetry through the sprite-effect boundary, including dynamic active-state fallback. This still does not prove exact axis pivot, collision rotation/scale, draw-order/palette interaction, or full presentation parity.
