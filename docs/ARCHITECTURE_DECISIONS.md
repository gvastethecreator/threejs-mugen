...
Current cut: `RuntimeEffectActorWorld` is the accepted boundary for helper/projectile/explod stores. `MatchWorld` creates and injects it; `PlayableMatchRuntime` can request spawns, active-effect advances, presentation-effect advances, removals, snapshots, summaries, the bounded projectile contact/reject/override/damage/removal loop, and bounded `projpriority` projectile-vs-projectile clash resolution through that contract. `FighterMatchState` no longer stores the raw effect actor store. `MatchWorld` also exposes target-memory links from actor snapshots for debug/trace evidence. This does not yet mean helper VM parity, exact projectile combat parity, exact priority/cancel/remove animation parity, exact target semantics, exact effect pause/tick order, or full parent/root ownership parity.
