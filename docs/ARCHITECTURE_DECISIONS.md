...
Current variable cut: `VarRandom` is now a bounded `StateControllerExecutor` variable operation using deterministic sandbox-side actor RNG supplied by `PlayableMatchRuntime`; trace evidence proves owner-local int var writes only, not exact MUGEN random stream or helper/parent/root variable scope.
