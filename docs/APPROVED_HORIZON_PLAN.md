...
7. R1: after the recovery-threshold, bounded synthetic actor-frame tick-order, bounded synthetic air-recovery velocity, and bounded synthetic ground-selection/velocity gates, add exact controller/VM tick-order, optional KFM threshold oracle, broader recovery parity, or guard/Common1 evidence beyond the current positive route plus early reject pair.
