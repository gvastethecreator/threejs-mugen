...
- Typed controller-operation evidence where the gate depends on compiled IR side effects such as `HitDef`, `HitVelSet`, `PosFreeze`/`ScreenBound`, `AttackMulSet`/`DefenceMulSet`, `Target*`, `Pause`/`SuperPause`, `Projectile`, `Helper`, or `Explod`.
...
- `kfm-official-default-fall-recovery-input.json`: optional trace where official KFM as defender receives a recoverable fall HitDef, `CanRecover` becomes true, defender input `x+y` activates `command = "recovery"`, and real Common1 states execute `5050 -> 5210 -> 52 -> 0` before final idle/control. The current local fixture artifact executes `52, 200, 5000, 5030, 5050, 5210` with active commands `x`, `y`, and `recovery`, checksum `411f835b`. This proves the bounded official air recovery-input route only, not exact recovery thresholds/velocities, tick-order parity, or guard-state parity.
