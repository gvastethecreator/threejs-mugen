...
- Typed controller-operation evidence where the gate depends on compiled IR side effects such as `HitDef`, `HitVelSet`, `PosFreeze`/`ScreenBound`, `AttackMulSet`/`DefenceMulSet`, `Target*`, `Pause`/`SuperPause`, `SprPriority`, `Projectile`, `Helper`, or `Explod`.
