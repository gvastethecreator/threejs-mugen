...
- `synthetic-imported-trans.json`: required imported CMD/CNS State -1 `x -> 200` route where static `Trans trans = addalpha,128,128` compiles into typed `sprite-effect:trans`, executes during state `200`, and exposes actor-frame render-opacity telemetry (`opacity <= 0.5`). Current checksum: `993d50ed`. This proves bounded opacity telemetry only, not exact add/sub alpha math, palette/remap interaction, draw-order interaction, or timing parity.
...
- `synthetic-imported-helper-supermovetime.json`: required imported CMD/CNS route where a visual `Helper` with bounded `supermovetime = 4` advances during `SuperPause` after source movetime expires while the opponent and sibling Explod freeze. Current checksum: `391eb2dc`. This proves bounded visual helper pause-budget handling only, not helper VM execution, exact pause layering/tick order, redirects, keyctrl, parent/root semantics, helper combat, or full Helper parity.
- `synthetic-imported-helper-pausemovetime.json`: required imported CMD/CNS route where a visual `Helper` with bounded `pausemovetime = 4` advances during regular `Pause` after source movetime expires while the opponent and sibling Explod freeze. Current checksum: `59cd59ad`. This proves bounded visual helper regular-pause budget handling only, not helper VM execution, exact pause layering/tick order, redirects, keyctrl, parent/root semantics, helper combat, or full Helper parity.
- `synthetic-imported-helper-ignorehitpause.json`: required imported CMD/CNS route where a visual `Helper` with `ignorehitpause = 1` advances through hitpause while sibling Explods provide freeze/advance contrast. Current checksum: `5dd3072e`. This proves bounded visual helper hitpause advance only, not helper VM execution, exact hitpause layering/tick order, redirects, keyctrl, parent/root semantics, helper combat, or full Helper parity.
