...
`RuntimeRootAdvancePhaseWorld` now owns the immutable explicit-IKEMEN normal-tick classification of roots into `playable`, `active-motion`, and `bounded-standby`. `RuntimeRootMotionAdvanceWorld` executes only state clock, side-effect-free motion CNS, local kinematics, and animation for an already-live P3-P8 root, in that order. `RuntimeRootPhaseCapabilities/v1` exposes the same boundary to diagnostics. This is a deliberately incomplete character-action slice: direct native input/AI, Pause/hitpause motion, sprite/effect lifecycle, hit/contact/recovery, stage constraints, combat, round, presentation, audio/HUD, and resources remain separate owners.

`RuntimeRootPresentationWorld` owns `RuntimeRootPresentation/v0`, a renderer-independent projection with stable per-root reasons and separate ordered body/shadow draw and camera-X ids. `MugenSnapshot.actors` stays the exact playable/HUD/audio/collision pair; reserves remain separate. `RuntimeMatchPresentationSnapshotWorld` consumes selected camera roots but keeps EnvShake/effects pair-owned, and `ThreeMugenRenderer` applies selected roots only to `CharacterRenderer`. This closes bounded visible active-root handoff, not stage clamp/push, collision, combat, exact Tag overlap, camera Y/zoom, HUD/audio, round, resources, or full presentation parity.
