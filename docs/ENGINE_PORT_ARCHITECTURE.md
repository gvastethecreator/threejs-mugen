...
`synthetic-imported-explod-supermovetime.json` adds the first Explod-owned pause-budget contract: compiled/raw `supermovetime` is stored on the visual Explod actor, paused presentation passes can advance that actor after the source actor's `movetime` expires, and a sibling Explod without that budget remains frozen. This keeps pause-budget logic in `ExplodSystem` / `RuntimeEffectActorWorld` instead of teaching Three.js or generic presentation code about CNS params.
