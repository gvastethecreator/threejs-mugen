...
47. `RuntimeHitFallControllerWorld`: own bounded passive `HitFallVel`, `HitFallDamage`, and `HitFallSet` mutations from typed operations or raw controller params outside inline executor branches.
48. `RuntimeStateTypeWorld`: own bounded passive `StateTypeSet` metadata setup from typed operations or raw controller params outside private executor helpers.
49. `RuntimeDamageScaleWorld`: own bounded passive `AttackMulSet` and `DefenceMulSet` multiplier setup from typed operations or raw controller params outside inline executor branches.
50. `RuntimeHitDefenseWorld`: own bounded passive `HitBy`, `NotHitBy`, and `HitOverride` slot setup/removal from typed operations or raw controller params outside private executor helpers.
