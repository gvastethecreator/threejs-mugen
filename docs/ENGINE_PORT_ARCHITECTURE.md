...
`RuntimeEffectSpawnWorld` now owns the bounded spawn/dispatch bridge from active CNS controllers into the effect actor world: Explod action/position/bind/default duration resolution, Helper state/action resolution including state-owner sprite/action lookup, visual-helper removal handoff, Projectile action/offset/terminal-action resolution, RemoveExplod dispatch, and ModifyProjectile dispatch. `PlayableMatchRuntime` still records controller execution/operation evidence and owns match orchestration, so this is an ownership cleanup rather than exact effect spawn timing, helper VM, real `DestroySelf`, parent/root redirect, or FightFX/Common animation parity.
...
13. `HelperSystem`: own the current visual helper actor lifecycle, explicit owner-side removal, and snapshots.
...
17. `RuntimeEffectSpawnWorld` / `EffectSpawnSystem`: own bounded Explod/Helper/Projectile spawn resolution plus RemoveExplod/ModifyProjectile/visual-helper removal dispatch before those operations reach the effect actor world.
