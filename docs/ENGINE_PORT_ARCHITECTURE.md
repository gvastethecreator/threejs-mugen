...
4. `RuntimeHitEffectWorld` / `HitEffectSystem`: own bounded direct `HitDef` spark telemetry outside the match runtime; `HitSparkRenderer` owns the current 120-frame fallback visual projection while exact FightFX/render binding stays future work.
