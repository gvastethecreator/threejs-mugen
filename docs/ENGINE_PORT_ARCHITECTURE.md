...
`RuntimeRoundSystem.roundNoDamage` is the bounded source-mapped close read model for `over.hittime`: `PlayableMatchRuntime` forwards it through `RuntimeMatchPostFighterWorld` into `RuntimeMatchInteractionWorld`, which skips late HitDef/reversal/priority/projectile/helper admission and HitDef target commits while preserving target/effect maintenance, body push, bindings, clamps, and presentation. The public `RuntimePostRound/v0` snapshot remains unchanged. This is a combat-admission boundary, not complete CNS resource-controller suppression, exact frame-start ordering, `over.forcewintime`, release choreography, or full MUGEN/IKEMEN round parity.
