...
`RuntimeRootAdvancePhaseWorld` owns immutable explicit-IKEMEN normal-tick classification into `playable`, `active-motion`, and `bounded-standby`. `RuntimeRootMotionAdvanceWorld` executes state clock, side-effect-free motion CNS, local kinematics, animation, then actor-local stage-X constraints for an already-live P3-P8 root. `RuntimeRootPhaseCapabilities/v4` additionally exposes bounded plural body push and read-only hit admission. Direct native input/AI, Pause/hitpause motion, sprite/effect lifecycle, hit/contact/recovery mutation, combat, round, audio/HUD, and resources remain separate owners.
