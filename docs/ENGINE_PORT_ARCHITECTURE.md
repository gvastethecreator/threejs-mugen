...
`VarRandom` now compiles into typed `variable:varrandom` operation evidence and runs through the same runtime-controller dispatch as `VarSet`, `VarAdd`, and `VarRangeSet`. `PlayableMatchRuntime` owns deterministic per-actor sandbox RNG for trace-stable controller/trigger expression evaluation; this is not exact MUGEN random stream parity and does not solve helper/parent/root variable scopes.
