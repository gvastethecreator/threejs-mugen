...
Current typed operation coverage includes bounded static `hitfall:*`, `kinematic:*`, `collision:*`, `bounds:*`, `metadata:*`, `orientation:*`, `sprite-effect:*`, `envcolor`, `audio:*`, `resource:*`, `variable:*`, `eligibility:*`, `hitoverride`, `reversaldef`, `hitdef`, `pause:*`, `target:*`, effect-spawn, contact, and `damage-scale:*` families, with raw expression fallback retained where params are dynamic or unsupported. The active sprite-effect boundary now emits typed telemetry for bounded dynamic `PalFX time/add/mul/color/invertall`, `AfterImage` params, `AfterImageTime value/time`, `RemapPal source/dest`, `Trans alpha`, `SprPriority value`, and `Angle value/scale` after expression resolution. `RuntimeEnvColorWorld` and `RuntimeEnvShakeWorld` accept bounded dynamic resolvers for their dynamic presentation params and now emit typed `envcolor` / `envshake` telemetry after resolution, `RuntimeActorConstraintWorld` accepts bounded dynamic `Width player/value` resolvers that now emit typed `collision:width` telemetry after resolution, `RuntimeBoundsControllerWorld` accepts bounded dynamic `PlayerPush value` resolvers that now emit typed `collision:playerpush` telemetry after resolution, and `RuntimeAudioControllerDispatchWorld` accepts bounded dynamic sound-value plus audio numeric resolvers for `PlaySnd` / `SndPan` / `StopSnd` event telemetry when no typed audio op exists. `PauseSystem`, `RuntimeAudioWorld`, `RuntimeHitEffectWorld`, `RuntimeContactPresentationWorld`, `RuntimeEnvShakeWorld`, `RuntimeEnvColorWorld`, `SpriteEffectSystem`, `ExplodSystem`, `HelperSystem`, `ProjectileSystem`, `RuntimeProjectileCombatWorld`, `EffectActorSystem`, `TargetSystem`, and `CombatResolver` own the current partial semantic boundaries below that dispatch layer. `RuntimeTraceGate` records actor/effect/stage evidence without renderer coupling. This is still a partial compatibility layer, not exact playback, FightFX/common layering, helper VM parity, target/team semantics, screenpack ownership, or full CNS parity.
...
`RuntimeHelperTelemetryWorld` owns the bounded helper-local Projectile and resource telemetry binding that still lived inline in `PlayableMatchRuntime`: match actors receive `onHelperController` / `onHelperOperation` handlers through one named boundary, helper-local `Projectile`, `CtrlSet`, `LifeAdd`, `LifeSet`, `PowerAdd`, and `PowerSet` controller/operation events are forwarded into compatibility telemetry, and helper state numbers are preferred over owner state numbers with owner-state fallback when helper state is unavailable. `RuntimeHelperTelemetryWorld` does not merge helper resources into root/team banks; `ProjectileSystem` exposes no mutable life/power fields, so projectile snapshots remain resource-less effects. `PlayableMatchRuntime` still supplies the concrete telemetry recorder and owner list, so this is ownership cleanup and bounded local-resource evidence rather than exact helper Projectile tick timing, helper maxima/damage ownership, teams/simul helper ownership, or full Helper VM parity.
...
`RuntimeRootAdvancePhaseWorld` owns immutable explicit-IKEMEN normal-tick classification into `playable`, `active-motion`, and `bounded-standby`. `RuntimeRootMotionAdvanceWorld` executes state clock, restricted CNS, local kinematics, animation, then actor-local stage-X constraints for an already-live P3-P8 root. Restricted CNS admits motion/state/Tag operations plus direct `HitDef`; every other side-effect route remains blocked. `RuntimeRootPhaseCapabilities/v5` exposes plural body push, hit admission, direct combat, unordered-pair typed HitDef priority arbitration, frame-batched equal Hit/Hit graph transactions, and frame-local no-contact suppression for Miss/Dodge ties. Direct native input/AI, Pause/hitpause motion, broad effect lifecycle, projectile/helper combat, exact ReversalDef/HitOverride/guard interaction, throw randomization, fully immutable secondary trade effects, round, HUD, and shared resources remain separate owners.

`RuntimeRootPresentationWorld` owns `RuntimeRootPresentation/v1`, a renderer-independent projection with stable per-root reasons and separate ordered body/shadow, camera-X, and collision-debug ids. `MugenSnapshot.actors` stays the exact playable/HUD/audio pair; reserves remain separate. `RuntimeRootBodyPushWorld` independently consumes stable runtime roots for bounded X/Width push before pair combat. `RuntimeRootDirectHitAdmissionWorld` then publishes a deterministic read-only enemy-pair projection from runtime roots; it has no mutation hooks and never consumes presentation/collision ids. Hit/guard/reversal mutation, exact Tag overlap, camera Y/zoom, HUD/audio, round, resources, and full presentation parity remain open.
...
`RuntimeStateTypeWorld` owns bounded StateTypeSet metadata setup that used to live as private `StateControllerExecutor` helpers: `stateType`, `moveType`, and `physics` writes from typed `metadata:statetypeset` operations, raw controller params, or the bounded active-state enum-expression fallback covered by `synthetic-imported-statetypeset-dynamic.json`. `StateControllerExecutor` still owns controller routing and broad runtime-controller execution, so this is not broad string-param parity, helper/team/redirect ownership, exact physics/tick-order interactions, or full StateTypeSet parity.
...
`RuntimeTeamRoundHandoffWorld` owns the bounded Turns promotion transaction after that read model: it preflights all outgoing/incoming roots, records ordered decision/preflight/commit phases, and atomically changes only `standby` and `overKo`. `PlayableMatchRuntime` and `MatchWorld` expose the decision and explicit handoff APIs while the pair snapshot, `RuntimeRoundSystem`, input, slot identity, life/resources, effects, presentation, and lifebars remain separate owners. The required `synthetic-imported-team-round-handoff` trace now drives a real lethal contact through the public runtime boundary and proves KO-before-promotion plus reserve activation. This is a team-state transaction boundary, not automatic KO scheduling, Ikemen slot/reference remapping, round continuation, or full team-round parity.

`RuntimeTeamRoundLifebarWorld` owns the bounded renderer-independent IKEMEN non-Single lifebar read model: stable per-side leader/member slots, plural active-root ids, explicit active/standby/KO/disabled state, finite life/max-life ratios, and `NoBarDisplay` visibility. `RuntimeSnapshotWorld` and `RuntimeTrace` preserve that diagnostic outside behavior checksums, while the Studio debug panel and main round HUD consume it without folding power or shared resources into the contract. Active-root selection, camera membership, and resource ownership remain separate contracts, so a Tag swap can change visible active ids without reordering or mutating team slot facts. This is a presentation-data boundary, not motif/AIR lifebar animation, red-life interpolation, power/stun/shared-resource behavior, automatic Turns continuation, exact Tag switching, or full MUGEN/IKEMEN HUD parity.
...
`RuntimeEffectActorAdvanceWorld` owns the bounded low-level effect-actor advance order inside `RuntimeEffectActorWorld`: active-effect passes tick Helpers before Projectiles, paused presentation ticks Helpers before Explods, and normal presentation ticks Explods without helper execution. `RuntimeEffectActorWorld` now registers exact actor-keyed stores for every authoritative root, preserves P1/P2 compatibility access, rejects unknown owners, and resets every unique registered store. The match and lifecycle worlds still own when active or paused presentation passes happen; root store ownership does not itself schedule P3-P8 effect advancement, presentation, or combat. This boundary isolates the store-local order so future helper/projectile/explod lifecycle work has one tested ownership point. This is current-behavior ownership, not exact helper pause/combat ordering, projectile lifetime parity, remove-trigger timing, visual/audio parity, or full Helper/Projectile VM parity.

`RuntimeMatchInteractionWorld` accepts a deterministic target-maintenance roster. Pair/Single defaults to P1/P2. Explicit Tag supplies stable unique valid roots, ages every target store once before active effects, clears stale binding-subject telemetry across all authoritative roots, then resolves all existing TargetBind passes before all BindToTarget passes against complete exact-id candidates. Disabled, non-player, invalid-id, and duplicate roots are excluded from resolution. `post-fighter:target-maintenance` exposes actor-scoped proof. This does not add hit-target acquisition, widen target controllers/combat, or alter pause/hitpause scheduling; direct-HitDef contact memory is the separate boundary below.

`RuntimeHitDefContactMemorySystem` owns actor-local committed and pending getter ids for non-projectile direct HitDefs. Live CNS target refs remain immediate and deduplicated in `RuntimeTargetWorld`; accepted direct hit/guard/HitOverride and direct ReversalDef append exact getter ids to pending contact memory. Plural priority also buffers the exact suppressed loser-to-winner getter pair, avoiding global `hasHit` suppression while another opponent remains eligible. `RuntimeMatchInteractionWorld` commits pending ids after direct combat and before Projectile combat. MoveStart, HitDef, and ReversalDef reset both lists. `RuntimeHitDefContactMemory/v0` is detached public evidence, and `post-fighter:hitdef-contact-commit` records P1-P8 schedule ownership. Pair actors without explicit lists preserve scalar compatibility; exact hitonce/juggle/ReversalDef and all priority-type parity remain unsupported.
...
`RuntimeAudioControllerDispatchWorld` owns the bounded active-state audio controller dispatch that used to live inline in `PlayableMatchRuntime`: controller telemetry, typed `audio:*` operation selection for static params, dynamic `PlaySnd value` raw fallback, dynamic numeric raw fallback for expression-backed params, operation telemetry, and `PlaySnd` / `SndPan` / `StopSnd` event handoff into `RuntimeAudioWorld`. Direct `HitDef hitsound` and `guardsound` dynamic group/index fallback plus live root/RedirectID and Helper `ModifyHitDef hitsound` / `guardsound` replacement are resolved in their caller context and handed to `RuntimeAudioWorld` through contact presentation. `RuntimeMatchPauseControllerWorld` can also emit bounded `SuperPause sound` telemetry through `RuntimeAudioWorld` when a pause starts. `PlayableMatchRuntime` still owns trigger filtering, active-state order, hit/contact timing, pause callback routing, and actor context, so this is ownership cleanup for current sound-event telemetry, not exact SND playback, channel priority, mixing, FightFX/common fallback, exact panning, super-background audio, or full audio parity.

`RuntimeEnvColorControllerDispatchWorld` owns the bounded active-state EnvColor controller dispatch that used to live inline in `PlayableMatchRuntime`: controller telemetry, static or resolved dynamic typed `envcolor` operation selection, operation telemetry, and event handoff into `RuntimeEnvColorWorld`. `PlayableMatchRuntime` still owns trigger filtering, active-state order, stage-world ownership, and pause/hitpause callback routing, so this is ownership cleanup for current stage-flash telemetry, not exact blend math, layer/window ordering, pause timing, renderer parity, helper/redirect ownership, or full presentation parity.
...
`RuntimeActorConstraintControllerDispatchWorld` owns the bounded active-state Width controller dispatch that used to live inline in `PlayableMatchRuntime`: controller telemetry, typed `collision:width` operation selection or runtime-resolved dynamic fallback telemetry for `player`/`value`, operation telemetry, and body-width mutation handoff into `RuntimeActorConstraintWorld`. `PlayableMatchRuntime` still owns trigger filtering, active-state order, per-frame constraint reset, stage clamp, and body-push ordering, so this is ownership cleanup for current Width telemetry/mutation, not exact player/edge collision, team/helper push behavior, screen-edge/camera parity, Width edge semantics, or full constraint VM parity. Passive `PlayerPush`/`PosFreeze`/`ScreenBound` setup now routes through `RuntimeBoundsControllerWorld`.
...
`RuntimeTeamResourceBankWorld` plus `RuntimeTeamResourceBankRuntime` own the bounded team resource contract layered above root mutations: independent life/power sharing policy, stable root-local or side bank ids, explicit `resourceOwnerId` bindings, current values/maxima, post-tick delta reconciliation, clamping, shared-root mirroring, and reset rebinding. Snapshot and trace consumers can identify the owner without using leader or active-root status as an implicit shortcut. The v1 mutation policy deliberately covers only root LifeShare/PowerShare paths; exact Set conflict ordering, helper/projectile ownership, red-life, guard/stun, variable maps, round persistence, rollback, netplay, and full team resource parity remain separate contracts.

Required route evidence now exercises the same boundary from imported Tag
`SuperPause.poweradd` and `TargetLifeAdd`/`TargetPowerAdd` controller paths.
The traces keep LifeShare and PowerShare independent and prove standby mirrors
only when the corresponding switch is enabled; this is integration evidence,
not an exact upstream mutation-order claim.
...
`RuntimeRoundSystem` owns the current bounded round timer, KO/time-over finish decision, winner selection, reset, and a resolved `RuntimeRoundTiming` contract whose defaults open phase `4` at post-round frame `45`, expire the shared post-round window at `255`, and drive the bounded KO-only slowdown/fade values. Partial timing overrides enter through `PlayableMatchRuntimeOptions.roundTiming`; imported `fight.def` `[Round]` values enter through `MugenSystemAssets` and imported fighter metadata when no explicit override exists; legacy default constants remain compatibility aliases. `RuntimeMatchRoundWorld` consumes the normalized global `RoundNotOver` projection and holds the phase-4 clock for KO or time-over without stopping playback until that assertion clears. `RuntimeRoundWinPose/v0` adds a bounded phase-4 reserved-state handoff for the active normal/tag pair when `180/170/175` is available, requests it only after the same resolved timing reaches the win-pose readiness boundary, and publishes explicit unavailable/ambiguous diagnostics; the time-over draw path can therefore reach state `175` when available. `RuntimeTraceGate.requiredRoundFrames` now lets required artifacts gate that snapshot evidence by round state, winner, message, frame count, and observed timer range; `synthetic-imported-round-ko.json` proves the bounded KO snapshot route after a lethal imported `HitDef`, and `synthetic-imported-round-timeover.json` proves a bounded time-over/draw route through a short `roundTimerFrames` fixture. The match loop still decides when combat/resources change life and when playback stops, so this is bounded runtime ownership plus evidence coverage, not exact MUGEN/IKEMEN release, lifebar, team, simul/tag, intro, Common1 winpose, or screenpack parity.

`RuntimeRoundTiming.forceWinTimeFrames` now carries imported `over.forcewintime` into a bounded phase-4 readiness hold. `PlayableMatchRuntime` supplies the active-root proxy; `RuntimeRoundSystem` freezes the wait-boundary frame and remaining post-round clock until readiness or force expiry. This is source-backed root timing evidence, not complete `intro`/`winwaittime` accounting, Common1/ZSS ownership, skip input, release fade/motif/dialogue, team choreography, or full round parity.

`RuntimeRoundTiming` also carries imported `fadeout.time`/`fadeout.col` as a bounded terminal presentation contract. T284 adds optional `fadeout.anim`/`fadeout.snd`, resolves FightFX AIR duration, publishes additive animation/sound metadata, renders the resolved AIR/SFF action through `RoundFadeRenderer`, and routes the global sound through FightFX with one-shot deduplication. Source-derived duration uses the maximum of `over.time`, `fadeout.time`, and resolved animation duration; unresolved assets preserve the color fallback. Exact localcoord/motif transforms, fade-in/release ordering, dialogue/skip ownership, and full release parity remain separate.

T285 adds the complementary reset-owned `RuntimePreRound/v0` boundary for imported `fadein.time`/`fadein.col`/`fadein.anim`/`fadein.snd`. The normalized runtime resets its pre-round frame for reset and next-round handoff, derives duration from color or FightFX AIR length, and reuses `RoundFadeRenderer` plus the global FightFX audio route. Resolved animation suppresses the color contribution; unresolved assets retain reverse-opacity fallback diagnostics. Exact intro/shutter/frame-start ordering, timer/input gating, motif/localcoord, dialogue/skip, release choreography, and full parity remain separate.

`RuntimeRoundSystem.roundNoDamage` is the bounded source-mapped close read model for `over.hittime`: `PlayableMatchRuntime` forwards it through `RuntimeMatchPostFighterWorld` into `RuntimeMatchInteractionWorld`, which skips late HitDef/reversal/priority/projectile/helper admission and HitDef target commits while preserving target/effect maintenance, body push, bindings, clamps, and presentation. T280 also forwards the same internal bit through `RuntimeControllerDispatchWorld` for imported player/root active and State -1 resource controllers, where `StateControllerExecutor` suppresses bounded life/red-life/guard/dizzy/power writes while preserving telemetry and control writes. The public `RuntimePostRound/v0` snapshot remains unchanged. Helper/Target resource ownership, source enable flags, exact frame-start ordering, release choreography, and full MUGEN/IKEMEN round parity remain open.
...
30c. `RuntimeTeamRoundHandoffWorld`: own bounded preflight and atomic Turns `standby/overKo` promotion outside pair-owned round, input, and presentation mutation.
30d. Required team-round trace evidence: keep lethal contact, ordered handoff phases, and active-to-standby/reserve-to-active frame transitions in the aggregate compatibility gate without widening pair-round ownership.
...
`EnvShakeSystem` sits on the runtime side of that boundary. It converts `EnvShake` and `FallEnvShake` into bounded runtime events, records typed `fallenvshake` evidence for compiled fall-shake controllers, resolves dynamic EnvShake params into typed `envshake` evidence, and resolves the camera shake vector; Three.js only applies the snapshot values. `RuntimeEnvColorWorld` follows the same boundary for `EnvColor`: it uses `EnvColorSystem` helpers to convert static or resolved dynamic `value`/`time`/`under` params into bounded stage-flash snapshots plus typed `envcolor` evidence, and Three.js only renders the overlay. Exact stage layer/window/blend parity remains future work.

`RuntimeSpriteEffectWorld` / `RuntimeSpriteEffectControllerWorld` / `SpriteEffectSystem` also sit on the runtime side of that boundary. They resolve legacy visual-controller params and active-state sprite-effect controller dispatch into actor snapshot state; Three.js should render those state values without parsing CNS. Current `AngleDraw` support rotates the rendered sprite quad from bounded `renderAngle` telemetry, current `SprPriority`, `PalFX`, `AfterImageTime`, `RemapPal`, and `Trans alpha` can resolve bounded active dynamic params into typed sprite-effect telemetry, current `AfterImage` can resolve bounded active dynamic params into snapshot telemetry, and current `AfterImage` support exposes bounded trail telemetry. Exact MUGEN/IKEMEN axis pivot, collision-box rotation, draw-order interaction, palette/remap interaction, trail cadence, material math, exact Trans add/sub alpha math, broad dynamic expression parity, and helper/redirect visual ownership remain future work.
...
T706 extends that boundary with a bounded helper-local live `ModifyProjectile`
`projanim` path: the typed expression resolves once in the Helper caller
context, only helper-parented Projectiles are selected, AIR playback resets,
and Helper/Projectile/ModifyProjectile telemetry plus spawn/active/remove
lifecycle evidence are retained. Cross-namespace broadcast, FFX prefixes,
terminal timing, nested teams, rollback, and full Helper/Projectile parity
remain outside the boundary.

`ProjectileSystem` produces effect actor snapshots and projectile hitbox projection from already-resolved owner/action/position data, including explicit `projectile` actor identity, typed `projectile` operation evidence for compiled imported controllers, bounded `projhits`/`projmisstime` state, bounded `accel` movement, bounded `velmul` velocity multiplier movement, bounded static `projscale` render scale, bounded owner-side `ModifyProjectile` mutation of live projectiles through typed `modifyprojectile` evidence, bounded owner-side and helper-local dynamic `ModifyProjectile` removal-bound expressions for `projedgebound` / `projstagebound` / `projheightbound`, bounded `NumProj` / `NumProjID(id)` count reads over the current owner store, and bounded terminal playback for resolved `projhitanim`/`projremanim`/`projcancelanim` AIR actions. `ExplodSystem` likewise exposes bounded `NumExplod(id)` count reads over the current owner store for visual effect actors. `RuntimeProjectileCombatWorld` consumes those projected hitboxes plus defender/projectile boxes through `RuntimeEffectActorWorld` and applies the bounded contact/reject/HitOverride/hit-or-guard/removal loop, bounded single-target re-contact cooldown, bounded `ProjContact`/`ProjHit`/`ProjGuarded(projid)` owner trigger memory, plus bounded equal-priority trade and higher-priority `projpriority` cancel with winner-priority decrement through the shared partial combat resolver. These systems do not yet implement dynamic non-bound `ModifyProjectile` params, exact `ModifyProjectile` tick-order/selection parity, exact projectile priority classes, exact projectile count/lifetime parity, exact contact-trigger timing/lifetime, exact multi-target projectile behavior, exact terminal timing/rem-trigger parity, exact `velmul` tick-order parity, scaled collision parity, full guard-specific effects/timing, helper-owned projectile rules, or exact HitDef inheritance.
...
`RuntimeAudioWorld` sits on the runtime side of that boundary. It uses `AudioEventSystem` helpers to convert legacy controllers, bounded dynamic `PlaySnd value`, bounded dynamic numeric audio params, and bounded direct `HitDef` `hitsound`/`guardsound` refs into `RuntimeSoundEvent` snapshots and leaves decoding, channel nodes, browser unlock, and playback to `MugenAudioSystem`.
