...
7. `RuntimeSpriteEffectWorld` / `RuntimeSpriteEffectControllerWorld` / `SpriteEffectSystem`: own match-runtime `SprPriority`, `PalFX`, `Trans`, `AngleSet`/`AngleAdd`/`AngleDraw`, `AfterImage`, and `AfterImageTime` dispatch/mutation/ticking, while Three.js applies snapshot material, trail, opacity, and rotation changes.
