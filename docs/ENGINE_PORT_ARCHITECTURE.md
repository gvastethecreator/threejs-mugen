...
`RuntimeRootAdvancePhaseWorld` owns immutable explicit-IKEMEN normal-tick classification into `playable`, `active-motion`, and `bounded-standby`. `RuntimeRootMotionAdvanceWorld` executes state clock, side-effect-free motion CNS, local kinematics, animation, then actor-local stage-X constraints for an already-live P3-P8 root. `RuntimeRootPhaseCapabilities/v2` exposes the same boundary. Direct native input/AI, Pause/hitpause motion, sprite/effect lifecycle, hit/contact/recovery, plural push, collision-debug admission, combat, round, audio/HUD, and resources remain separate owners.

`RuntimeRootPresentationWorld` owns `RuntimeRootPresentation/v1`, a renderer-independent projection with stable per-root reasons and separate ordered body/shadow, camera-X, and collision-debug ids. `MugenSnapshot.actors` stays the exact playable/HUD/audio pair; reserves remain separate. `RuntimeMatchPresentationSnapshotWorld` consumes selected camera roots but keeps EnvShake/effects pair-owned, while `ThreeMugenRenderer` strictly resolves draw and diagnostic collision roots across pair/reserve storage. This closes bounded visible and Clsn-debug active-root handoff, not plural push, hit admission/combat, exact Tag overlap, camera Y/zoom, HUD/audio, round, resources, or full presentation parity.
