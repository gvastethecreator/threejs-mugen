...
8. `RuntimeDirectCombatWorld` / `DirectCombatSystem`: own bounded direct hit/guard result mutation outside the match runtime while collision, target routing, and Common1/custom-state transitions stay explicit integration points.
