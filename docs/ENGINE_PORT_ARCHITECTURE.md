...
`RuntimeTeamResourceBankWorld` owns the bounded team resource-owner projection layered above those actor-local mutations: independent life/power sharing policy, stable root-local or side bank ids, and explicit `resourceOwnerId` bindings. Snapshot and trace consumers can identify the owner without using leader or active-root status as an implicit shortcut. Mutation routing, reset, Pause/SuperPause, target/helper redirects, shared values, and full team resource parity remain separate follow-up contracts.
