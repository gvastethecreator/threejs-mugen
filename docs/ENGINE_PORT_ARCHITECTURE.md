...
`RuntimeHitDefContactMemorySystem` owns actor-local committed and pending getter ids for non-projectile direct HitDefs. Live CNS target refs remain immediate and deduplicated in `RuntimeTargetWorld`; accepted direct hit/guard/HitOverride and direct ReversalDef append exact getter ids to pending contact memory. `RuntimeMatchInteractionWorld` commits pending ids after both direct-combat passes and before Projectile combat, preserving source order and duplicates. MoveStart, HitDef, and ReversalDef reset both lists. `RuntimeHitDefContactMemory/v0` is detached public evidence, and `post-fighter:hitdef-contact-commit` records P1-P8 schedule ownership. Root admission consumes exact committed/pending getter identity, but pair mutation still uses scalar `hasHit`; this cut does not grant active-root mutation or exact hitonce/juggle/priority/reversal parity.
