...
`synthetic-imported-projectile-motion.json` adds the current Projectile visual-motion contract: compiled/raw `accel` is stored on projectile actors and applied during presentation advance, and compiled/raw `projscale` reaches effect payload plus `renderScale` snapshots for Three.js projection. This keeps bounded projectile motion/scale in `ProjectileSystem` / `RuntimeEffectActorWorld`; exact `velmul`, scaled collision parity, and contact/removal tick parity remain outside this cut.
