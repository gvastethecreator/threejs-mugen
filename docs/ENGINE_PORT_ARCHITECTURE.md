...
`synthetic-imported-explod-pausemovetime.json` and `synthetic-imported-explod-supermovetime.json` add the first Explod-owned pause-budget contracts: compiled/raw `pausemovetime` and `supermovetime` are stored on visual Explod actors, paused presentation passes can advance those actors after the source actor's `movetime` expires, and sibling Explods without those budgets remain frozen. This keeps pause-budget logic in `ExplodSystem` / `RuntimeEffectActorWorld` instead of teaching Three.js or generic presentation code about CNS params.
