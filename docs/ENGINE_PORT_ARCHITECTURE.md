...
`RuntimeRootAdvancePhaseWorld` owns immutable explicit-IKEMEN normal-tick classification into `playable`, `active-motion`, and `bounded-standby`. `RuntimeRootMotionAdvanceWorld` executes state clock, side-effect-free motion CNS, local kinematics, animation, then actor-local stage-X constraints for an already-live P3-P8 root. `RuntimeRootPhaseCapabilities/v3` additionally exposes bounded plural body-push admission. Direct native input/AI, Pause/hitpause motion, sprite/effect lifecycle, hit/contact/recovery, combat, round, audio/HUD, and resources remain separate owners.

`RuntimeRootPresentationWorld` owns `RuntimeRootPresentation/v1`, a renderer-independent projection with stable per-root reasons and separate ordered body/shadow, camera-X, and collision-debug ids. `MugenSnapshot.actors` stays the exact playable/HUD/audio pair; reserves remain separate. `RuntimeRootBodyPushWorld` independently consumes stable runtime roots for bounded X/Width push before pair combat; presentation/collision ids never become simulation authority. Hit admission/combat, exact Tag overlap, camera Y/zoom, HUD/audio, round, resources, and full presentation parity remain open.
