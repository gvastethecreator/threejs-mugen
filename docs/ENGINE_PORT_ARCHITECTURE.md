...
`RuntimeRoundTiming` also carries imported `fadeout.time`/`fadeout.col` as a bounded terminal presentation contract. Source-derived duration uses `max(over.time, fadeout.time)`, `RuntimeRoundSystem` publishes additive `RuntimeRoundFade/v0`, and `ThreeMugenRenderer` owns the final color/opacity overlay. Fight-screen animation/sound assets, motif/dialogue/skip ownership, exact frame-start order, and full release parity remain separate.
