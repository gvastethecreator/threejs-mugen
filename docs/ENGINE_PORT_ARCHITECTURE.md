...
`RuntimeExpressionContextWorld` owns the bounded active runtime expression/trigger context that used to be duplicated inside `PlayableMatchRuntime`: target redirects, contact/projectile reads, direct hit-count/readback, received-damage/readback, effect actor counts, command lookups, const/state/anim/hitvar reads, `HitDefAttr`, hitpause/hitover reads, and `InGuardDist` wiring. `PlayableMatchRuntime` still supplies actor/opponent/owner inputs, trigger grouping/order, next-random callbacks, animation timing callbacks, and concrete controller dispatch, so this is read-model ownership cleanup rather than full expression language or exact VM timing parity.
