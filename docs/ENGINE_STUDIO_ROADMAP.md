...
UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The latest Studio CSS hygiene pass split legacy/editor/hardening/runtime command-room/shell/command-center/desktop authority into smaller import-ordered modules while keeping active command shell ownership in `src/styles/studio-command-shell.css`, `src/styles/studio-command-pipeline.css`, `src/styles/studio-command-playfield.css`, and `src/styles/studio-command-console.css`, Build/Evidence ownership in `src/styles/studio-trust-ledgers.css`, and Modules/Debug ownership in `src/styles/studio-system-ledgers.css`; `pnpm qa:css` reports 2,626 rules, zero exact duplicate CSS rules, zero same-file duplicate selector keys, zero selectors shared with legacy `src/style.css`, zero fully shadowed cross-file rules, 168 repeated declaration groups, and 135 cross-file overlaps still to collapse through primitives. The S1 Trust Chain cut now makes Build and Evidence read the same Build Readiness contract for runtime manifest, QA evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries, with `pnpm qa:smoke` asserting the shared row ids and next-action binding. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.
