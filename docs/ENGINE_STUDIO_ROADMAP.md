...
UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The latest Studio CSS hygiene pass routes the ordered Studio cascade through `src/styles/studio.css` and category entrypoints, with active command shell ownership in `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css`, Build/Evidence ownership in `src/styles/workflows/studio-trust-ledgers.css`, Modules/Debug ownership in `src/styles/workflows/studio-system-ledgers.css`, reusable ledge/header primitive ownership in `src/styles/surfaces/studio-primitives.css`, reusable drawer ownership in `src/styles/surfaces/studio-ledger-drawers.css`, shared status-rail ownership in `src/styles/surfaces/studio-status-rails.css`, Assets ledger row ownership in `src/styles/workflows/studio-assets-ledger.css`, and Assets dependency graph ownership in `src/styles/workflows/studio-assets-graph.css`; `pnpm qa:css` reports 535,843 active CSS bytes, 2,353 rules, zero exact duplicate CSS rules, zero same-file duplicate selector keys, zero selectors shared with legacy `src/style.css`, zero fully shadowed cross-file rules, 117 repeated declaration groups, and 107 cross-file overlaps still to collapse through primitives. The latest desktop command polish gives the command palette and bottom console larger readable rows, clearer source/severity columns, hard neutral surfaces, focused scroll affordances, and exact-property focus transitions while preserving `pnpm qa:smoke` command-palette keyboard/focus checks; command-palette result/status/focus colors, Stage Lab diagnostic colors, shared shell badge/control/stage-status colors, Studio topbar/workstation/playfield plus command playfield control colors, command shell chrome/actions/player markers, desktop foundation badge/status/stage-status surfaces, and Runtime/Inspect command-room backgrounds now use palette/global tokens instead of one-off literal declarations, and Studio KV plus non-status command/ledger labels preserve hierarchy without forced uppercase body metadata. The latest label/readability pass lets Workbench, Build/Evidence, Modules/Debug, and Inspector descriptive copy wrap instead of forcing single-line truncation, while Workbench/Debug empty-state chrome and runtime command-deck line-height declarations now share fewer rules. The Assets right pane now uses one `studio-ledger-drawer` helper/class for repeated collapsible panels instead of parallel summary/body markup, Assets ledger/graph selectors use scoped `:is(...)` groups where safe, Assets action icons/primary states reuse existing Studio action primitives, Build/Evidence right-rail headers reuse the shared Studio primitive header selectors, desktop workbench/list/engine-contract/drawer truncation now routes through shared Studio primitives, Build/Evidence action labels now share one compact action clamp, Studio desktop shell truncation now shares one workstation selector group, left-pane kicker/eyebrow styling now shares one desktop foundation group, legacy Studio truncation/text-wrap rows plus small grid/alignment/text atoms share grouped CSS declarations, retired pseudo indicator rules no longer survive as hidden overrides, life/power meter colors and mission/mode status dots share grouped rules, shared row status rails are extracted into `src/styles/surfaces/studio-status-rails.css`, the CSS budget now freezes bytes as well as rule/overlap counts, the neutral legacy pass is removed, global responsive shell rules moved from the responsive legacy pass into `src/styles/base/app-shell.css`, unused legacy command/health/readiness selectors have been removed after source search, and the latest structural prune removes unused Build/Evidence list, old asset focus/flow, trace scrubber, stat-card, and build-export-console hooks. The S1 Trust Chain cut makes Build and Evidence read the same Build Readiness contract for runtime manifest, QA evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries, with `pnpm qa:smoke` asserting shared row ids, target metadata, deltas, and next-action binding. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.

Current S1 UI checkpoint: Trust Chain jumps now leave a visible focused row and the QA bridge exposes `studioFocusedTrustRowId` so smoke can prove the linked package/source/trace/gate row remains visible after navigation. Build/Evidence and Modules/Debug also share one ledger key-value/badge primitive through `src/styles/surfaces/studio-primitives.css`; Studio command, shell, desktop, workflow, shared primitive, and responsive Assets CSS keep visible text at a 12px floor. Stage layer diagnostics separate metadata and badges so imported-stage layer rows stay readable at that floor. The base motion token, base controls, desktop command rows, Trust ledgers, and System ledgers now carry local reduced-motion guards. The current responsive/color/readability consolidation removes a redundant Assets drawer summary clamp already covered by the shared drawer primitive, combines Build/Evidence action-label truncation into one rule, routes command-palette result/status/focus colors plus Stage Lab diagnostics through tokens, keeps Studio KV and non-status command/ledger labels readable without forced uppercase body metadata, routes shared shell badges/controls/stage-status colors, Studio shell/playfield chrome, command shell chrome/actions/player markers, command playfield tools, desktop foundation badge/status/stage-status surfaces, and Runtime/Inspect command-room surfaces through existing tokens, merges duplicate Workbench/Debug empty-state chrome plus runtime command-deck line-height declarations, lets Studio action labels wrap instead of forcing single-line truncation, and lets Trust/System ledger copy plus Inspector list/controller text wrap where it carries descriptive evidence rather than compact IDs. Current `pnpm qa:css` reports 535,843 bytes, 2,353 rules, 117 repeated declaration groups, and 107 cross-file overlaps under the frozen 536,051-byte / 2,364-rule / 119 repeated-group / 108-overlap budget; the static interface detector tiny-text, missing-reduced-motion, and all-caps body buckets are now 0, total findings are down from 290 to 155, nowrap-risk is down to 80, and hardcoded-color drift is down to 0.
...
2. Make Assets, Evidence, and Build denser and more truthful before broad editing. Current visual checkpoints cover compact Studio navigator/mission rows, dense Assets ledger rows, Evidence audit ledgers, Build/Modules command ledgers, Debug system ledgers, the shared Studio command-desk status strip with live Pause/HitPause data, and a CSS hygiene pass that keeps command actions visible while eliminating exact duplicate CSS rules. These remain backed by real QA bridge/project/runtime data.
...
2. R1: expand KFM/Common1 gates beyond default `5000` entry, stand progression, guard-hit routing, fall branch, bounded synthetic recovery, bounded synthetic recovery input, bounded synthetic recovery-threshold handoff, bounded synthetic recovery actor-frame tick order, bounded synthetic air-recovery velocity, bounded synthetic ground-recovery selection/velocity, official KFM ground-impact/bounce/lie-down entry, official KFM lie-down/get-up, official KFM air recovery-input, official KFM ground recovery-input, optional official KFM ordered threshold evidence, and early-recovery reject evidence into exact threshold tables/velocity math, exact controller/VM tick-order parity, broader recovery parity, and exact guard-rule parity.
3. S1: deepen the shared Build/Evidence Trust Chain from generic next actions into target-specific trace, asset, source-package, and package-file row jumps.
