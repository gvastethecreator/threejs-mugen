...
UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The latest Studio CSS hygiene pass routes the ordered Studio cascade through `src/styles/studio.css` and category entrypoints, with active command shell ownership in `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css`, Build/Evidence ownership in `src/styles/workflows/studio-trust-ledgers.css`, Modules/Debug ownership in `src/styles/workflows/studio-system-ledgers.css`, reusable drawer ownership in `src/styles/surfaces/studio-ledger-drawers.css`, and Assets ledger row ownership in `src/styles/workflows/studio-assets-ledger.css`; `pnpm qa:css` reports 2,594 rules, zero exact duplicate CSS rules, zero same-file duplicate selector keys, zero selectors shared with legacy `src/style.css`, zero fully shadowed cross-file rules, 159 repeated declaration groups, and 123 cross-file overlaps still to collapse through primitives. The latest desktop command polish gives the command palette and bottom console larger readable rows, clearer source/severity columns, hard neutral surfaces, focused scroll affordances, and exact-property focus transitions while preserving `pnpm qa:smoke` command-palette keyboard/focus checks. The Assets right pane now uses one `studio-ledger-drawer` helper/class for repeated collapsible panels instead of parallel summary/body markup, legacy Studio truncation/text-wrap rows share grouped CSS atoms, and redundant legacy responsive/shell overrides have been pruned when later authoritative layers own the same selectors. The S1 Trust Chain cut makes Build and Evidence read the same Build Readiness contract for runtime manifest, QA evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries, with `pnpm qa:smoke` asserting shared row ids, target metadata, deltas, and next-action binding. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.
