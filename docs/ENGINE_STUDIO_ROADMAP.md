...
UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The latest Studio CSS hygiene pass routes the ordered Studio cascade through `src/styles/studio.css` and category entrypoints, with active command shell ownership in `src/styles/command/studio-command-shell.css`, `src/styles/command/studio-command-pipeline.css`, `src/styles/command/studio-command-playfield.css`, and `src/styles/command/studio-command-console.css`, Build/Evidence ownership in `src/styles/workflows/studio-trust-ledgers.css`, Modules/Debug ownership in `src/styles/workflows/studio-system-ledgers.css`, reusable ledge/header primitive ownership in `src/styles/surfaces/studio-primitives.css`, reusable drawer ownership in `src/styles/surfaces/studio-ledger-drawers.css`, shared status-rail ownership in `src/styles/surfaces/studio-status-rails.css`, Assets ledger row ownership in `src/styles/workflows/studio-assets-ledger.css`, and Assets dependency graph ownership in `src/styles/workflows/studio-assets-graph.css`; `pnpm qa:css` reports 536,876 active CSS bytes, 2,368 rules, zero exact duplicate CSS rules, zero same-file duplicate selector keys, zero selectors shared with legacy `src/style.css`, zero fully shadowed cross-file rules, 119 repeated declaration groups, and 108 cross-file overlaps still to collapse through primitives. The latest desktop command polish gives the command palette and bottom console larger readable rows, clearer source/severity columns, hard neutral surfaces, focused scroll affordances, and exact-property focus transitions while preserving `pnpm qa:smoke` command-palette keyboard/focus checks. The Assets right pane now uses one `studio-ledger-drawer` helper/class for repeated collapsible panels instead of parallel summary/body markup, Assets ledger/graph selectors use scoped `:is(...)` groups where safe, Assets action icons/primary states reuse existing Studio action primitives, Build/Evidence right-rail headers reuse the shared Studio primitive header selectors, desktop workbench/list/engine-contract/drawer truncation now routes through shared Studio primitives, Studio desktop shell truncation now shares one workstation selector group, left-pane kicker/eyebrow styling now shares one desktop foundation group, legacy Studio truncation/text-wrap rows plus small grid/alignment/text atoms share grouped CSS declarations, retired pseudo indicator rules no longer survive as hidden overrides, life/power meter colors and mission/mode status dots share grouped rules, shared row status rails are extracted into `src/styles/surfaces/studio-status-rails.css`, the CSS budget now freezes bytes as well as rule/overlap counts, the neutral legacy pass is removed, global responsive shell rules moved from the responsive legacy pass into `src/styles/base/app-shell.css`, unused legacy command/health/readiness selectors have been removed after source search, and the latest structural prune removes unused Build/Evidence list, old asset focus/flow, trace scrubber, stat-card, and build-export-console hooks. The S1 Trust Chain cut makes Build and Evidence read the same Build Readiness contract for runtime manifest, QA evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries, with `pnpm qa:smoke` asserting shared row ids, target metadata, deltas, and next-action binding. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.
