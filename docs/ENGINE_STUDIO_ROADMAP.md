...
UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The latest Studio CSS hygiene pass split legacy/editor/hardening/runtime command-room/desktop authority into smaller import-ordered modules while keeping command-center authority in `src/styles/studio-command-center.css`, Build/Evidence ownership in `src/styles/studio-trust-ledgers.css`, and Modules/Debug ownership in `src/styles/studio-system-ledgers.css`; `pnpm qa:css` reports zero exact duplicate CSS rules, zero same-file duplicate selector keys, and zero selectors shared with legacy `src/style.css`. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.
