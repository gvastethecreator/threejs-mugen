...
2. Make Assets, Evidence, and Build denser and more truthful before broad editing. Current visual checkpoints cover compact Studio navigator/mission rows, dense Assets ledger rows, Evidence audit ledgers, Build/Modules command ledgers, and the shared Studio command-desk status strip with live Pause/HitPause data, all still backed by real QA bridge/project/runtime data.
