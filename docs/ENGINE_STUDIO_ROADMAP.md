...
Source-editing rule: the first writable Studio source surface is intentionally
bounded. `StudioSemanticDraft/v0` parses and compiles one CNS/ST document in
memory, binds it to the active source fingerprint/project revision, and keeps
the writable stream closed for invalid or stale drafts. The existing folder
write/reimport transaction is allowed only after a fresh permission and folder
fingerprint check; ZIP rewrite, create/delete, watch/merge, post-close rollback,
and broad structured editors stay blocked until their own evidence contracts
exist.

UX acceptance rule: every Studio surface must expose one primary next action, every `partial`/`warn`/`blocked` state must explain cause and repair path, and every `ok` state must link to evidence. The current visual audit found the shell broadly usable with no horizontal overflow, but flagged repeated status/action patterns across Assets, Evidence, Build, Debug, and Modules as the main coherence risk. The Studio CSS cascade routes through `src/styles/studio.css` and category entrypoints, with command shell ownership in `src/styles/command/*`, Build/Evidence ownership in `src/styles/workflows/studio-trust-ledgers.css`, Modules/Debug ownership in `src/styles/workflows/studio-system-ledgers.css`, reusable ledge/header primitive ownership in `src/styles/surfaces/studio-primitives.css`, reusable drawer ownership in `src/styles/surfaces/studio-ledger-drawers.css`, shared status-rail ownership in `src/styles/surfaces/studio-status-rails.css`, Assets ledger row ownership in `src/styles/workflows/studio-assets-ledger.css`, and Assets dependency graph ownership in `src/styles/workflows/studio-assets-graph.css`. Current `pnpm qa:css` passes as an audit at 581,084 active CSS bytes, 2,576 rules, 1 duplicate selector key, 0 exact duplicate rules, 126 repeated declaration groups, 155 cross-file overlaps, and 17 fully shadowed cross-file rules; `pnpm qa:css:budget` is currently red against older ceilings, so CSS budget recovery remains a separate cleanup task. The S1 Trust Chain cut makes Build and Evidence read the same Build Readiness contract for runtime manifest, QA evidence, project package, asset validation, source packages, compatibility gates, and architecture boundaries; package/source rows now resolve to concrete `package-file` export entries and `source-file` required paths with visible destination focus. Broader cross-file primitive extraction remains a product-system task, not a finished cleanup.

Current S1 UI checkpoint: Trust Chain jumps now leave a visible focused row and the QA bridge exposes `studioFocusedTrustRowId`, `studioFocusedPackageFilePath`, `studioFocusedSourcePackageId`, and `studioFocusedSourcePath` so smoke can prove package/source destination rows remain visible after navigation. Build/Evidence and Modules/Debug also share one ledger key-value/badge primitive through `src/styles/surfaces/studio-primitives.css`; Studio command, shell, desktop, workflow, shared primitive, and responsive Assets CSS keep visible text at a 12px floor. Current `pnpm qa:css` reports 581,084 bytes, 2,576 rules, 126 repeated declaration groups, and 155 cross-file overlaps; `pnpm qa:css:budget` is red against older ceilings and should be handled as cleanup work, not as a green S1 gate.
...
3. S1: deepen the shared Build/Evidence Trust Chain from package/source file jumps into target-specific trace, asset, gate, and report row jumps.
4. Use the source identity fingerprint/relink gate as the contract boundary, then expand the visible relink flow with persisted File System Access/IndexedDB source handles for reopened project manifests where the browser allows it.
