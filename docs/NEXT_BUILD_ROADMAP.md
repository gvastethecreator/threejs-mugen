...
R2 helper direct default target memory gate
  -> synthetic-imported-helper-default-target.json checksum e1bcced0 is now a required qa:trace artifact
  ...
  -> current qa:trace aggregate is 192/192 artifacts, 172 required and 20 optional
  ...
  -> this narrows explicit/default helper HitDef plus helper-parented Projectile explicit/default target memory evidence only; no helper Target* mutation controllers, helper custom-state targets, throws, teams/simul, multi-target/helper-owned opponent selection, exact helper hitpause/tick order, exact helper HitDef/Projectile lifetime parity, visual parity, score movement, or full helper target/combat/projectile parity claim
...
R1 presentation spark-offset requirement gate
  ...
  -> affected behavior checksums remained stable at that checkpoint; current qa:trace aggregate after the helper direct default-target gate is 192/192 artifacts, 172 required and 20 optional
  -> this narrows trace precision only; no exact renderer binding/timing/layering/scale/palette, SND playback, helper-owned presentation ownership, score movement, or full presentation parity claim
