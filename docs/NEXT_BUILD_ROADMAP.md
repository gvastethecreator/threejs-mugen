...
```txt
R2 helper Projectile default target memory gate
  -> synthetic-imported-helper-projectile-default-target.json checksum b1541afc is now a required qa:trace artifact
  -> helper-local state 1200 spawns helper-parented owner-side Projectile with no projid/id, hits P2 for 18 damage, mirrors target memory into the spawning Helper with target id 0, exposes target links p1 -> p2 / 0 and p1-helper-0 -> p2 / 0, and branches to 1227/958 through NumTarget(0) plus Target(0), Life
  -> current qa:trace aggregate is 191/191 artifacts, 171 required and 20 optional
  ...
  -> this narrows explicit-id helper HitDef plus helper-parented Projectile explicit/default target memory evidence only; no helper Target* mutation controllers, helper custom-state targets, throws, teams/simul, multi-target/helper-owned opponent selection, exact helper hitpause/tick order, exact helper HitDef/Projectile lifetime parity, visual parity, score movement, or full helper target/combat/projectile parity claim
...
R1 presentation spark-offset requirement gate
  ...
  -> affected behavior checksums remained stable at that checkpoint; current qa:trace aggregate after the helper Projectile default-target gate is 191/191 artifacts, 171 required and 20 optional
  -> this narrows trace precision only; no exact renderer binding/timing/layering/scale/palette, SND playback, helper-owned presentation ownership, score movement, or full presentation parity claim
...
R2 helper-local ProjContactTime trace gate
  ...
  -> this narrows helper-local generic projectile-contact trigger evidence only; no helper-owned Projectile combat/contact presentation, helper Target* mutation parity, exact ProjContact/ProjHit/ProjGuarded tick order or lifetime, exact projectile namespaces/scopes, dynamic ids/params, teams, score movement, or full parity claim
...
R2 helper-local micro-VM ownership
  ...
  -> no indexed redirects, EnemyNear(index), player-state BindToParent/BindToRoot, dynamic bind params, team/keyctrl ownership, exact helper resource scopes, helper-owned opponents, helper Target* mutation/multi-target parity, helper custom-state targets, helper fvar/sysvar VarRandom, exact random stream parity, exact helper-local sound timing/channel/redirect ownership, helper-owned Projectile combat/contact beyond bounded target memory, helper-owned effect namespaces, exact helper HitDef lifetime/multi-hit parity, full helper combat parity, nested helper ancestry, exact tick-order/pause parity, full custom-state helper lifecycle, or score movement claim
R2 visual-helper removal ownership
  ...
  -> no redirects, parent/root/team ownership, helper Target* mutation/multi-target parity, helper custom-state targets, helper-owned Projectile combat/contact beyond bounded target memory, exact helper HitDef lifetime/multi-hit parity, exact lifecycle tick-order parity, or score movement claim
R1 required combined hit/guard-effect contact-package trace strengthening
  ...
  -> current aggregate after the helper Projectile default-target gate is 191/191 artifacts, 171 required and 20 optional
