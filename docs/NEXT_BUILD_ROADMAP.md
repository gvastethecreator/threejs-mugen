...
```txt
R1 required dynamic PalFX typed sprite-effect trace gate
  ...
  -> actor-frame/final evidence requires paletteFx time 12, add [64,-16,255], mul [224,144,256], color 200, invert true
  -> pnpm qa:trace passes 523/523 artifacts, 492 required and 31 optional
  -> official Elecbyte docs allow numeric controller params as expressions and define PalFX time/add/mul/invertall/color semantics
  -> no score movement; sinadd, exact palette math/blend/remap order, ACT/SFF pixel parity beyond existing bounded handoff, renderer parity, helper/redirect ownership, and full presentation VM parity remain blocked
