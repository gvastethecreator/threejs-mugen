...
physics = N
anim = 0
[State -4, IKEMEN helper pause-safe global state]
type = VarAdd
trigger1 = 1
v = 0
value = 4
...
`,
  ...
  it("routes active Projectile RedirectID into the destination-owned effect store", () => {
    const effectActorWorld = new RuntimeEffectActorWorld();
    const caller = createImportedFixture({
      id: "redirected-projectile-caller",
      withStateMove: false,
      passiveResourceController: `
[State 0, Redirected Projectile]
type = Projectile
trigger1 = 1
projid = 81
projanim = 910
...
`,
    ...
    const target = createImportedFixture({
      id: "redirected-projectile-target",
      withStateMove: false,
    });
    const runtime = new PlayableMatchRuntime(caller, target, trainingStage, {
      runtimeProfile: "ikemen-go",
    ...
    const snapshot = runtime.step({ p1: new Set(), p2: new Set() });
    const projectile = effectActorWorld.projectiles("p2")[0];
    ...
    expect(projectile?.pos.x).toBeCloseTo((snapshot.actors[1]?.runtime.pos.x ?? 0) - 10);
    expect(snapshot.logs.some((line) => line.includes("Blocked Projectile RedirectID"))).toBe(false);
    expect(snapshot.compatibilitySession?.actors.some((actor) => actor.executedControllers.Projectile)).toBe(true);
  });

  it("fails closed for invalid active Projectile RedirectID", () => {
    const effectActorWorld = new RuntimeEffectActorWorld();
    const caller = createImportedFixture({
      id: "invalid-projectile-redirect",
      withStateMove: false,
      passiveResourceController: `
[State 0, Invalid Projectile Redirect]
type = Projectile
trigger1 = 1
projid = 81
projanim = 910
...
`,
    ...
    const target = createImportedFixture({ id: "invalid-projectile-target", withStateMove: false });
    const runtime = new PlayableMatchRuntime(caller, target, trainingStage, {
      runtimeProfile: "ikemen-go",
      effectActorWorld,
    });
    ...
    expect(effectActorWorld.projectiles("p1")).toEqual([]);
    expect(effectActorWorld.projectiles("p2")).toEqual([]);
    expect(snapshot.logs).toContain("Blocked projectile RedirectID 999 for p1");
    expect(snapshot.compatibilitySession?.actors.some((actor) => actor.executedControllers.Projectile)).toBe(false);
  });
...
`,
  ...
  it("uses imported fight-screen timing when no runtime override is supplied", () => {
    const attacker = createImportedFixture({
      id: "imported-timing-attacker",
      displayName: "Imported Timing Attacker",
      withStateMove: false,
      hitDefDamage: 2000,
      extraStateNos: [170, 180],
    });
    const defender = createImportedFixture({
      id: "imported-timing-defender",
      displayName: "Imported Timing Defender",
      withStateMove: false,
      hitDefDamage: 0,
      extraStateNos: [170, 180],
    });
...
`).states[0]!;
  ...
  it("executes imported HitDef controllers as active runtime attacks", () => {
    ...
    expect(snapshot.actors[1]?.runtime.hitVelocity).toEqual({ x: 4, y: 0, z: 0 });
...
`,
  ...
  it("accepts an omitted-damage root HitDef as zero damage while retaining target and GetHitVar metadata", () => {
    ...
    const internals = runtime as unknown as {
      p1: { targets: Array<{ actorId: string; targetId: number; age: number }> };
      p2: { runtime: CharacterRuntimeState };
    };
    internals.p2.runtime.vel = { x: 13, y: -11 };
    internals.p2.runtime.hitVelocity = { x: 9, y: -7, z: 5 };
    internals.p2.runtime.hitVars = { hitVelocities: { ground: { x: 9, y: -7, z: 5 } } };
    ...
    expect(snapshot.actors[1]?.runtime.hitVars).toMatchObject({
      damage: 0,
      hitDamage: 0,
      guardDamage: 0,
    ...
    expect(runtimeHitVar(snapshot.actors[1]!.runtime, "xvel")).toBe(0);
...
`,
  ...
  it("preserves live air.velocity siblings through RedirectID dynamic pair, single, and omissions", () => {
    const caller = createImportedFixture({
      id: "redirect-air-velocity-caller",
      withStateMove: false,
      passiveResourceController: `
[State 0, Receiver player id]
type = VarSet
...
[State 0, Dynamic air X]
type = VarSet
trigger1 = Time = 0
v = 10
value = -7

[State 0, Dynamic air Y]
type = VarSet
trigger1 = Time = 0
v = 11
value = -5

[State 0, Dynamic single air X]
type = VarSet
trigger1 = Time = 0
v = 12
value = -11

[State 0, Redirected dynamic air pair]
type = ModifyHitDef
trigger1 = Time = 1
air.velocity = var(10),var(11)
RedirectID = var(0)

[State 0, Pair omission]
type = ModifyHitDef
trigger1 = Time = 1
damage = 5
RedirectID = var(0)

[State 0, Redirected dynamic air single]
type = ModifyHitDef
trigger1 = Time = 2
air.velocity = var(12)
RedirectID = var(0)

[State 0, Single omission]
type = ModifyHitDef
trigger1 = Time = 2
numhits = 3
RedirectID = var(0)
`,
    ...
    const receiver = createImportedFixture({
      id: "redirect-air-velocity-receiver",
      withStateMove: false,
      passiveResourceController: `
[State 0, Receiver HitDef]
type = HitDef
...
`,
    ...
    internals.p1.runtime.vel = { x: 0, y: 0 };
    internals.p2.runtime.pos = { x: 35, y: 0 };
    const hit = runtime.step({ p1: new Set(), p2: new Set() });
    ...
    expect(hit.actors[0]?.runtime).toMatchObject({
      guarding: false,
      stateType: "A",
      vel: { x: -11, y: -5 },
      hitVelocity: { x: -11, y: -5, z: 4 },
      combatDepth: { velocity: 4 },
    });
    expect(runtimeHitVar(hit.actors[0]!.runtime, "xvel")).toBe(-11);
    expect(runtimeHitVar(hit.actors[0]!.runtime, "yvel")).toBe(-5);
    expect(runtimeHitVar(hit.actors[0]!.runtime, "zvel")).toBe(4);
    expect(
      hit.compatibilitySession?.actors.find(({ actorId }) => actorId === "p2")?.executedOperations.modifyhitdef,
    ).toBe(4);
  });
    ...
    expect(snapshot.compatibilitySession?.actors[0]?.redirectedTargetDispatches).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          requestedId: 77,
          candidateTargetIds: ["p1"],
          selectedTargetIds: ["p1"],
          mutatedActorIds: ["p1"],
          writebackActorIds: ["p1"],
          writebackMode: "direct",
          telemetryId: expect.stringMatching(/^redirect:p1:\d+$/),
        }),
      ]),
    );
  });
...
`,
  ...
  it("routes state-entry TargetPowerAdd RedirectID through the target owner target memory", () => {
    ...
    const redirectedTarget = createImportedFixture({
      id: "target-state-entry-redirect-target",
      withStateMove: true,
      hitDefDamage: 0,
      hitDefTargetId: 77,
      multiFrameAction: { id: 200, durations: [48] },
      stateEntryResourceController: `
[State -1, Redirected Entry Target Power]
type = TargetPowerAdd
triggerall = var(30) = 0
...
[State -1, Redirected Entry Target Power Gate]
type = VarSet
triggerall = var(30) = 0
triggerall = StageTime >= 40
triggerall = NumTarget(77) > 0
    ...
    expect(snapshot.compatibilitySession?.actors[1]?.redirectedTargetDispatches).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          destinationId: "p1",
          stateOwnerId: "p1",
          destinationRevision: "56:p1",
          controllerType: "targetpoweradd",
          operationClass: "target-resource",
          requestedId: 77,
          candidateTargetIds: ["p2"],
          selectedTargetIds: ["p2"],
          mutatedActorIds: ["p2"],
          writebackActorIds: ["p2"],
          writebackMode: "direct",
          telemetryId: expect.stringMatching(/^redirect:p2:\d+$/),
        }),
      ]),
    );
  });
...
`,
  ...
  it("routes active BindToTarget RedirectID through the destination target memory", () => {
    const redirectedCaller = createImportedFixture({
      id: "bind-to-target-redirect-caller",
      withStateMove: true,
      withBindToTarget: true,
      hitDefDamage: 0,
      hitDefTargetId: 77,
      bindToTargetRedirectId: 57,
    });
    const redirectedTarget = createImportedFixture({
      id: "bind-to-target-redirect-target",
      withStateMove: true,
      hitDefDamage: 0,
      hitDefTargetId: 77,
    });
    ...
    let routedSnapshot: typeof snapshot | undefined;
    const rememberRoutedSnapshot = () => {
      if (!routedSnapshot && (snapshot.compatibilitySession?.actors ?? []).some((actor) => actor.executedControllers.BindToTarget)) {
        routedSnapshot = snapshot;
      }
    };
    rememberRoutedSnapshot();
    ...
    expect(routedSnapshot).toBeDefined();
    expect(routedSnapshot?.actors[1]?.runtime.bindToTarget).toMatchObject({ actorId: "p1", targetId: 77, remaining: 3 });
    expect(snapshot.actors[0]?.runtime.targetCount).toBeGreaterThanOrEqual(1);
    expect(snapshot.actors[1]?.runtime.targetCount).toBeGreaterThanOrEqual(1);
...
`,
  ...
  it("routes state-entry BindToTarget RedirectID through the destination target memory", () => {
    const redirectedCaller = createImportedFixture({
      id: "bind-to-target-state-entry-redirect-caller",
      withStateMove: true,
      hitDefDamage: 0,
      hitDefTargetId: 77,
      multiFrameAction: { id: 200, durations: [48] },
    ...
    const redirectedTarget = createImportedFixture({
      id: "bind-to-target-state-entry-redirect-target",
      withStateMove: true,
      hitDefDamage: 0,
      hitDefTargetId: 77,
      multiFrameAction: { id: 200, durations: [48] },
      stateEntryResourceController: `
[State -1, Redirected Entry Bind To Target]
type = BindToTarget
triggerall = var(25) = 0
triggerall = StageTime >= 40
triggerall = NumTarget(77) > 0
...
[State -1, Redirected Entry Bind To Target Gate]
type = VarSet
triggerall = var(25) = 0
triggerall = StageTime >= 40
triggerall = NumTarget(77) > 0
...
`,
    ...
    expect(routedSnapshot).toBeDefined();
    expect(routedSnapshot?.actors[0]?.runtime.bindToTarget).toMatchObject({ actorId: "p2", targetId: 77, remaining: 3 });
    expect(snapshot.actors[0]?.runtime.targetCount).toBeGreaterThanOrEqual(1);
    expect(snapshot.actors[1]?.runtime.targetCount).toBeGreaterThanOrEqual(1);
    expect(snapshot.compatibilitySession?.actors[0]?.executedControllers.BindToTarget).toBeGreaterThanOrEqual(1);
    expect(snapshot.compatibilitySession?.actors[0]?.executedOperations.bindtotarget).toBeGreaterThanOrEqual(1);
  });
...
`,
  ...
  it("fails closed for an invalid BindToTarget RedirectID", () => {
    const caller = createImportedFixture({
      withStateMove: true,
      withBindToTarget: true,
      passiveTargetController: `
[State 0, Invalid Bind To Target Redirect]
type = BindToTarget
trigger1 = 1
id = 77
pos = 20,-8,Foot
...
`,
    ...
    expect(snapshot.actors[0]?.runtime.bindToTarget).toBeUndefined();
    expect(snapshot.compatibilitySession?.actors[0]?.executedControllers.BindToTarget).toBeUndefined();
    expect(snapshot.compatibilitySession?.actors[0]?.executedOperations.bindtotarget).toBeUndefined();
    expect(snapshot.logs.some((line) => line.includes("Blocked bindtotarget RedirectID 999"))).toBe(true);
  });
    ...
    expect(snapshot.compatibilitySession?.actors[0]?.redirectedTargetDispatches).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          operationClass: "target-resource",
          requestedId: 77,
          candidateTargetIds: ["p1", "p1-helper-0"],
          selectedTargetIds: ["p1"],
          mutatedActorIds: ["p1"],
          writebackActorIds: ["p1"],
    ...
    expect(snapshot.compatibilitySession?.actors[0]?.redirectedTargetDispatches).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          destinationRevision: "59:p2-helper-0",
          controllerType: "targetpoweradd",
          operationClass: "target-resource",
          requestedId: 77,
          candidateTargetIds: expect.arrayContaining(["p1"]),
          selectedTargetIds: ["p1"],
          mutatedActorIds: ["p1"],
          writebackActorIds: ["p1"],
          writebackMode: "helper-wrapper",
...
forcenofall = (var(25) - 1) / 2.0
forcestand = (var(25) - 1) / 2.0
...
`,
  ...
  it("routes IKEMEN root ModifyReversalDef p2getp1state through the target-owned state", () => {
    const caller = createImportedFixture({
      extraStateNos: [888],
      passiveResourceController: `
[State 0, Redirected ModifyReversalDef player id]
...
[State 0, Redirected ModifyReversalDef p2 owner]
type = ModifyReversalDef
trigger1 = Time = 1
p2getp1state = 0
RedirectID = var(0)
`,
    ...
    const destination = createImportedFixture({
      id: "redirected-root-modifyreversaldef-p2owner-destination",
      withStateMove: false,
      passiveReversalDef: { attr: "S,NA", p1StateNo: 777, p2StateNo: 888, hitPause: 3, targetId: 95 },
      passiveReversalTrigger: "Time = 0",
    });
    const runtime = new PlayableMatchRuntime(caller, destination, {
      ...trainingStage,
    ...
    expect(modified.actors[1]?.runtime.reversal).toMatchObject({ p2StateNo: 888, p2GetP1State: false });
    expect(modified.compatibilitySession?.actors[1]?.executedControllers.ModifyReversalDef).toBe(1);
    expect(modified.compatibilitySession?.actors[1]?.executedOperations.modifyreversaldef).toBe(1);
...
[State 0, Redirected ModifyReversalDef guard flag]
type = ModifyReversalDef
trigger1 = Time = 1
reversal.guardflag = H
RedirectID = var(0)
`,
    });
    const destination = createImportedFixture({
      id: "redirected-root-modifyreversaldef-guardflag-destination",
      withStateMove: false,
      passiveReversalDef: {
        attr: "S,NA",
        reversalGuardFlag: "A",
    ...
    expect(modified.actors[1]?.runtime.reversal).toMatchObject({ reversalGuardFlag: "H" });
    expect(modified.compatibilitySession?.actors[1]?.executedControllers.ModifyReversalDef).toBe(1);
    expect(modified.compatibilitySession?.actors[1]?.executedOperations.modifyreversaldef).toBe(1);
...
`,
  ...
  it("routes IKEMEN root ModifyReversalDef reversal.guardflag.not through an active receiver", () => {
    const caller = createImportedFixture({
      guardFlag: "H",
      passiveResourceController: `
[State 0, Redirected ModifyReversalDef player id]
...
[State 0, Redirected ModifyReversalDef guard flag not]
type = ModifyReversalDef
trigger1 = Time = 1
...
`,
    ...
    const destination = createImportedFixture({
      id: "redirected-root-modifyreversaldef-guardflag-not-destination",
      withStateMove: false,
      passiveReversalDef: {
        attr: "S,NA",
    ...
    expect(modified.actors[1]?.runtime.reversal).toMatchObject({ reversalGuardFlagNot: "A" });
    expect(modified.compatibilitySession?.actors[1]?.executedControllers.ModifyReversalDef).toBe(1);
    expect(modified.compatibilitySession?.actors[1]?.executedOperations.modifyreversaldef).toBe(1);
    expect(countered.actors[0]?.runtime.moveType).toBe("H");
    expect(countered.actors[1]?.runtime.stateNo).toBe(777);
...
`,
  ...
  it("routes IKEMEN Helper PlayerPush RedirectID to a root with caller-owned dynamic policy", () => {
    const caller = {
      ...createImportedFixture({
        withStateMove: false,
        withHelper: true,
        helperStateControllers: `
[State 1200, Redirected PlayerPush value]
type = VarSet
trigger1 = Time = 0
v = 0
value = 0

[State 1200, Redirected PlayerPush priority]
type = VarSet
trigger1 = Time = 0
v = 1
value = 4

[State 1200, Redirected PlayerPush]
type = PlayerPush
trigger1 = Time = 0
value = var(0)
priority = var(1)
...
`,
    ...
    const destination = createImportedFixture({
      id: "redirected-helper-playerpush-destination",
      withStateMove: false,
    });
    const farStage = {
      ...trainingStage,
...
function createImportedFixture(
  options: {
    ...
    bindToTargetRedirectId?: number | string;
...
time = 4
${options.bindToTargetRedirectId === undefined ? "" : `RedirectID = ${options.bindToTargetRedirectId}`}
`
    : "";
