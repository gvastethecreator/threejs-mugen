...
describe("RuntimeTraceGatePresets", () => {
  ...
  it("creates a synthetic imported sound artifact with PlaySnd and StopSnd event evidence", () => {
    ...
    expect(evidence?.soundEvents).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ actorId: "p1", type: "PlaySnd", group: 5, index: 0, channel: 2, lowPriority: true, volumeScale: 50, freqMul: 0.5, loop: true, stateNo: 200 }),
    ...
    expect(artifact.trace.finalActors.find((actor) => actor.id === "p1")?.soundEvents).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: "PlaySnd", group: 5, index: 0, channel: 2, lowPriority: true, volumeScale: 50, freqMul: 0.5, loop: true, stateNo: 200 }),
