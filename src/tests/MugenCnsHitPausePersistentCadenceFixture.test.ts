...
import {
  createMugenCnsHitPausePersistentCadenceFixtureVfs,
  MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST,
} from "../mugen/runtime/MugenCnsHitPausePersistentCadenceFixture";
import { createMugenCnsHitPausePersistentCadenceTraceArtifact } from "../mugen/runtime/RuntimeTraceGatePresets";

describe("M.U.G.E.N CNS HitPause persistent cadence fixture", () => {
  it("keeps the paired and unwrapped raw CNS controllers source-located", async () => {
    const character = await new MugenCharacterLoader().load(
      MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.entry,
      createMugenCnsHitPausePersistentCadenceFixtureVfs(),
    );

    expect(character.files.states).toEqual([MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.statePath]);
    expect(character.states.map((state) => state.id)).toEqual(
      MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.expectedStates,
    );
    expect(character.states.find((state) => state.id === 200)?.controllers).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          source: expect.objectContaining({ path: MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.statePath }),
        }),
        expect.objectContaining({
          type: "VelSet",
  ...
  it("uses a separate positive counter during HitPause and keeps the unwrapped control frozen", async () => {
    const artifact = await createMugenCnsHitPausePersistentCadenceTraceArtifact({
    ...
    const pairedPosAddEvents = p1Events.filter(
      (event) => event.controller === "PosAdd" &&
        event.stateSource?.path === MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.statePath,
    );
    expect(pairedPosAddEvents.map((event) => event.tick)).toEqual([2, 4, 6]);
    ...
    expect(p1Events).not.toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({ controller: "VelSet", stateSource: expect.objectContaining({ path: MUGEN_CNS_HITPAUSE_PERSISTENT_CADENCE_FIXTURE_MANIFEST.statePath }) }),
      ]),
    );
