...
describe("ProjectileSystem", () => {
  ...
  it("resolves Projectile fall impact per component and preserves finite siblings", () => {
    const controllerValue = controller({
      projanim: "1005",
      fall: "1",
      "fall.damage": "var(0) + .8",
      ...
      "fall.yvelocity": "-7",
      "fall.zvelocity": "var(2) + .5",
    });
    const operation = compileControllerIr(controllerValue).operation as ProjectileControllerOp;
    const spawn = (
      serialId: string,
      resolveFallImpact?: Parameters<typeof createRuntimeProjectile>[0]["resolveFallImpact"],
    ) => createRuntimeProjectile({
      serialId,
      controller: controllerValue,
      operation,
    ...
    expect(spawn("p1-projectile-fall-impact-dynamic", () => ({
      damage: 17.8,
      xVelocity: -6.5,
      yVelocity: -99,
      zVelocity: 2.5,
    ...
    expect(spawn("p1-projectile-fall-impact-partial", () => ({
      damage: 17.8,
      xVelocity: Number.NaN,
      zVelocity: 2.5,
    })).fall).toEqual({
      enabled: true,
      damage: 17,
    ...
    expect(spawn("p1-projectile-fall-impact-unresolved").fall).toEqual({
      enabled: true,
      yVelocity: -7,
    });
  });
  ...
  it("matches pinned ModifyProjectile guard.velocity Y/Z zero-fill semantics", () => {
    const seeded = {
      guardPush: 7,
      guardVelocityY: -6,
      guardVelocityZ: 4,
      hitVelocities: { guard: { x: -7, y: -6, z: 4 } },
    };
    const omitted = projectile({ projectileId: 77, ...seeded });
    expect(modifyRuntimeProjectiles([omitted], {
      controller: controller({ id: "77" }),
    ...
    const pair = projectile({ projectileId: 77, ...seeded });
    expect(modifyRuntimeProjectiles([pair], {
      controller: controller({ id: "77", "guard.velocity": "-9,-8" }),
    })).toBe(1);
    expect([pair.guardPush, pair.guardVelocityY, pair.guardVelocityZ]).toEqual([9, -8, 0]);
    expect(pair.hitVelocities?.guard).toEqual({ x: -9, y: -8, z: 0 });

    const triple = projectile({ projectileId: 77, ...seeded });
    expect(modifyRuntimeProjectiles([triple], {
      controller: controller({ id: "77", "guard.velocity": "-9,-8,3" }),
    })).toBe(1);
    expect([triple.guardPush, triple.guardVelocityY, triple.guardVelocityZ]).toEqual([9, -8, 3]);
    expect(triple.hitVelocities?.guard).toEqual({ x: -9, y: -8, z: 3 });
  });
  ...
  it("selects one same-id ModifyProjectile down.velocity match by oldest-first index", () => {
    const newest = projectile({ serialId: "down-index-newest", projectileId: 77, downVelocityX: 1, downVelocityY: 2, downVelocityZ: 3 });
    const oldest = projectile({ serialId: "down-index-oldest", projectileId: 77, downVelocityX: 4, downVelocityY: 5, downVelocityZ: 6 });
    const trap = projectile({ serialId: "down-index-trap", projectileId: 88, downVelocityX: 7, downVelocityY: 8, downVelocityZ: 9 });
    const operation = compileControllerIr(controller({
      id: "77",
      index: "var(0)",
      "down.velocity": "-11,-5,3",
    }, "ModifyProjectile")).operation as ModifyProjectileControllerOp;
    const resolvedKeys: string[] = [];

    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "var(0)", "down.velocity": "-11,-5,3" }),
      operation,
      resolveModifyProjectile: {
        resolveNumber: (key) => {
          resolvedKeys.push(key);
          return key === "index" ? 1 : undefined;
        },
      },
    })).toBe(1);

    expect(resolvedKeys).toEqual(["index"]);
    expect(newest).toMatchObject({ downVelocityX: -11, downVelocityY: -5, downVelocityZ: 3 });
    expect(newest.hitVelocities?.down).toEqual({ x: -11, y: -5, z: 3 });
    expect(oldest).toMatchObject({ downVelocityX: 4, downVelocityY: 5, downVelocityZ: 6 });
    expect(trap).toMatchObject({ downVelocityX: 7, downVelocityY: 8, downVelocityZ: 9 });
  });

  it("selects one same-id ModifyProjectile ground.velocity match by oldest-first index", () => {
    const newest = projectile({ serialId: "ground-index-newest", projectileId: 77, push: 1, hitVelocityY: 2, hitVelocityZ: 3 });
    const oldest = projectile({ serialId: "ground-index-oldest", projectileId: 77, push: 4, hitVelocityY: 5, hitVelocityZ: 6 });
    const trap = projectile({ serialId: "ground-index-trap", projectileId: 88, push: 7, hitVelocityY: 8, hitVelocityZ: 9 });
    const staticOperation = compileControllerIr(controller({
      id: "77",
      index: "0",
      "ground.velocity": "-6,-4",
    ...
    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "0", "ground.velocity": "-6,-4" }),
      operation: staticOperation,
    })).toBe(1);
    expect(oldest).toMatchObject({ push: 6, hitVelocityY: -4, hitVelocityZ: 6 });
    expect(oldest.hitVelocities?.ground).toEqual({ x: -6, y: -4, z: 6 });
    expect(newest).toMatchObject({ push: 1, hitVelocityY: 2, hitVelocityZ: 3 });
    ...
    const resolvedKeys: string[] = [];
    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "var(0)", "ground.velocity": "-7,-5,2" }),
      operation: dynamicOperation,
      resolveModifyProjectile: {
        resolveNumber: (key) => {
          resolvedKeys.push(key);
    ...
    expect(resolvedKeys).toEqual(["index"]);
    expect(newest).toMatchObject({ push: 7, hitVelocityY: -5, hitVelocityZ: 2 });
    expect(newest.hitVelocities?.ground).toEqual({ x: -7, y: -5, z: 2 });
    expect(oldest).toMatchObject({ push: 6, hitVelocityY: -4, hitVelocityZ: 6 });
    expect(trap).toMatchObject({ push: 7, hitVelocityY: 8, hitVelocityZ: 9 });
  });

  it("selects one same-id ModifyProjectile guard.velocity match by oldest-first index", () => {
    const newest = projectile({ serialId: "guard-index-newest", projectileId: 77, guardPush: 1, guardVelocityY: 2, guardVelocityZ: 3 });
    const oldest = projectile({ serialId: "guard-index-oldest", projectileId: 77, guardPush: 4, guardVelocityY: 5, guardVelocityZ: 6 });
    const trap = projectile({ serialId: "guard-index-trap", projectileId: 88, guardPush: 7, guardVelocityY: 8, guardVelocityZ: 9 });
    const staticOperation = compileControllerIr(controller({
      id: "77",
      index: "0",
      "guard.velocity": "-6,-4",
    ...
    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "0", "guard.velocity": "-6,-4" }),
      operation: staticOperation,
    })).toBe(1);
    expect(oldest).toMatchObject({ guardPush: 6, guardVelocityY: -4, guardVelocityZ: 0 });
    expect(oldest.hitVelocities?.guard).toEqual({ x: -6, y: -4, z: 0 });
    expect(newest).toMatchObject({ guardPush: 1, guardVelocityY: 2, guardVelocityZ: 3 });

    const dynamicOperation = compileControllerIr(controller({
      id: "77",
      index: "var(0)",
      "guard.velocity": "-7,-5,2",
    }, "ModifyProjectile")).operation as ModifyProjectileControllerOp;
    const resolvedKeys: string[] = [];
    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "var(0)", "guard.velocity": "-7,-5,2" }),
      operation: dynamicOperation,
      resolveModifyProjectile: {
        resolveNumber: (key) => {
          resolvedKeys.push(key);
    ...
    expect(resolvedKeys).toEqual(["index"]);
    expect(newest).toMatchObject({ guardPush: 7, guardVelocityY: -5, guardVelocityZ: 2 });
    expect(newest.hitVelocities?.guard).toEqual({ x: -7, y: -5, z: 2 });
    expect(oldest).toMatchObject({ guardPush: 6, guardVelocityY: -4, guardVelocityZ: 0 });
    expect(trap).toMatchObject({ guardPush: 7, guardVelocityY: 8, guardVelocityZ: 9 });
  });

  it("selects one same-id ModifyProjectile airguard.velocity match by oldest-first index", () => {
    const newest = projectile({ serialId: "airguard-index-newest", projectileId: 77, airGuardPush: 1, airGuardVelocityY: 2, airGuardVelocityZ: 3 });
    const oldest = projectile({ serialId: "airguard-index-oldest", projectileId: 77, airGuardPush: 4, airGuardVelocityY: 5, airGuardVelocityZ: 6 });
    const trap = projectile({ serialId: "airguard-index-trap", projectileId: 88, airGuardPush: 7, airGuardVelocityY: 8, airGuardVelocityZ: 9 });
    const staticOperation = compileControllerIr(controller({
      id: "77",
      index: "0",
    ...
    expect(modifyRuntimeProjectiles([newest, trap, oldest], {
      controller: controller({ id: "77", index: "0", "airguard.velocity": "-6,-4" }),
      operation: staticOperation,
    })).toBe(1);
    expect(oldest).toMatchObject({ airGuardPush: 6, airGuardVelocityY: -4, airGuardVelocityZ: 0 });
    expect(oldest.hitVelocities?.airGuard).toEqual({ x: -6, y: -4, z: 0 });
    expect(newest).toMatchObject({ airGuardPush: 1, airGuardVelocityY: 2, airGuardVelocityZ: 3 });
    ...
    expect(resolvedKeys).toEqual(["index"]);
    expect(newest).toMatchObject({ airGuardPush: 7, airGuardVelocityY: -5, airGuardVelocityZ: 2 });
    expect(newest.hitVelocities?.airGuard).toEqual({ x: -7, y: -5, z: 2 });
    expect(oldest).toMatchObject({ airGuardPush: 6, airGuardVelocityY: -4, airGuardVelocityZ: 0 });
