  ...
  it("exposes profile-scoped HelperVar fields through the IKEMEN micro-VM", () => {
    const runtimeProgram = {
      states: [
        stateProgram(
          stateDef(6000),
          [compiledControllerIr(6000, "VarSet", ["HelperVar(helpertype) = 1 && HelperVar(id) = 200 && HelperVar(keyctrl) = 1 && HelperVar(ownpal) = 1 && HelperVar(ownprojectile) = 1 && HelperVar(preserve) = 1 && HelperVar(ownclsnscale) = 1"], { v: "0", value: "1" })],
        ),
      ],
    };

    const owned = helper({ keyCtrl: true, ownPalette: true, ownProjectile: true, preserve: true, ownClsnScale: true, runtimeProgram, vars: [0] });
    const rootOwned = helper({ ownPalette: false, ownProjectile: false, preserve: false, ownClsnScale: false, runtimeProgram, vars: [0] });
    const legacy = helper({ ownPalette: true, ownProjectile: true, preserve: true, ownClsnScale: true, runtimeProgram, vars: [0] });
