...
`).commands[0];
  ...
  it("classifies supported and unsupported trigger expressions before runtime evaluation", () => {
    ...
    const inverseTrig = compileExpression("Acos(1)");
...
`);
  ...
  it("compiles static resource and variable controllers into typed operations", () => {
    ...
    expect(compileControllerIr(controller(200, "ParentVarSet", [], { v: "1.9", value: "99" })).operation).toEqual({
      kind: "variable",
  ...
  it("compiles live ModifyHitDef hitsound refs and rejects malformed arity", () => {
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      redirectid: "57",
      hitsound: "S5,4",
    })).operation).toMatchObject({
    ...
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      ...
      kind: "modifyhitdef",
      hitSoundExpression: "Fvar(0),var(1)",
      redirectPlayerIdExpression: "57",
    });
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      redirectid: "57",
  ...
  it("compiles live ModifyHitDef guardsound.channel integers and rejects malformed expressions", () => {
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      redirectid: "57",
      "guardsound.channel": "3.9",
    })).operation).toMatchObject({
      kind: "modifyhitdef",
      guardSoundChannel: 3,
      redirectPlayerIdExpression: "57",
    ...
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      redirectid: "57",
      "guardsound.channel": "var(2) + 1",
    })).operation).toMatchObject({
      kind: "modifyhitdef",
      guardSoundChannel: "var(2) + 1",
    });
    expect(compileControllerIr(controller(200, "ModifyHitDef", [], {
      redirectid: "57",
      "guardsound.channel": "var(",
    })).operation).toBeUndefined();
  });
  ...
  it("compiles static damage scale controllers into typed operations", () => {
    ...
    const guardPointsOnly = compileControllerIr(controller(200, "AttackMulSet", [], { guardpoints: "0.5" }));
    ...
    expect(guardPointsOnly.operation).toEqual({
      kind: "damage-scale",
      controllerType: "attackmulset",
      guardPointsMultiplier: 0.5,
    });
    expect(dynamic.operation).toBeUndefined();
  });
  ...
  it("compiles dynamic Projectile EnvShake packages and rejects malformed values", () => {
    const projectile = compileControllerIr(controller(1000, "Projectile", [], {
    ...
    expect(projectile.operation).not.toHaveProperty("envShakeTime");
    expect(compileControllerIr(controller(1000, "Projectile", [], {
      "envshake.time": "var(",
    })).operation).toBeUndefined();
  });
