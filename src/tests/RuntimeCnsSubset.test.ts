...
describe("ExpressionEvaluator", () => {
  it("evaluates basic MUGEN-style comparisons and boolean operators", () => {
    ...
    expect(evaluateExpression("RunOrder", { self: runtimeState({ runOrder: 2 }) })).toBe(2);
    expect(evaluateExpression("RunOrder", { self: runtimeState() })).toBe(-1);
  ...
  it("evaluates profile-scoped IKEMEN HelperVar fields", () => {
    ...
    expect(evaluateExpression("HelperVar(helpertype) = 1 && HelperVar(id) = 42 && HelperVar(keyctrl) = 1 && HelperVar(ownpal) = 1 && HelperVar(ownprojectile) = 1 && HelperVar(preserve) = 1", {
      self: state,
      isHelper: true,
    ...
    expect(evaluateExpression("HelperVar(preserve) = 0", {
      self: state,
      isHelper: true,
      helperVarEnabled: true,
      helperPreserve: false,
    })).toBe(1);
    ...
    expect(evaluateExpression("HelperVar(id) = 0 && HelperVar(keyctrl) = 0 && HelperVar(ownpal) = 0 && HelperVar(ownprojectile) = 0 && HelperVar(preserve) = 0", { self: state })).toBe(1);
  });
  ...
  it("evaluates common MUGEN axis, range, const, and hitvar trigger syntax", () => {
    ...
    state.combatDepth = { position: 12, velocity: 0, size: [3, 3], attack: [4, 4] };
    expect(evaluateExpression("Pos Z = 12", { self: state, opponent })).toBe(1);
    state.combatDepth.velocity = -3;
    expect(evaluateExpression("Vel Z = -3", { self: state, opponent })).toBe(1);
    expect(
  ...
  it("executes compatibility movement and meter controllers used by common CNS files", () => {
    ...
    const compiledAssertSpecial = compileControllerIr(
      controller("AssertSpecial", {
        flag: "NoAutoTurn",
        flag2: "NoWalk, Invisible, TimerFreeze, RoundNotOver, NoKOSlow, Intro, NoDizzyPointsDamage",
    ...
    expect(state.assertSpecial).toMatchObject({
      flags: ["noautoturn", "nowalk", "invisible", "nodizzypointsdamage", "nojugglecheck", "nogetupfromliedown", "nofastrecoverfromliedown"],
