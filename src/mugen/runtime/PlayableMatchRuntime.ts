...
import { RuntimeBgPalFxWorld } from "./RuntimeBgPalFxSystem";
          ...
          const result = matchActorAdvanceWorld.advance({
            ...
            advanceHelper: (helper) => {
              ...
              this.effectLifecycleWorld.advanceHelper(owner, helper, this.stage, opponent, {
                ...
                opponentConstants: opponent.definition.constants,
                opponentLocalCoord: opponent.definition.localCoord,
                p2BodyDistYUsesSizeBoxes: !usesMugenPlayerPushMinimumWidth(owner.definition),
...
function redirectableHitEligibilityControllerType(controller: ControllerIr): RedirectableHitEligibilityControllerType | undefined {
  return controller.normalizedType === "hitby" || controller.normalizedType === "nothitby"
    ? controller.normalizedType
    : undefined;
}
...
function hitEligibilityControllerRedirectExpression(controller: ControllerIr): string | undefined {
  if (redirectableHitEligibilityControllerType(controller) === undefined) {
    return undefined;
  }
...
function materializeHitEligibilityRedirectController(
  controller: ControllerIr,
  caller: FighterMatchState,
  context: ReturnType<typeof runtimeControllerContext>,
): ControllerIr | undefined {
  const operation = controller.operation?.kind === "eligibility"
    ? controller.operation
...
function runActiveStateControllers(
  ...
  const hookSet = activeControllerHookSetWorld.create<FighterMatchState>({
      ...
      hitDefControllerDispatchWorld.apply({
        ...
        resolveIntegerScalar: (key) => {
          ...
          const raw = key === "snaptime" && operation?.snapTime !== undefined
            ? operation.snapTime
            : key === "priority" && operation?.priority !== undefined
            ? operation.priority
    ...
    modifyHitDef: ({ controller, actor, opponent: targetOpponent, owner: stateOwner, tick: activeTick }) => {
      ...
      const result = hitDefControllerDispatchWorld.modify({
        ...
        resolveIntegerScalar: (key) => {
          ...
          const raw = key === "priority" && operation?.priority !== undefined
            ? operation.priority
            : key === "priority" && operation?.priorityExpression !== undefined
            ? operation.priorityExpression
            : findParam(controller, key) ??
        ...
        effectSpawnControllerDispatchWorld.apply({
          ...
          resolveProjectileGroundFriction:
            effect === "projectile"
              ...
              : undefined,
          resolveProjectileGroundVelocity:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const pair = operation?.groundVelocityExpressions;
                  const zExpression = operation?.groundVelocityZExpression;
          ...
          resolveProjectileGuardVelocity:
            effect === "projectile"
              ...
              : undefined,
          resolveProjectileAirGuardVelocity:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
              ...
              : undefined,
          resolveProjectileAirVelocity:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const pair = operation?.airVelocityExpressions;
                  const zExpression = operation?.airVelocityZExpression;
                  if (pair === undefined && zExpression === undefined) return undefined;
                  const resolveComponent = (value: number | string | undefined): number | undefined => {
                    if (typeof value === "number") return Number.isFinite(value) ? value : undefined;
                    if (value === undefined) return undefined;
          ...
          resolveProjectileDownVelocity:
            effect === "projectile"
              ...
              : undefined,
          resolveProjectileDownHitTime:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.downHitTimeExpression;
              ...
              : undefined,
          resolveProjectileGroundHitTime:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.groundHitTimeExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileGroundSlideTime:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.groundSlideTimeExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileAirHitTime:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.airHitTimeExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
          ...
          resolveProjectileRemoveTime:
            effect === "projectile"
              ...
              : undefined,
          resolveProjectileMissTime:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.missTimeExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectilePriority:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.priorityExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
            ...
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.hitCountExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
            ...
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.guardHitTimeExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
            ...
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const value = operation?.damageExpressions;
                  if (value === undefined) return undefined;
                  const resolveComponent = (component: number | string | undefined): number | undefined => {
          ...
          resolveProjectileGivePower:
            effect === "projectile"
              ...
              : undefined,
          resolveProjectileGuardPoints:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.guardPointsExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileP2Facing:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.p2FacingExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileP1StateNo:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.p1StateNoExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileP2StateNo:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.p2StateNoExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
              ...
              : undefined,
          resolveProjectileP2GetP1State:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.p2GetP1StateExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? expression : undefined;
                  }
                  const resolved = resolveDispatchNumber(
                    undefined,
                    expression,
              ...
              : undefined,
          resolveProjectileKeepState:
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.keepStateExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? expression : undefined;
                  }
            ...
            effect === "projectile"
              ? () => {
                  const operation = controller.operation?.kind === "projectile"
                    ? controller.operation
                    : undefined;
                  const expression = operation?.projAnimExpression;
                  if (expression === undefined) return undefined;
                  if (typeof expression === "number") {
                    return Number.isFinite(expression) ? Math.trunc(expression) : undefined;
                  }
    ...
    bgPalFx: ({ controller, actor, opponent: targetOpponent, owner: stateOwner, tick: activeTick }) => {
      const bgPalFxWorld = options.bgPalFxWorld;
      if (!bgPalFxWorld) {
        return;
      }
      ...
      const operation =
        controller.operation?.kind === "bgpalfx"
          ? controller.operation
          : bgPalFxWorld.resolveOperation(controller.source, undefined, resolvePaletteFx);
    ...
    runtimeController: ({ dispatch, actor, opponent: targetOpponent, owner, tick: activeTick }) => {
      ...
      const materializedScreenBoundController =
        dispatch.controller.normalizedType === "screenbound" && redirectExpression !== undefined
          ...
          : undefined;
      const materializedHitEligibilityController =
