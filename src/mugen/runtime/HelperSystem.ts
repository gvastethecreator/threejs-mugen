...
export function resolveRuntimeHelperIntegerPairParam(
  ...
  const operationValue: MugenHitDefExpressionPair | undefined =
    operation?.kind === "hitdef" || operation?.kind === "modifyhitdef" || operation?.kind === "projectile"
      ? key === "pausetime" && (operation.kind === "hitdef" || operation.kind === "modifyhitdef" || operation.kind === "projectile")
        ? operation.pauseTimeExpressions
        : key === "guard.pausetime" && (operation.kind === "hitdef" || operation.kind === "modifyhitdef" || operation.kind === "projectile")
...
export function resolveRuntimeHelperIntegerScalarParam(
  helper: RuntimeHelper,
  controller: ControllerIr,
  key: "id" | "chainid" | "p1facing" | "p1getp2facing" | "p2facing" | "p1sprpriority" | "p2sprpriority" | "priority" | "ground.hittime" | "ground.slidetime" | "air.hittime" | "down.hittime" | "guard.hittime" | "guard.slidetime" | "guard.ctrltime" | "airguard.ctrltime" | "guard.dist" | "down.bounce" | "air.juggle" | "numhits" | "forcestand" | "forcecrouch" | "forcenofall" | "snaptime" | "hitsound.channel" | "guardsound.channel" | "guardpoints" | "keepstate" | "projremovetime" | "projmisstime" | "projpriority" | "projhits" | "projanim" | "p1stateno" | "p2stateno" | "p2getp1state",
  options: Parameters<typeof resolveHelperNumber>[3],
): number | undefined {
  if ((key === "p1stateno" || key === "p2stateno" || key === "p2getp1state") && controller.operation?.kind === "projectile") {
    const expression = key === "p1stateno"
      ? controller.operation.p1StateNoExpression
      : key === "p2stateno"
        ? controller.operation.p2StateNoExpression
        : controller.operation.p2GetP1StateExpression;
    return expression === undefined ? undefined : resolveRuntimeHelperIntegerExpression(helper, expression, options);
  }
  if (key === "priority") {
    const operation = controller.operation;
  ...
  if (key === "snaptime") {
    const operation = controller.operation;
    const value = operation?.kind === "hitdef" ? operation.snapTime : undefined;
    if (value !== undefined) return resolveRuntimeHelperIntegerExpression(helper, value, options);
    const raw = findControllerParam(controller.source, "snap");
    const parts = topLevelCommaIndices(raw ?? "");
    if (raw !== undefined && parts.length === 3) {
      const expression = raw.slice(parts[2]! + 1).trim();
      const resolved = expression ? resolveHelperFloat(helper, expression, options) : undefined;
      return resolved === undefined || !Number.isFinite(resolved) ? undefined : Math.trunc(resolved);
    }
  }
  if (key === "down.hittime") {
  ...
  if (key === "projremovetime") {
    const operation = controller.operation;
    const value = operation?.kind === "projectile"
      ? operation.removeTimeExpression
      : undefined;
    if (value !== undefined) return resolveRuntimeHelperIntegerExpression(helper, value, options);
  }
  if (key === "projmisstime") {
  ...
  const raw = findControllerParam(controller.source, key) ??
    (key === "p1sprpriority"
      ...
      : key === "projremovetime"
        ? findControllerParam(controller.source, "removetime")
        : undefined);
...
/**
 * Resolves a Helper-authored Projectile's dynamic fresh air.velocity vector.
 * Missing components stay undefined so ProjectileSystem can apply fresh zero defaults.
 */
export function resolveRuntimeHelperProjectileAirVelocity(
  helper: RuntimeHelper,
  controller: ControllerIr,
  options: Parameters<typeof resolveHelperNumber>[3],
): [number?, number?, number?] | undefined {
