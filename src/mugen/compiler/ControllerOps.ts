...
export type ModifyHitDefControllerOp = {
  ...
  guardVelocityExpression?: number | string;
  /** Component-wise live guard.velocity X/Y replacement evaluated in caller context. */
  guardVelocityExpressions?: MugenHitDefExpressionPair;
  guardVelocityZ?: number;
  /** Live guard.velocity Z replacement evaluated in caller context. */
  ...
  /** Ikemen-only live hitsound.channel replacement evaluated in the ModifyHitDef caller context. */
  hitSoundChannel?: number | string;
...
export type ModifyReversalDefControllerOp = {
  ...
  /** Live ReversalDef attacker-facing replacement evaluated in caller context. */
  p1Facing?: number | string;
  /** Live ReversalDef attacker-facing replacement from incoming P2 facing. */
  p1GetP2Facing?: number | string;
  p2Facing?: number | string;
  targetId?: number;
...
export type ProjectileControllerOp = {
  ...
  fallEnvShake?: MugenHitDefEnvShakeOp;
  /** Dynamic or mixed Projectile fall impact package evaluated in caller context. */
  fallImpact?: MugenHitDefFallImpactOp;
...
export type ReversalDefControllerOp = {
  ...
  p2GetP1State?: boolean | string;
  /** ReversalDef attacker-facing replacement evaluated in the caller context. */
  p1Facing?: number | string;
  /** ReversalDef attacker-facing replacement from the incoming P2 facing. */
  p1GetP2Facing?: number | string;
...
function compileReversalDefControllerOp(controller: MugenStateController): ReversalDefControllerOp | undefined {
  ...
  const attackDepth = Array.isArray(attackDepthValue) && attackDepthExpressions === undefined
    ? [attackDepthValue[0], attackDepthValue[1] ?? attackDepthValue[0]] as [number, number]
    : undefined;
...
function compileModifyHitDefControllerOp(controller: MugenStateController): ModifyHitDefControllerOp | undefined {
  ...
  const guardVelocityExpression = guardVelocityRaw === undefined
    ...
    : staticGuardVelocity !== undefined
      ...
      : typeof guardVelocityValue === "object"
        ? guardVelocityValue.xy[0]
        : optionalScalarNumberOrExpression(controller, "guard.velocity");
  const guardVelocityExpressions = typeof guardVelocityValue === "object"
    ? guardVelocityValue.xy
    : undefined;
  const guardVelocityZValue = typeof guardVelocityValue === "object" ? guardVelocityValue.z : undefined;
  const guardVelocityZ = typeof guardVelocityZValue === "number" ? guardVelocityZValue : true;
  const guardVelocityZExpression = typeof guardVelocityZValue === "string" ? guardVelocityZValue : undefined;
  ...
  const hitSoundChannel = optionalIntegerExpressionParam(controller, "hitsound.channel");
...
function compileModifyReversalDefControllerOp(controller: MugenStateController): ModifyReversalDefControllerOp | undefined {
  ...
  return {
    ...
    ...(normalizedP2StateNo === undefined ? {} : { p2StateNo: normalizedP2StateNo }),
    ...(normalizedP2GetP1State === undefined ? {} : { p2GetP1State: normalizedP2GetP1State }),
    ...(normalizedP1Facing === undefined ? {} : { p1Facing: normalizedP1Facing }),
...
function compileProjectileControllerOp(controller: MugenStateController): ProjectileControllerOp | undefined {
  ...
  const fallEnvShakeExpressions = fallEnvShake === true || !Object.values(fallEnvShake).some((value) => typeof value === "string")
    ...
    : fallEnvShake;
  const fallImpact = optionalHitDefFallImpactParam(controller);
  if (fallImpact === false) return undefined;
  const fallImpactExpressions = fallImpact === true || !Object.values(fallImpact).some((value) => typeof value === "string")
    ? undefined
    : fallImpact;
  const fallRecovery = optionalHitDefFallRecoveryParam(controller);
