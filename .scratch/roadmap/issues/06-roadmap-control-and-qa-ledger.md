...
- Current 2026-07-11 roadmap/architecture checkpoint: highest numbered backlog truth is entry 398 and the declared aggregate is 539/539 traces (508 required, 31 optional). Required `synthetic-imported-ikemen-helper-self-tag` closes Wayfinder 092 without score movement.
- Current lane decision: I1 remains scanner/reporting; I2 explicit-profile runtime routes through `.scratch/roadmap/issues/07-ikemen-runtime-topology.md`. Wayfinder 093 maps active-root input, combat, round, presentation, effect, and resource ownership before selecting one executable gameplay cut.
- Current delivery order: finish the active-root ownership map, then choose its smallest coherent I2 gameplay gate. The bounded MUGEN-lite post-KO / `NoKOSlow` timeline remains the next competing runtime lane rather than being folded into team work.
