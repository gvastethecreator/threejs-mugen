...
- Current 2026-07-11 roadmap/architecture checkpoint: highest numbered backlog truth is entry 403 and the declared aggregate is 541/541 traces (510 required, 31 optional). Wayfinder 097 closes restricted normal-tick active-root CNS/motion/animation; Wayfinder 098 maps the first browser-visible presentation boundary without implying later gameplay owners.
...
- Current delivery order: map and implement presentation/camera for the already-live motion root, then root-key effects, combat, round, and resources as separate source-backed gates. Direct native input/AI remains an independent policy. The bounded MUGEN-lite post-KO / `NoKOSlow` timeline remains the next competing runtime lane rather than being folded into team work.
