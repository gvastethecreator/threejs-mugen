...
- Current 2026-07-11 roadmap/architecture checkpoint: highest numbered backlog truth is entry 404 and the declared aggregate remains 541/541 traces (510 required, 31 optional). Wayfinder 097 closes restricted normal-tick active-root CNS/motion/animation; Wayfinder 098 closes the source-backed presentation map; Wayfinder 099 implements the first browser-visible draw/camera handoff without implying later gameplay owners.
- Current lane decision: I1 remains scanner/reporting; I2 explicit-profile runtime routes through `.scratch/roadmap/issues/07-ikemen-runtime-topology.md`. `RuntimeRootPhaseCapabilities/v1` is the current machine-readable phase truth.
- Current delivery order: implement and browser-gate the selected presentation/camera contract for the already-live motion root, then root-key effects, combat, round, and resources as separate source-backed gates. Direct native input/AI remains an independent policy. The bounded MUGEN-lite post-KO / `NoKOSlow` timeline remains the next competing runtime lane rather than being folded into team work.
