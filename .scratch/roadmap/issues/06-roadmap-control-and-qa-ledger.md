...
- Current 2026-07-11 roadmap/architecture checkpoint: highest numbered backlog truth is entry 399 and the declared aggregate remains 539/539 traces (508 required, 31 optional). Wayfinder 093 closes the source-backed active-root ownership map without executable or score movement.
- Current lane decision: I1 remains scanner/reporting; I2 explicit-profile runtime routes through `.scratch/roadmap/issues/07-ikemen-runtime-topology.md`. Wayfinder 094 implements Tag side command routing plus reserve-root trace observability without granting P3-P8 direct gameplay ownership.
- Current delivery order: close command mapping, then add a per-phase active-root capability contract before direct input/kinematics, presentation, root-key effects, combat, round, and resources. The bounded MUGEN-lite post-KO / `NoKOSlow` timeline remains the next competing runtime lane rather than being folded into team work.
