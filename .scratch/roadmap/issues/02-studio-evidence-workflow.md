...
- Current CSS ownership checkpoint: Build/Evidence trust-ledger right rails live in `src/styles/studio-trust-ledgers.css`, and Modules/Debug system-ledger rows now live in `src/styles/studio-system-ledgers.css` instead of tail-end `src/style.css` blocks. `pnpm qa:css` reports 4,048 scanned rules, 372 duplicate selector keys, 0 exact duplicate rules, and 314 repeated declaration groups; `pnpm qa:smoke` passed, and `.scratch/qa/qa-smoke/studio-modules.png`, `.scratch/qa/qa-smoke/studio-debug.png`, `.scratch/qa/qa-smoke/studio-build.png`, and `.scratch/qa/qa-smoke/studio-evidence.png` were visually inspected.
