# Issue 95 — Ikemen-GO `GetHitVar(hitpower)` readback

Status: closed-bounded
Lane: I2
Priority: P1
Dependency: issue 94 / T520
Date: 2026-08-02

## Official contract

The current Ikemen-GO changed-trigger documentation defines
`GetHitVar(hitpower)` as the first value of the last HitDef `givepower`
parameter: the power received when hit. The current `develop` compiler maps
the trigger to `OC_ex_gethitvar_hitpower`, bytecode reads `ghv.hitpower`, and
the character hit path retains the authored value in last-hit metadata.
...
- Add typed last-hit hit-power metadata without conflating it with the
  defender's current `power` resource.
- Parse the first `givepower` value for direct HitDef and Projectile sources,
...
- Carry the authored value through direct and Projectile contacts and expose it
  through `GetHitVar(hitpower)`.
- Leave hitflag, current power resource readback, cumulative multi-hit/reset
...
This issue covers numeric `GetHitVar(hitpower)` readback for direct HitDef and
Projectile hit/guard contacts in the bounded runtime path. It does not claim
current power resource readback, `GetHitVar(hitflag)`, cumulative reset
semantics, or full M.U.G.E.N/Ikemen parity.

## Closeout evidence

- Focused compiler/runtime-context/direct-combat/Projectile-system/
  Projectile-combat coverage: 5 files / 228 tests passed.
...
- Full Vitest remains at the inherited retired-roster baseline: 14 failed /
  311 passed files and 58 failed / 3289 passed tests (3347 total). Visible
  failures are the retired Nova/Mira/Rook/DA29/DA30 expectations and the
  inherited projectile multi-hit trace; no T521-specific failure remains.
