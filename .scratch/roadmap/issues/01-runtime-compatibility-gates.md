...
- Next recommended gate: exact-enough recovery threshold or recovery velocity evidence beyond the current synthetic plus official KFM too-early reject windows. Good candidate: expose the `CanRecover`/`fall.recovertime` transition or air/ground recovery velocity in actor-frame evidence.
