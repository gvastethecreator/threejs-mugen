...
- Latest completed R2 cut: `RuntimePauseWorld` now owns current match pause state, snapshot projection, source-movetime checks, countdown ticks, controller application, and reset while preserving existing `Pause`/`SuperPause` trace behavior through focused `PauseSystem` / `PlayableMatchRuntime` tests.
- Latest completed R1 cut: optional `kfm-official-default-fall-recovery-threshold.json` checksum `891d0f6d` confirms the private official KFM fixture reaches state `5050` while `hitFall.recoverTime` is still positive, then routes through ground recovery `5200 -> 5201 -> 52 -> 0` after `command = "recovery"` is accepted near the ground. This relies on local fixture presence and does not claim public broad compatibility.
...
- Next recommended gate: broader recovery parity, exact controller/VM tick-order beyond the summarized actor-frame sequence, or a broader guard/AssertSpecial confirmation. Threshold handoff, optional KFM threshold confirmation, bounded actor-frame tick order, and bounded synthetic air/ground velocity now have evidence; exact parity remains blocked.
