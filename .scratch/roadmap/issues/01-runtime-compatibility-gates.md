...
- Latest completed R2 cut: `RuntimeSpriteEffectWorld` now owns current match-runtime `SprPriority`, `PalFX`, `AfterImage`, `AfterImageTime`, and `Angle*` mutation/ticking consumed by `PlayableMatchRuntime`, preserving existing actor presentation telemetry through focused `SpriteEffectSystem` tests.
