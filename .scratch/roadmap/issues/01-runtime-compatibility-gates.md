...
- Latest completed guard cut: required `synthetic-imported-default-guard-state.json` checksum `016938a1` now gates ordered named controller/typed-operation evidence for the bounded stand guard-hit route: `150` `ChangeAnim` `Guard Shake Anim` -> `ChangeState` `Guard Shake Over` -> `151` `HitVelSet` `Apply Guard Velocity` -> `kinematic:hitvelset` -> `CtrlSet` `Regain Guard Control` -> `resource:ctrlset` -> `ChangeState` `Guard Hit Over`.
