...
- Latest completed R2 cut: `RuntimeAudioWorld` now owns bounded `PlaySnd`/`StopSnd` event insertion consumed by `PlayableMatchRuntime`, preserving the same actor sound-event history for Web Audio/debug/trace consumers through focused `AudioEventSystem` tests.
