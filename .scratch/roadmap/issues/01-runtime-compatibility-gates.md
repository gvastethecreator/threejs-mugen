...
- Latest completed R2 cut: `RuntimeReversalWorld` now owns bounded ReversalDef activation, active counter detection, and counter-result mutation for direct HitDef contact consumed by `PlayableMatchRuntime`, preserving existing ReversalDef telemetry through focused `ReversalSystem` tests.
...
- Exact direct-combat priority classes, hit/guard controller order, throws, custom-state breadth, multi-hit/combo lifetime, helper/team/redirect combat ownership, or full MUGEN/IKEMEN direct-combat parity from the `RuntimeDirectCombatWorld` ownership cut.
