...
- Latest completed R1 cut: required `synthetic-imported-default-fall-recovery-tick-order.json` checksum `e2691aab` now gates both ordered actor-frame evidence for `5050` with positive `hitFall.recoverTime` before `5210` with `recoverTime = 0` and ordered controller-event evidence for the bounded `5050` `VelAdd` -> recovery `ChangeState` -> `5210` `VelSet` -> `HitFallSet` -> `ChangeState` route.
...
- Next recommended gate: broader recovery parity, deeper controller/VM tick-order coverage beyond the current bounded controller-event sequence, or a broader guard/AssertSpecial confirmation. Threshold handoff, optional KFM ordered-threshold confirmation, bounded actor-frame tick order, bounded controller-event order, and bounded synthetic air/ground velocity now have evidence; exact parity remains blocked.
...
- Exact tick order beyond the bounded actor-frame and controller-event sequences without fixture-backed evidence.
