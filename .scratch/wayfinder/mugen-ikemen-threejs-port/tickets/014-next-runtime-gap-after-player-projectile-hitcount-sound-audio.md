...
Open. Candidate inputs: `.scratch/roadmap/issues/01-runtime-compatibility-gates.md`, `docs/WORKPLAN.md`, `docs/ROADMAP_PACKAGE_MILESTONES.md`, current `pnpm qa:trace` coverage, and blocked claims around Projectile/helper `hitcountpersist`, broader helper Projectile normal-hit contact sound breadth, exact SND lookup/playback, channel priority classes, timing/mixing/panning, helper/redirect/team ownership, renderer parity proof, target ownership, dynamic pause params, and parser-only controllers that can become typed no-crash runtime operations.
