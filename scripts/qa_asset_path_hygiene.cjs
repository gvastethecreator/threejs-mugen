...
const digestChecks = [];
const permissions = [];
const assetIds = fs.readdirSync(publicCharactersRoot, { withFileTypes: true })
  .filter((entry) => entry.isDirectory())
  .map((entry) => entry.name)
  .sort();
for (const assetId of assetIds) {
  const assetRoot = path.join(publicCharactersRoot, assetId);
  const metadataRelativePath = `characters/${assetId}/asset-permission.json`;
  let permission;
  try {
    permission = JSON.parse(fs.readFileSync(path.join(assetRoot, "asset-permission.json"), "utf8"));
  } catch (error) {
    violations.push({ path: metadataRelativePath, token: "metadata parse failed: " + error.message });
  }
  if (permission) {
    permissions.push({
    ...
    if (permission.schemaVersion !== "mugen-web-sandbox/asset-permission/v0") {
      violations.push({ path: metadataRelativePath, token: "unexpected schema" });
    }
    if (permission.assetId !== assetId || permission.ownership !== "repository-owned" || permission.permission !== "repository-owned") {
      violations.push({ path: metadataRelativePath, token: "ownership or asset identity is not repository-owned" });
    }
    if (permission.license?.expression !== "CC0-1.0" || permission.license?.verified !== true || permission.license?.sourceRef !== "LICENSE.txt") {
      violations.push({ path: metadataRelativePath, token: "license declaration is not verified CC0-1.0" });
    }
    for (const [category, files] of [["source", permission.sourceFiles], ["output", permission.outputFiles]]) {
      if (!Array.isArray(files) || files.length === 0) {
        violations.push({ path: metadataRelativePath, token: category + " file list is empty" });
        continue;
      }
      for (const fileRecord of files) {
        const result = { assetId, ...verifyPermissionFile(assetRoot, fileRecord, category) };
        digestChecks.push(result);
        if (result.status !== "pass") {
          violations.push({ path: `characters/${assetId}/${result.path ?? "unknown"}`, token: result.reason ?? "digest or byte length mismatch" });
        }
      }
    }
  }
  ...
  if (!fs.existsSync(licensePath) || !fs.readFileSync(licensePath, "utf8").includes("SPDX-License-Identifier: CC0-1.0")) {
    violations.push({ path: `characters/${assetId}/LICENSE.txt`, token: "missing SPDX CC0-1.0 declaration" });
  }
}
