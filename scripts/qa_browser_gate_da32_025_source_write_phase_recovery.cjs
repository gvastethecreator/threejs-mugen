/**
 * DA32-025: show the durable source-write phase after a write boundary has
 * closed but before a receipt settles, then replay the exact preimage.
 */
...
const reportPath = path.join(repoRoot, "docs/evidence/da32/da32-025-source-write-phase-recovery-browser-gate.json");
...
const intent = {
  ...
  intentId: "source-intent:da32-025:phase-source:chars/phase/phase.cns:pending",
  path: "chars/phase/phase.cns",
  preimageBytes: [...Buffer.from(sourceText, "utf8")],
  preimageSha256: fnvHex(Buffer.from(sourceText, "utf8")),
  projectId: "da32-025-source-write-phase-recovery",
  sourcePackageId: "phase-source",
  draftDigest: "fnv1a32:phase-write",
...
async function main() {
  ...
  const subject = buildSubjectEnvelope(repoRoot, {
    probePaths: ["scripts/qa_browser_gate_da32_025_source_write_phase_recovery.cjs"],
  ...
  try {
    ...
    const report = {
      schema: "Da32StudioSourceWritePhaseRecoveryBrowserGate/v1",
      id: "DA32-025",
      ...
      claimCeiling: subject.provisional
        ? "provisional source-write phase recovery observations for named viewports"
        : "the named Studio route reads a durable incomplete source-write phase and replays its exact preimage without settling a receipt",
      claims: {
        allowed: ok
          ? [
              "IndexedDB retains the write-closed phase and write byte length before receipt settlement",
              "the Studio bridge and visible recovery surface expose the incomplete phase",
              "Load preimage replays exact persisted bytes while leaving the intent pending",
              "the named desktop and mobile routes perform no source-handle write and have no horizontal overflow or unexpected page errors",
            ]
          : [],
        blocked: [
...
async function runViewport(browser, base, projectPath, options) {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__MUGEN_WEB_SANDBOX__?.project?.id === "da32-025-source-write-phase-recovery",
      null,
      { timeout: 30_000 },
    );
    ...
    const before = await page.evaluate(async (database) => ({
      ...
      bodyHasPhase: (document.body.textContent ?? "").includes("write-closed"),
    ...
    steps.durablePhase = beforeIntent?.phase === "write-closed" && beforeIntent?.writeByteLength === intent.writeByteLength;
    steps.bridgePhase = before.bridge?.phase === "write-closed" && before.bridge?.result === undefined;
    steps.visiblePhase = before.bodyHasPhase && before.phaseNode === "write-closed";
    ...
    steps.pendingAfterRecovery = after.bridge?.intentId === intent.intentId && after.bridge?.phase === "write-closed" && after.bridge?.result === undefined && afterIntent?.phase === "write-closed";
    steps.noSourceHandleWrite = after.sourceHandles.every((handle) => handle.sourcePackageId !== intent.sourcePackageId || (handle.persisted === false && handle.state === "not-linked" && handle.canRead === false));
    ...
    const screenshot = path.join(outDir, `da32-025-source-write-phase-recovery-${options.id}.png`);
    ...
    return {
      ...
      after: {
        ...
        sourceHandles: after.sourceHandles,
        editorValue: after.editorValue,
        intent: afterIntent,
      },
...
function writeRecoveryProject() {
  const projectPath = path.join(outDir, "da32-025-source-write-phase-recovery-project.json");
  const project = {
    schemaVersion: "mugen-web-sandbox/project/v0",
    id: "da32-025-source-write-phase-recovery",
    name: "DA32-025 Source Write Phase Recovery",
    ...
    sourcePackages: [{
      id: "phase-source",
      name: "Phase Recovery Source",
      kind: "folder",
      fileCount: 1,
      status: "missing",
      characterId: "nova-boxer",
      characterName: "Nova Boxer",
      defPath: "chars/phase/phase.def",
      stageIds: [],
      stageDefPaths: [],
      requiredPaths: ["chars/phase/phase.cns"],
    }],
