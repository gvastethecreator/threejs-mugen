...
Current Common1 recovery-threshold cut: required `synthetic-imported-default-fall-recovery-too-early.json` checksum `050e7e3c` proves `command = "recovery"` can be active while `fall.recovertime` is still positive without entering `5210`; the defender remains in `5050`. This is a bounded early-input reject gate, not exact recovery timing, velocity, or tick-order parity.
