...
Recovery-input coverage now includes both the positive synthetic `5050 -> 5210 -> 0` route, optional official KFM air/ground positive routes, and synthetic plus optional official KFM negative too-early routes where the command is detected but recovery states are forbidden while `fall.recovertime` remains positive.
