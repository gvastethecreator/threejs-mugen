...
Current AssertSpecial guard-deny cut: `synthetic-imported-assertspecial-guarddeny.json` checksum `f636748d` proves a defender-owned `NoWalk` + `NoStandGuard` route can keep a held-back imported defender in its owner state, deny standing guard, resolve an otherwise `A`-guardable `HitDef` as a hit, record hitstun actor-frame evidence, and avoid guard event evidence. Exact AssertSpecial lifetime, crouch/air variants, global/team/helper ownership, pause layering, priority, and full MUGEN/IKEMEN guard parity remain future work.
