...
4. Read `docs/ROADMAP_OPERATIONAL_CHECKLIST.md` before choosing verification gates.
5. If the user asks "how far are we?", read `docs/PORT_COMPLETION_SCORECARD.md` and `docs/PROGRESS_TRACKER.md` before answering.
6. If picking implementation work, open the linked `.scratch/roadmap/issues/<NN>-*.md` issue and keep claim allowed / claim blocked wording aligned.
7. Before editing, identify whether the work is runtime trace, UI/visual, Studio/product, generated assets, IKEMEN scanner, modular contract, or docs-only; use the matching checklist row.
8. Close with gates, docs/backlog update, and a checkpoint commit when the user has asked for persistent progress.
