...
- Latest-implementation truth: `docs/BUILD_EXECUTION_BACKLOG.md` wins for recent closeout history; package and roadmap docs must be refreshed when they point at already-closed gates. Current latest closed ownership cut is `RuntimeOrientationWorld` unless the backlog proves a newer one.
...
- Immediate next slice after the `RuntimeOrientationWorld` checkpoint: R1 Common1/FightFX precision or R2 MatchWorld helper/effect/combat ownership, whichever can produce focused evidence without broad drift.
