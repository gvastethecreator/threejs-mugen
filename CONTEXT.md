...
- Latest helper/effect checkpoint: required `synthetic-imported-helper-modifyprojectile.json` checksum `77df008b` now proves bounded helper-local `ModifyProjectile` mutation against helper-parented owner-side Projectile actors by static id after helper-local spawn. Current trace aggregate is 182/182 artifacts, 162 required and 20 optional. Previous `synthetic-imported-helper-numproj.json` checksum `4f8612b0` remains the helper-local projectile-count proof against helper-parented Projectiles only.
