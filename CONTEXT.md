...
- Latest R2 player Projectile TargetState checkpoint: required `synthetic-imported-projectile-targetstate.json` checksum `dd1c7962` proves a player-owned Projectile-only hit can record owner target link `p1 -> p2 / 77`, then later execute owner-local `TargetState value = 888` and route P2 through attacker-owned custom-state data `888 -> 889` before `SelfState` returns P2 to state `0`/control. Evidence includes Projectile effect anim `913`, lifecycle spawn/remove, effect payload `ownerId = p1` / `effectId = 77` / `hasHit = true` / `removalReason = hit` / `terminalReason = hit`, required executed `ChangeState`, `TargetState`, and `SelfState`, custom-owner actor frames for P2, final P2 `life = 969`, and no direct `HitDef` controller evidence in the isolated fixture. Current trace aggregate is 206/206 artifacts, 186 required and 20 optional. Previous player Projectile Target-controller proof remains required as `synthetic-imported-projectile-target-controllers.json` checksum `8c7bd6c2`, and previous player Projectile target redirect proof remains required as `synthetic-imported-projectile-target-redirect.json` checksum `cd099094`. This adds player-owned Projectile TargetState into owner-backed state data to the existing direct player Target*, player Projectile target trigger reads, player Projectile Target side effects, helper direct/Projectile explicit/default/bare target memory, bounded helper-owned direct-HitDef and explicit/default helper-parented Projectile Target side effects, and helper TargetState evidence; direct HitDef plus Projectile mixing, helper-owned custom state tables, throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact final-animation parity, visual parity, and full target/combat/custom-state parity remain blocked.
