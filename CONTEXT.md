...
- Latest R2 player Projectile Target-controller checkpoint: required `synthetic-imported-projectile-target-controllers.json` checksum `8c7bd6c2` proves a player-owned Projectile-only hit can record owner target link `p1 -> p2 / 77`, then execute owner-local `TargetLifeAdd`, `TargetPowerAdd`, `TargetVelSet`, `TargetVelAdd`, `TargetFacing`, `TargetBind`, and `TargetDrop` against P2 without direct `HitDef` controller evidence in the isolated fixture. Evidence includes Projectile effect anim `912`, lifecycle spawn/remove, effect payload `ownerId = p1` / `effectId = 77` / `hasHit = true` / `removalReason = hit` / `terminalReason = hit`, TargetBind offset `36,-12`, P1 final `targetCount = 0`, and final P2 `life = 949` / `power = 40`. Current trace aggregate is 205/205 artifacts, 185 required and 20 optional. Previous player Projectile target redirect proof remains required as `synthetic-imported-projectile-target-redirect.json` checksum `cd099094`. This adds player-owned Projectile Target side effects to the existing direct player Target*, player Projectile target trigger reads, helper direct/Projectile explicit/default/bare target memory, bounded helper-owned direct-HitDef and explicit/default helper-parented Projectile Target side effects, and helper TargetState evidence; direct HitDef plus Projectile mixing, helper-owned custom state tables, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, visual parity, and full target/combat parity remain blocked.
