...
- Latest R2 helper Projectile default Target-controller checkpoint: required `synthetic-imported-helper-projectile-default-target-controllers.json` checksum `6d8c51dd` proves a helper-parented owner-side Projectile with omitted `projid` / `id` can hit P2, default target memory to id `0`, expose owner target link `p1 -> p2 / 0`, mirror target memory into helper target memory `p1-helper-0 -> p2 / 0`, branch helper-local state through `NumTarget(0)` plus `Target(0), Life`, then execute bounded helper-owned `TargetLifeAdd`, `TargetPowerAdd`, `TargetVelSet`, `TargetVelAdd`, `TargetFacing`, `TargetBind`, and `TargetDrop`. Evidence includes binding offset `36,-12`, final P2 `life = 958` / `power = 40`, helper payload `targetCount = 0`, projectile payload `parentId = p1-helper-0`, and shared `S5,8` / FightFX `F7014` contact package. Current trace aggregate is 200/200 artifacts, 180 required and 20 optional. Previous explicit Projectile Target-controller proof remains required as `synthetic-imported-helper-projectile-target-controllers.json` checksum `ebf5099a`; previous helper TargetState proof remains required as `synthetic-imported-helper-targetstate.json` checksum `011633b8`; previous helper target-controller proof remains required as `synthetic-imported-helper-target-controllers.json` checksum `61f4c61e`; previous helper bare `Target, Life` proof remains required as `synthetic-imported-helper-bare-target.json` checksum `15f3c1db`; previous player bare `Target, Life` proof remains required as `synthetic-imported-bare-target-redirect.json` checksum `f9c90aa8`; previous default `Target(0), Life` proof remains required as `synthetic-imported-default-target-redirect.json` checksum `d43caabf`; previous default `NumTarget(0)` proof remains required as `synthetic-imported-default-numtarget.json` checksum `5869ebbd`; previous helper direct default target proof remains required as `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default target proof remains required as `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is direct player bare/default/static/dynamic target trigger reads plus helper direct/Projectile explicit/default/bare target memory plus bounded helper-owned direct-HitDef and explicit/default helper-parented Projectile Target side effects plus helper-local TargetState into owner-backed state data only; helper-owned custom state tables, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, visual parity, and full target/combat parity remain blocked.
