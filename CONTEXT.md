...
- Latest R2 ownership checkpoint: required `synthetic-imported-helper-default-target.json` checksum `e1bcced0` proves a helper-owned direct `HitDef` without `id` defaults to target id `0`, can remember P2 in helper-owned target memory, expose target link `p1-helper-0 -> p2 / 0`, and branch helper-local state through `NumTarget(0)` plus `Target(0), Life` into `1229/961`. Current trace aggregate is 192/192 artifacts, 172 required and 20 optional. Previous helper Projectile default target proof remains required as `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is helper direct `HitDef` plus helper-parented Projectile explicit/default target memory only; helper `Target*` mutation controllers, helper custom-state targets, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, and full helper target/combat parity remain blocked.
