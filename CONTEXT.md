...
- Latest R2 ownership checkpoint: required `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc` proves a helper-parented owner-side Projectile without `projid`/`id` defaults to target id `0`, can remember P2 in both owner target memory and helper-owned target memory, expose target links `p1 -> p2 / 0` plus `p1-helper-0 -> p2 / 0`, and branch helper-local state through `NumTarget(0)` plus `Target(0), Life` into `1227/958`. Current trace aggregate is 191/191 artifacts, 171 required and 20 optional. Previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`, and previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is helper HitDef plus helper-parented Projectile explicit/default target memory only; helper `Target*` mutation controllers, helper custom-state targets, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, and full helper target/combat parity remain blocked.
