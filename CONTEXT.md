...
- Latest R2 target-memory checkpoint: required `synthetic-imported-default-numtarget.json` checksum `5869ebbd` proves a direct player `HitDef` without `id` defaults to target id `0`, exposes target link `p1 -> p2 / 0`, and branches owner state `268` through `NumTarget(0)`. Current trace aggregate is 193/193 artifacts, 173 required and 20 optional. Previous helper direct default target proof remains required as `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default target proof remains required as `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is direct player default `HitDef` target id plus helper direct/Projectile explicit/default target memory only; `Target*` mutation breadth, helper custom-state targets, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, and full target/combat parity remain blocked.
