...
- Latest R2 target-memory checkpoint: required `synthetic-imported-helper-bare-target.json` checksum `15f3c1db` proves a helper-owned direct `HitDef` with target id `8878` exposes helper target link `p1-helper-0 -> p2 / 8878` and branches helper state `1230/962` through bare `Target, Life <= 965`. Current trace aggregate is 196/196 artifacts, 176 required and 20 optional. Previous player bare `Target, Life` proof remains required as `synthetic-imported-bare-target-redirect.json` checksum `f9c90aa8`; previous default `Target(0), Life` proof remains required as `synthetic-imported-default-target-redirect.json` checksum `d43caabf`; previous default `NumTarget(0)` proof remains required as `synthetic-imported-default-numtarget.json` checksum `5869ebbd`; previous helper direct default target proof remains required as `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default target proof remains required as `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is direct player bare/default/static/dynamic target trigger reads plus helper direct/Projectile explicit/default/bare target memory only; `Target*` mutation breadth, helper/projectile target redirects, helper custom-state targets, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, visual parity, and full target/combat parity remain blocked.
