...
- Latest R2 target-memory checkpoint: required `synthetic-imported-default-target-redirect.json` checksum `d43caabf` proves a direct player `HitDef` without `id` defaults to target id `0`, exposes target link `p1 -> p2 / 0`, and branches owner state `269` through `Target(0), Life < 1000`. Current trace aggregate is 194/194 artifacts, 174 required and 20 optional. Previous default `NumTarget(0)` proof remains required as `synthetic-imported-default-numtarget.json` checksum `5869ebbd`; previous helper direct default target proof remains required as `synthetic-imported-helper-default-target.json` checksum `e1bcced0`; previous helper Projectile default target proof remains required as `synthetic-imported-helper-projectile-default-target.json` checksum `b1541afc`; previous explicit Projectile target proof remains required as `synthetic-imported-helper-projectile-target.json` checksum `1a44cc04`; previous direct helper target proof remains required as `synthetic-imported-helper-target.json` checksum `68f95b67`. This is direct player default `HitDef` target id plus owner-local `NumTarget(0)` / `Target(0), Life` trigger reads and helper direct/Projectile explicit/default target memory only; `Target*` mutation breadth, helper/projectile target redirects, helper custom-state targets, helper throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact helper hitpause/tick order, exact helper `HitDef`/Projectile lifetime parity, and full target/combat parity remain blocked.
