...
- Latest R2 player Projectile default TargetState checkpoint: required `synthetic-imported-projectile-default-targetstate.json` checksum `8f35f1fa` proves a player-owned Projectile-only hit with omitted `projid` / `id` defaults target memory to id `0`, then later executes owner-local `TargetState value = 888` and routes P2 through attacker-owned custom-state data `888 -> 889` before `SelfState` returns P2 to state `0`/control. Evidence includes Projectile effect anim `914`, lifecycle spawn/remove, effect payload `ownerId = p1` / `effectId = 0` / `hasHit = true` / `removalReason = hit` / `terminalReason = hit`, required executed `ChangeState`, `TargetState`, and `SelfState`, custom-owner actor frames for P2, final P2 `life = 969`, and no direct `HitDef` controller evidence in the isolated fixture. Current trace aggregate is 207/207 artifacts, 187 required and 20 optional. Previous explicit-id player Projectile TargetState proof remains required as `synthetic-imported-projectile-targetstate.json` checksum `dd1c7962`, previous player Projectile Target-controller proof remains required as `synthetic-imported-projectile-target-controllers.json` checksum `8c7bd6c2`, and previous player Projectile target redirect proof remains required as `synthetic-imported-projectile-target-redirect.json` checksum `cd099094`. This adds player-owned Projectile default-id TargetState into owner-backed state data to the existing direct player Target*, player Projectile target trigger reads, player Projectile Target side effects, explicit-id player Projectile TargetState, helper direct/Projectile explicit/default/bare target memory, bounded helper-owned direct-HitDef and explicit/default helper-parented Projectile Target side effects, and helper TargetState evidence; direct HitDef plus Projectile mixing, helper-owned custom state tables, throws, teams/simul, multi-target/helper-owned opponent selection, exact target lifetime/tick order, exact final-animation parity, visual parity, and full target/combat/custom-state parity remain blocked.
